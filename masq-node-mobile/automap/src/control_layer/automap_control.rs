...
impl AutomapControl for AutomapControlReal {
    fn get_public_ip(&mut self) -> Result<IpAddr, AutomapError> {
        ...
        debug!(self.logger, "Public IP discovered; address redacted");
    ...
    fn delete_mappings(&mut self) -> Result<(), AutomapError> {
        match &self.inner_opt {
            ...
            Some(inner) => {
                ...
                let stop_result = transactor.stop_housekeeping_thread().map(|_| ());
                if errors.is_empty() {
                    stop_result
                } else {
                    Err(errors[0].clone())
...
impl AutomapControlReal {
        ...
        if let Some(change_handler) = housekeeping_tools.change_handler_opt.take() {
            ...
            match transactor.start_housekeeping_thread(change_handler, router_ip) {
                ...
                Err(e) => {
                    ...
                    match Self::put_change_handler_back(transactor, &mut housekeeping_tools) {
                        Ok(()) => Err(e),
    ...
    fn prepare_and_perform_experiment<T: PartialEq + Debug>(
        ...
        let transactors = self.transactors.borrow();
        let transactor = transactors
            .get(inner.transactor_idx)
            .ok_or(AutomapError::HousekeeperCrashed)?;
    ...
    fn choose_working_protocol<T: PartialEq + Debug>(
            ...
            let transactor = transactors_ref_mut
                ...
                .ok_or_else(|| {
                    AutomapError::ProtocolError(
                        "Configured Automap protocol has no transactor".to_string(),
                    )
                })?;
            if let Ok((router_ip, t)) = self.try_protocol(transactor.as_mut(), &experiment) {
                let transactor_idx =
                    Self::find_transactor_index(transactors_ref_mut.as_slice(), usual_protocol)
                        .ok_or(AutomapError::HousekeeperCrashed)?;
        ...
        match protocol_router_ip_and_experimental_outcome_result {
            Ok((protocol, router_ip, t)) => {
                let transactor_idx =
                    Self::find_transactor_index(transactors_ref_mut.as_slice(), protocol)
                        .ok_or(AutomapError::HousekeeperCrashed)?;
                self.inner_opt = Some(AutomapControlRealInner {
                    router_ip,
                    transactor_idx,
...
pub fn replace_transactor(
    ...
    let protocol = transactor.protocol();
    let mut transactors = subject.transactors.borrow_mut();
    match AutomapControlReal::find_transactor_index(transactors.as_slice(), protocol) {
        Some(idx) => transactors[idx] = transactor,
        None => transactors.push(transactor),
    }
    drop(transactors);
    subject
}
...
mod tests {
    ...
    #[test]
    fn choose_working_protocol_reports_a_missing_configured_transactor() {
        let mut subject = AutomapControlReal::new(Some(AutomapProtocol::Pcp), Box::new(|_| {}));
        subject.transactors.borrow_mut().clear();
        let experiment: TransactorExperiment<String> =
            Box::new(|_, _| panic!("Experiment must not run without a transactor"));
    ...
    #[test]
    fn invalid_selected_transactor_index_returns_housekeeper_crashed() {
        let mut subject = AutomapControlReal::new(None, Box::new(|_| {}));
        let invalid_inner = AutomapControlRealInner {
            router_ip: *ROUTER_IP,
            transactor_idx: usize::MAX,
        };
        let experiment: TransactorExperiment<String> =
            Box::new(|_, _| panic!("Experiment must not run with an invalid transactor index"));

        let experiment_result = subject.prepare_and_perform_experiment(&invalid_inner, experiment);
        subject.inner_opt = Some(invalid_inner);
        let delete_result = subject.delete_mappings();

        assert_eq!(experiment_result, Err(AutomapError::HousekeeperCrashed));
        assert_eq!(delete_result, Err(AutomapError::HousekeeperCrashed));
    }

    #[test]
    fn replace_transactor_adds_a_previously_missing_protocol_without_panicking() {
        let subject = AutomapControlReal::new(None, Box::new(|_| {}));
        subject.transactors.borrow_mut().clear();

        let subject =
            replace_transactor(subject, Box::new(TransactorMock::new(AutomapProtocol::Pcp)));

        let transactors = subject.transactors.borrow();
        assert_eq!(transactors.len(), 1);
        assert_eq!(transactors[0].protocol(), AutomapProtocol::Pcp);
        assert_eq!(
            AutomapControlReal::find_transactor_index(transactors.as_slice(), AutomapProtocol::Pmp),
            None
        );
    }
    ...
    #[test]
    fn add_mapping_reports_a_missing_housekeeping_commander_without_panicking() {
        let subject = make_null_subject();
        let mut transactor = TransactorMock::new(AutomapProtocol::Pcp).add_mapping_result(Ok(1000));
        transactor.housekeeping_thread_started = true;
        let mut subject = replace_transactor(subject, Box::new(transactor));
        subject.housekeeping_tools = RefCell::new(HousekeepingTools {
            change_handler_opt: None,
        ...
        assert!(subject.hole_ports.is_empty());
    }
    ...
    #[test]
    fn delete_mappings_reports_housekeeper_stop_failure_without_panicking() {
        let subject = make_active_two_port_subject(1);
        let transactor = TransactorMock::new(AutomapProtocol::Pmp)
            .delete_mapping_result(Ok(()))
            .delete_mapping_result(Ok(()))
            .stop_housekeeping_thread_result(Err(AutomapError::HousekeeperCrashed));
        let mut subject = replace_transactor(subject, Box::new(transactor));
    ...
    #[test]
    fn maybe_start_housekeeper_reports_stop_failure_without_panicking() {
        let subject = make_null_subject();
        subject.housekeeping_tools.borrow_mut().change_handler_opt = Some(Box::new(|_| ()));
        let mut transactor = TransactorMock::new(AutomapProtocol::Igdp)
            .start_housekeeping_thread_result(Err(AutomapError::HousekeeperAlreadyRunning))
    ...
    #[test]
    fn try_protocol_reports_housekeeper_stop_failure_without_panicking() {
        let mut transactor = TransactorMock::new(AutomapProtocol::Pmp)
            .find_routers_result(Ok(vec![*ROUTER_IP]))
            .start_housekeeping_thread_result(Ok(unbounded().0))
            .stop_housekeeping_thread_result(Err(AutomapError::HousekeeperCrashed));
