...
impl Transactor for PcpTransactor {
    ...
    fn add_permanent_mapping(
        ...
        Err(AutomapError::PermanentMappingError(
            "PCP does not support permanent mappings".to_string(),
        ))
    }
    ...
    fn stop_housekeeping_thread(&mut self) -> Result<ChangeHandler, AutomapError> {
        ...
        let stopper = match self.housekeeper_commander_opt.take() {
...
impl PcpTransactor {
    fn inner(&self) -> Result<MutexGuard<PcpTransactorInner>, AutomapError> {
        self.inner_arc
            .lock()
            .map_err(|_| AutomapError::HousekeeperCrashed)
    }
                    ...
                    if since_last_remapped.gt(&mapping_config.remap_interval) {
                        let inner = match inner_arc.lock() {
                            ...
                            Err(_) => {
                                error!(
                                    logger,
                                    "PCP housekeeping state is unavailable; stopping remapping"
                                );
...
impl MappingTransactor for MappingTransactorReal {
    fn transact(
        ...
        debug!(
            self.logger,
            "Mapping transaction: port {} through router for {} seconds; address redacted",
        ...
        let request_len = packet.marshal(&mut buffer).map_err(|e| {
            AutomapError::ProtocolError(format!("Could not marshal PCP request: {:?}", e))
        })?;
        let socket = match socket_result {
            ...
            Err(e) => {
                warning!(
                    self.logger,
                    "Error while connecting to router; address and details redacted"
                );
        ...
        match socket.send_to(&buffer[0..request_len], router_addr) {
            ...
            Err(e) => {
                warning!(
                    self.logger,
                    "Error while transmitting to router; address and details redacted"
                );
        ...
        let response = match socket.recv_from(&mut buffer) {
            Ok((len, _peer_addr)) => match PcpPacket::try_from(&buffer[0..len]) {
                ...
                Err(e) => {
                    warning!(
                        self.logger,
                        "Error while parsing packet from router; address and details redacted"
                    );
            ...
            Err(e) => {
                warning!(
                    self.logger,
                    "Error while receiving from router; address and details redacted"
                );
                return Err(AutomapError::SocketReceiveError(
                    AutomapErrorCause::Unknown(format!("{:?}", e)),
                ));
...
impl MappingTransactorReal {
    ...
    fn compute_mapping_result(
        ...
        let result_code = response.result_code_opt.ok_or_else(|| {
            AutomapError::ProtocolError("PCP response omitted its result code".to_string())
        })?;
        if result_code != ResultCode::Success {
            let msg = format!("{:?}", result_code);
            return if result_code.is_permanent() {
                let e = AutomapError::PermanentMappingError(msg);
                warning!(logger, "Router complained: {:?}; address redacted", e);
                Err(e)
            } else {
        ...
        let opcode_data = response
            ...
            .ok_or_else(|| {
                AutomapError::ProtocolError(
                    "PCP Map response contained incompatible opcode data".to_string(),
                )
            })?;
...
mod tests {
    ...
    fn mapping_transaction_handles_socket_factory_error() {
        ...
        TestLogHandler::new().exists_log_containing(
            "WARN: PcpTransactor: Error while connecting to router; address and details redacted",
        );
    }

    #[test]
    fn mapping_transaction_handles_receive_timeout_configuration_error_without_panicking() {
        init_test_logging();
        let router_ip = IpAddr::from_str("192.168.0.255").unwrap();
        ...
        TestLogHandler::new().exists_log_containing(
            "WARN: PcpTransactor: Error configuring UDP receive timeout; address and details redacted",
        );
    }
    ...
    fn mapping_transaction_handles_send_to_error() {
        ...
        TestLogHandler::new().exists_log_containing(
            "WARN: PcpTransactor: Error while transmitting to router; address and details redacted",
        );
    }
    ...
    fn mapping_transaction_handles_recv_from_error() {
        ...
        TestLogHandler::new().exists_log_containing(
            "WARN: PcpTransactor: Error while receiving from router; address and details redacted",
        );
    }
    ...
    fn mapping_transaction_handles_packet_parse_error() {
        ...
        TestLogHandler::new().exists_log_containing(
            "WARN: PcpTransactor: Error while parsing packet from router; address and details redacted",
        );
    }
    ...
    fn mapping_transaction_handles_wrong_direction() {
        ...
        TestLogHandler::new().exists_log_containing(
            "WARN: PcpTransactor: Router is misbehaving: ProtocolError([REDACTED]); address redacted",
        );
    }
    ...
    fn get_public_ip_handles_failure() {
        ...
        TestLogHandler::new().exists_log_containing(
            "WARN: PcpTransactor: Router complained: PermanentMappingError([REDACTED]); address redacted",
        );
    }
    ...
    fn temporary_mapping_errors_are_handled() {
        ...
        TestLogHandler::new().exists_log_containing(
            "WARN: PcpTransactor: Router complained: TemporaryMappingError([REDACTED]); address redacted",
        );
    }
    ...
    fn compute_mapping_result_rejects_incomplete_or_incompatible_response_shape() {
        let router_addr = SocketAddr::from_str("192.168.0.248:5351").unwrap();
        let logger = Logger::new("PcpTransactor");
        let mut missing_result = vanilla_response();
        missing_result.opcode = Opcode::Map;
        missing_result.result_code_opt = None;
        missing_result.opcode_data = vanilla_map_response();

        let missing_result_error =
            MappingTransactorReal::compute_mapping_result(missing_result, router_addr, &logger);

        assert_eq!(
            missing_result_error,
            Err(AutomapError::ProtocolError(
                "PCP response omitted its result code".to_string()
            ))
        );

        let mut incompatible_data = vanilla_response();
        incompatible_data.opcode = Opcode::Map;
        incompatible_data.result_code_opt = Some(ResultCode::Success);
        incompatible_data.opcode_data = Box::new(UnrecognizedData::new());

        let incompatible_data_error =
            MappingTransactorReal::compute_mapping_result(incompatible_data, router_addr, &logger);
    ...
    fn add_mapping_reports_missing_housekeeper_without_panicking() {
        let mapping_transactor =
            MappingTransactorMock::new().transact_result(Ok((8000, *vanilla_map_response())));
        let mut subject = PcpTransactor::default();
        subject.inner_arc.lock().unwrap().mapping_transactor = Box::new(mapping_transactor);
        subject.housekeeper_commander_opt = None;

        let result = subject.add_mapping(IpAddr::from_str("1.2.3.4").unwrap(), 6666, 10000);

        assert_eq!(result, Err(AutomapError::HousekeeperCrashed));
    }
    ...
    #[test]
    fn poisoned_state_returns_and_reports_housekeeper_crashed_without_panicking() {
        let subject = PcpTransactor::default();
        let poisoned_inner_arc = subject.inner_arc.clone();
        let poisoner = poisoned_inner_arc.clone();
        assert!(thread::spawn(move || {
            let _guard = poisoner.lock().unwrap();
            panic!("poison PCP state");
        })
        .join()
        .is_err());

        assert_eq!(
            subject.get_public_ip(IpAddr::from_str("1.2.3.4").unwrap()),
            Err(AutomapError::HousekeeperCrashed)
        );
        assert_eq!(
            subject.add_mapping(IpAddr::from_str("1.2.3.4").unwrap(), 6689, 600),
            Err(AutomapError::HousekeeperCrashed)
        );
        assert_eq!(
            subject.delete_mapping(IpAddr::from_str("1.2.3.4").unwrap(), 6689),
            Err(AutomapError::HousekeeperCrashed)
        );
    ...
    fn stop_housekeeping_thread_handles_missing_housekeeper_commander() {
        ...
        TestLogHandler::new().exists_log_containing(
            "WARN: PcpTransactor: Cannot stop housekeeping thread: commander is unavailable",
        );
    }
    ...
    fn stop_housekeeping_thread_handles_missing_join_handle() {
        ...
        assert_eq!(result.err().unwrap(), AutomapError::HousekeeperCrashed);
        TestLogHandler::new().exists_log_containing(
            "WARN: PcpTransactor: Cannot stop housekeeping thread: join handle is unavailable",
        );
    }
    ...
    fn thread_guts_complains_if_remapping_fails() {
        ...
        TestLogHandler::new().exists_log_containing(
            "ERROR: thread_guts_complains_if_remapping_fails: Remapping failure: TemporaryMappingError([REDACTED])",
        );
    }
