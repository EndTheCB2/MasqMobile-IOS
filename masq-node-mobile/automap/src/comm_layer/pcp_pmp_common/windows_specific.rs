            ...
            firsts
                ...
                .map(
                    |(first_elements, ip_addr_opt)| match (first_elements, ip_addr_opt) {
                        (_, Some(IpAddr::V4(ipv4_addr))) => Ok(IpAddr::V4(ipv4_addr)),
                        (first_elements, _) => {
                            let ip_addr_maybe_with_scope_id =
                                first_elements.get(2).ok_or_else(|| {
                                    AutomapError::FindRouterError(
                                        "Could not parse a default gateway from ipconfig output"
                                            .to_string(),
                                    )
                                })?;
                            let ip_addr_str = ip_addr_maybe_with_scope_id
                                .split('%')
                                .next()
                                .ok_or_else(|| {
                                    AutomapError::FindRouterError(
                            ...
                            IpAddr::from_str(ip_addr_str).map_err(|_| {
                                AutomapError::FindRouterError(
                                    "Could not parse a default gateway from ipconfig output"
                                        .to_string(),
                                )
...
";
    ...
    #[test]
    fn find_routers_rejects_ipconfig_output_that_cant_be_parsed_without_panicking() {
        let route_n_output = "
   Booga
...
";
        ...
        assert_eq!(
            ...
            Err(AutomapError::FindRouterError(
                "Could not parse a default gateway from ipconfig output".to_string()
            ))
        );
    }
