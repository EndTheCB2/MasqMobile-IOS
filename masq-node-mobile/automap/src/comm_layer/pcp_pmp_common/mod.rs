...
use masq_lib::utils::find_free_port_0000;
use std::io;
...
pub fn find_routers() -> Result<Vec<IpAddr>, AutomapError> {
    ...
    #[cfg(not(any(target_os = "linux", target_os = "windows", target_os = "macos")))]
    {
        Err(AutomapError::ProtocolError(
            "Router discovery is unavailable on this platform".to_string(),
        ))
    }
}
...
pub mod tests {
    ...
    fn free_port_factory_works() {
        ...
        let mut verified_port_opt = None;
        for _ in 0..10 {
            let port = subject.make();
            if UdpSocket::bind(make_local_socket_address(true, port)).is_ok() {
                verified_port_opt = Some(port);
                break;
            }
        }
        assert!(
            verified_port_opt.is_some(),
            "No wildcard-bindable port remained available across ten immediate probes"
        );
    }
