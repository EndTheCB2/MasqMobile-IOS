...
impl Transactor for IgdpTransactor {
    fn find_routers(&self) -> Result<Vec<IpAddr>, AutomapError> {
        ...
        let gateway = inner
            .gateway_opt
            .as_ref()
            .ok_or(AutomapError::CantFindDefaultGateway)?;
        Ok(vec![IpAddr::V4(*gateway.get_gateway_addr().ip())])
    }
    ...
    fn get_public_ip(&self, _router_ip: IpAddr) -> Result<IpAddr, AutomapError> {
        ...
        let gateway = inner
            ...
            .as_ref()
            .ok_or(AutomapError::CantFindDefaultGateway)?;
        match gateway.as_ref().get_external_ip() {
            Ok(ip) => {
                inner.public_ip_opt.replace(ip);
                Ok(IpAddr::V4(ip))
            }
            Err(e) => {
                warning!(inner.logger, "WARN: IgdpTransactor: Error getting public IP from router; address and details redacted");
    ...
    fn delete_mapping(&self, _router_ip: IpAddr, hole_port: u16) -> Result<(), AutomapError> {
            ...
            .remove_port(PortMappingProtocol::TCP, hole_port)
        {
            ...
            Err(e) => {
                warning!(
                    inner.logger,
                    "Can't delete mapping of port {} through router; address and details redacted",
                    hole_port
                );
    ...
    fn start_housekeeping_thread(
        ...
        let public_ip_poll_delay = {
            ...
            if inner.housekeeping_commander_opt.is_some() {
                info!(
                    inner.logger,
                    "Housekeeping thread for router is already running; address redacted"
                );
    ...
    fn stop_housekeeping_thread(&mut self) -> Result<ChangeHandler, AutomapError> {
        let stopper = {
            ...
            match inner.housekeeping_commander_opt.take() {
        ...
        let change_handler = match stopper.try_send(HousekeepingThreadCommand::Stop) {
            ...
            Err(_) => {
                warning!(Logger::new("IgdpTransactor"), "Tried to stop housekeeping thread that had already disconnected from the commander");
                return Err(AutomapError::HousekeeperCrashed);
            }
        };
        Ok(change_handler)
...
impl IgdpTransactor {
    ...
    fn ensure_gateway(&self) -> Result<(), AutomapError> {
        ...
        let gateway = match gateway_factory.make(SearchOptions::default()) {
            ...
            Err(_) => {
                warning!(
                    inner.logger,
                    "Error locating routers on the LAN; details redacted"
                );
    ...
    fn inner(&self) -> Result<MutexGuard<IgdpTransactorInner>, AutomapError> {
        self.inner_arc
            .lock()
            .map_err(|_| AutomapError::HousekeeperCrashed)
    }
    ...
    fn thread_guts_iteration(
        ...
        let inner = match inner_arc.lock() {
            Ok(inner) => inner,
            Err(_) => {
                change_handler(AutomapChange::Error(AutomapError::HousekeeperCrashed));
                return false;
            }
        };
    ...
    fn add_mapping(
        ...
        match gateway.add_port(
            ...
            Err(e) => {
                warning!(
                    self.logger,
                    "Failed to add {}sec mapping for port {}; details redacted",
                    lifetime,
                    hole_port
                );
...
mod tests {
    ...
    fn find_routers_handles_error() {
        ...
        TestLogHandler::new().exists_log_containing(
            "WARN: find_routers_handles_error: Error locating routers on the LAN; details redacted",
        );
    }
    ...
    fn get_public_ip_handles_error() {
        ...
        let index = logs.exists_log_containing(
            "WARN: IgdpTransactor: Error getting public IP from router; address and details redacted",
        );
        let log = logs.get_log_at(index);
        assert!(!log.contains("192.168.0.255"));
        assert!(!log.contains("ActionNotAuthorized"));
    }
    ...
    fn add_mapping_reports_missing_housekeeper_without_panicking() {
        let gateway = GatewayWrapperMock::new();
        let mapping_adder = MappingAdderMock::new().add_mapping_result(Ok(617));
        let subject = IgdpTransactor::new();
        {
            let mut inner = subject.inner_arc.lock().unwrap();
            inner.gateway_opt = Some(Box::new(gateway));
            inner.mapping_adder = Box::new(mapping_adder);
            inner.housekeeping_commander_opt = None;
        }
    ...
    fn add_mapping_complains_about_not_being_able_to_find_local_ip() {
        ...
        TestLogHandler::new().exists_log_containing(
            "WARN: IgdpTransactor: Cannot determine local IP address: \"GetPublicIpError([REDACTED])\"",
        );
    }
    ...
    fn add_mapping_handles_other_error() {
        ...
        TestLogHandler::new().exists_log_containing(
            "WARN: IgdpTransactor: Failed to add 1234sec mapping for port 7777; details redacted",
        );
    }
    ...
    fn delete_mapping_handles_error() {
        ...
        TestLogHandler::new().exists_log_containing(
            "WARN: IgdpTransactor: Can't delete mapping of port 7777 through router; address and details redacted",
        );
    }
    ...
    fn start_housekeeping_thread_complains_if_change_handler_is_already_running() {
        ...
        TestLogHandler::new().exists_log_containing(
            "INFO: IgdpTransactor: Housekeeping thread for router is already running; address redacted",
        );
    }
    ...
    fn stop_housekeeping_thread_handles_missing_housekeeper_commander() {
        ...
        TestLogHandler::new().exists_log_containing(
            "WARN: IgdpTransactor: Cannot stop housekeeping thread: commander is unavailable",
        );
    }
    ...
    fn stop_housekeeping_thread_handles_missing_join_handle() {
        ...
        assert_eq!(result.err().unwrap(), AutomapError::HousekeeperCrashed);
        TestLogHandler::new().exists_log_containing(
            "WARN: IgdpTransactor: Cannot stop housekeeping thread: join handle is unavailable",
        );
    }
    ...
    #[test]
    fn thread_guts_rejects_remap_interval_in_absence_of_mapping_config_without_panicking() {
        init_test_logging();
        let (tx, rx) = unbounded();
        let change_handler: ChangeHandler = Box::new(move |_| {});
        ...
        TestLogHandler::new().exists_log_containing(
            "ERROR: IgdpTransactor: Cannot set remap interval before the first mapping request",
        );
    }
    ...
    fn poisoned_state_returns_and_reports_housekeeper_crashed_without_panicking() {
        ...
        assert!(thread::spawn(move || {
            ...
            panic!("poison IGDP state");
        })
        .join()
        .is_err());

        assert_eq!(
            subject.find_routers(),
    ...
    fn thread_guts_iteration_handles_remap_error() {
        ...
        TestLogHandler::new().exists_log_containing(
            "ERROR: test: Remapping failure: PermanentMappingError([REDACTED])",
        );
    }
    ...
    fn thread_guts_iteration_reports_router_error_to_change_handler() {
        ...
        tlh.exists_log_containing(&format!(
            "ERROR: thread_guts_iteration_reports_router_error_to_change_handler: Remapping failure: TemporaryMappingError([REDACTED])",
        ));
        tlh.exists_log_containing(&format!(
            "ERROR: thread_guts_iteration_reports_router_error_to_change_handler: Remapping failure: TemporaryMappingError([REDACTED])",
        ));
