...
fn deploy_background_listener(
    ...
    let listener =
        match TcpListener::bind(SocketAddr::new(IpAddr::V4(Ipv4Addr::new(0, 0, 0, 0)), port)) {
            ...
            Err(error) => {
                let join_handle = thread::spawn(move || Err(error));
                return (
                    join_handle,
                    status.abort(AutomapError::ProbeReceiveError(
                        "Could not bind the local probe listener".to_string(),
                    )),
                );
            }
        };
    if let Err(error) = listener.set_nonblocking(true) {
        ...
        return (
            join_handle,
            status.abort(AutomapError::ProbeReceiveError(
                "Could not configure the local probe listener".to_string(),
            )),
        );
    }
...
pub fn request_probe(
        ...
        .is_err()
    {
        return status.fail(AutomapError::ProbeRequestError(
            AutomapErrorCause::ProbeServerIssue,
            "Could not configure the probe-server response timeout".to_string(),
        ));
    }
    let status =
    ...
    match connection.read(&mut buffer) {
        ...
        Ok(length) => {
            let response = match parse_probe_server_response(&buffer[0..length]) {
                Ok(response) => response,
                Err(error) => return status.fail(error),
            };
            if !response.contains("200 OK") {
                return status.fail(AutomapError::ProbeRequestError(
                    AutomapErrorCause::ProbeFailed,
                    "Probe server did not report success".to_string(),
                ));
            }
        }
...
fn parse_probe_server_response(response: &[u8]) -> Result<&str, AutomapError> {
    std::str::from_utf8(response).map_err(|_| {
        AutomapError::ProbeRequestError(
            AutomapErrorCause::ProbeServerIssue,
            "Probe server returned a non-UTF-8 response".to_string(),
        )
    })
}
...
mod tests {
    ...
    #[test]
    fn deploy_background_listener_reports_a_bind_failure_without_panicking() {
        ...
        assert_eq!(
            status.step_error,
            Some(AutomapError::ProbeReceiveError(
                "Could not bind the local probe listener".to_string()
            ))
        );
        assert_eq!(
    ...
    #[test]
    fn probe_server_response_parser_rejects_non_utf8_without_panicking_or_retaining_bytes() {
        let result = parse_probe_server_response(b"SENSITIVE_SERVER_RESPONSE\xFF");

        assert_eq!(
            result,
            Err(AutomapError::ProbeRequestError(
                AutomapErrorCause::ProbeServerIssue,
                "Probe server returned a non-UTF-8 response".to_string()
            ))
        );
        assert!(!format!("{:?}", result).contains("SENSITIVE_SERVER_RESPONSE"));
    }
