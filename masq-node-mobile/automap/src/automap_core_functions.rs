...
impl AutomapParameters {
    pub fn new(args: Args, probe_server_address_str: &str) -> Result<Self, String> {
        Self::from_strings(args, probe_server_address_str)
    }

    fn from_strings<I>(args: I, probe_server_address_str: &str) -> Result<Self, String>
    where
        I: IntoIterator<Item = String>,
    {
        let probe_server_address = SocketAddr::from_str(probe_server_address_str)
            .map_err(|_| "Invalid probe-server socket address".to_string())?;
        ...
        for arg in args.into_iter().skip(1) {
            match arg.as_str() {
                ...
                arg => {
                    hole_port = arg
                        .parse::<u16>()
                        .map_err(|_| "Invalid Automap port number".to_string())?;
                }
            }
        }
...
pub fn change_handler(change: AutomapChange) {
    match change {
        AutomapChange::NewIp(_) => info!("Notified of public-IP change; address redacted"),
        AutomapChange::Error(e) => error!("Notified of Automap error: {:?}", e),
    }
}
...
fn find_router(status: TestStatus, transactor: &dyn Transactor) -> (IpAddr, TestStatus) {
    ...
    match transactor.find_routers() {
        Ok(list) => match list.into_iter().next() {
            Some(found_router_ip) => (found_router_ip, status.succeed()),
            None => (
                null_ip(),
                status.abort(AutomapError::FindRouterError(
                    "No default gateway was reported".to_string(),
                )),
            ),
        },
...
fn start_housekeeping_thread(
    ...
    let status = status
        .begin_attempt("Starting housekeeping thread for router; address redacted".to_string());
...
fn seek_public_ip(
    ...
    let status =
        status.begin_attempt("Seeking public IP address from router; address redacted".to_string());
...
fn analyze_status(status: TestStatus) -> Result<(), AutomapErrorCause> {
    if status.cumulative_success {
        ...
        match status.step_error {
            Some(error) => Err(error.cause()),
            None => Err(AutomapErrorCause::Unknown(
                "Cumulative failure without a step error".to_string(),
            )),
        }
    }
}
...
mod tests {
    use super::*;
    use crate::mocks::TransactorMock;

    #[test]
    fn automap_parameters_reject_an_invalid_probe_server_without_panicking() {
        let result = AutomapParameters::from_strings(
            vec!["automap".to_string()],
            "SENSITIVE_INVALID_SOCKET",
        );

        assert_eq!(
            result.err(),
            Some("Invalid probe-server socket address".to_string())
        );
    }

    #[test]
    fn automap_parameters_reject_an_invalid_port_without_panicking() {
        let result = AutomapParameters::from_strings(
            vec!["automap".to_string(), "SENSITIVE_INVALID_PORT".to_string()],
            "127.0.0.1:8080",
        );

        assert_eq!(
            result.err(),
            Some("Invalid Automap port number".to_string())
        );
    }

    #[test]
    fn find_router_rejects_an_empty_router_list_without_panicking() {
        let transactor = TransactorMock::new(AutomapProtocol::Pcp).find_routers_result(Ok(vec![]));

        let (router_ip, status) = find_router(TestStatus::new(), &transactor);

        assert_eq!(router_ip, null_ip());
        assert_eq!(status.fatal, true);
        assert_eq!(
            status.step_error,
            Some(AutomapError::FindRouterError(
                "No default gateway was reported".to_string()
            ))
        );
    }
    ...
    #[test]
    fn analyze_status_reports_an_inconsistent_failure_without_panicking() {
        let mut status = TestStatus::new();
        status.cumulative_success = false;

        let result = analyze_status(status);

        assert_eq!(
            result,
            Err(AutomapErrorCause::Unknown(
                "Cumulative failure without a step error".to_string()
            ))
        );
    }
