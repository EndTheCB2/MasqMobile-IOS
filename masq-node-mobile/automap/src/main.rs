...
pub fn main() {
    let parameters = match AutomapParameters::new(std::env::args(), SERVER_SOCKET_ADDRESS) {
        Ok(parameters) => parameters,
        Err(error) => {
            eprintln!("Invalid Automap arguments: {}", error);
            std::process::exit(2);
        }
    };
    ...
    if let Err(error) = initiate_logger() {
        eprintln!("Could not initialize Automap logging: {}", error);
        std::process::exit(2);
    }
...
fn automatic(parameters: AutomapParameters) {
    ...
    let status = status.succeed();
    let protocol = match automap_control.get_mapping_protocol() {
        Some(protocol) => protocol,
        None => {
            let _ = status.abort(automap_lib::comm_layer::AutomapError::ProtocolError(
                "Automap selected no mapping protocol".to_string(),
            ));
            return;
        }
    };
    let status = status.begin_attempt(format!(
        ...
        public_ip, parameters.test_parameters.hole_port, protocol
    ));
