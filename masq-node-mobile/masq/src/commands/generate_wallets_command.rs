...
impl Debug for GenerateWalletsCommand {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("GenerateWalletsCommand")
            .field("db_password", &"[REDACTED]")
            .field("seed_spec_opt", &self.seed_spec_opt)
            .field(
                "consuming_path_opt",
                &self.consuming_path_opt.as_ref().map(|_| "[REDACTED]"),
            )
            .field(
                "earning_path_opt",
                &self.earning_path_opt.as_ref().map(|_| "[REDACTED]"),
            )
            .finish()
    }
}
...
mod tests {
    #[test]
    fn debug_redacts_wallet_generation_secrets() {
        let subject = GenerateWalletsCommand {
            ...
            consuming_path_opt: Some("SENSITIVE_CONSUMING_PATH".to_string()),
            earning_path_opt: Some("SENSITIVE_EARNING_PATH".to_string()),
        };
        ...
        for sensitive in [
            ...
            "SENSITIVE_CONSUMING_PATH",
            "SENSITIVE_EARNING_PATH",
        ] {
