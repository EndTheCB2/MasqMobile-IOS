...
impl Debug for SeedSpec {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SeedSpec")
            .field("mnemonic_phrase_words", &self.mnemonic_phrase.len())
            .field("language", &self.language)
            .field(
                "passphrase_opt",
                &self.passphrase_opt.as_ref().map(|_| "[REDACTED]"),
            )
            .finish()
    }
}
...
impl Debug for RecoverWalletsCommand {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("RecoverWalletsCommand")
            .field("db_password", &"[REDACTED]")
            .field("seed_spec_opt", &self.seed_spec_opt)
            .field("consuming", &"[REDACTED]")
            .field("earning", &"[REDACTED]")
            .finish()
    }
}
...
mod tests {
    #[test]
    fn debug_redacts_wallet_recovery_secrets() {
        let subject = RecoverWalletsCommand {
            db_password: "SENSITIVE_DB_PASSWORD".to_string(),
            seed_spec_opt: Some(SeedSpec {
                mnemonic_phrase: vec!["SENSITIVE_MNEMONIC".to_string()],
                language: "English".to_string(),
                passphrase_opt: Some("SENSITIVE_PASSPHRASE".to_string()),
            }),
            consuming: Either::Left("SENSITIVE_CONSUMING_KEY".to_string()),
            earning: Either::Left("SENSITIVE_EARNING_ADDRESS".to_string()),
        };
        ...
        for sensitive in [
            "SENSITIVE_DB_PASSWORD",
            "SENSITIVE_MNEMONIC",
            "SENSITIVE_PASSPHRASE",
            "SENSITIVE_CONSUMING_KEY",
            "SENSITIVE_EARNING_ADDRESS",
        ] {
