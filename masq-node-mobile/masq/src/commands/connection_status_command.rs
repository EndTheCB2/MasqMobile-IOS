...
use masq_lib::messages::{
    UiConnectionStage, UiConnectionStatusReason, UiConnectionStatusRequest,
    UiConnectionStatusResponse,
};
...
const ENTRY_NODES_UNREACHABLE_MSG: &str =
    "\nCause: ENTRY_NODES_UNREACHABLE: All configured entry-node attempts failed.";
const ROUTE_NOT_READY_MSG: &str =
    "\nCause: ROUTE_NOT_READY: A neighbor is connected, but end-to-end exit traffic is not yet proven.";
const ROUTE_PROOF_STALE_MSG: &str =
    "\nCause: ROUTE_PROOF_STALE: The last proven end-to-end exit response is older than five minutes.";
            ...
            Ok(response) => {
                ...
                let reason_msg = match response.reason {
                    Some(UiConnectionStatusReason::EntryNodesUnreachable) => {
                        ENTRY_NODES_UNREACHABLE_MSG
                    }
                    Some(UiConnectionStatusReason::RouteNotReady) => ROUTE_NOT_READY_MSG,
                    Some(UiConnectionStatusReason::RouteProofStale) => ROUTE_PROOF_STALE_MSG,
                    None => "",
                };
                short_writeln!(context.stdout(), "\n{}{}\n", stdout_msg, reason_msg);
                Ok(())
            }
...
mod tests {
    ...
    #[test]
    fn connection_status_command_explains_exhausted_entry_nodes() {
        assert_on_connection_status_response(
            UiConnectionStage::NotConnected,
            Some(UiConnectionStatusReason::EntryNodesUnreachable),
            (
    ...
    #[test]
    fn connection_status_command_explains_unproven_route() {
        assert_on_connection_status_response(
            UiConnectionStage::ConnectedToNeighbor,
    ...
    #[test]
    fn connection_status_command_explains_stale_route_proof() {
        assert_on_connection_status_response(
            UiConnectionStage::ConnectedToNeighbor,
    ...
    fn assert_on_connection_status_response(
        stage: UiConnectionStage,
        reason: Option<UiConnectionStatusReason>,
        response: (&str, &str),
    ) {
        let transact_params_arc = Arc::new(Mutex::new(vec![]));
        let expected_response = UiConnectionStatusResponse { stage, reason };
        let mut context = CommandContextMock::new()
            .transact_params(&transact_params_arc)
