...
impl Debug for ChangePasswordCommand {
...
mod tests {
    ...
    fn debug_redacts_both_passwords() {
        let subject = ChangePasswordCommand {
            old_password: Some("SENSITIVE_OLD_PASSWORD".to_string()),
            new_password: "SENSITIVE_NEW_PASSWORD".to_string(),
        };

        let debug = format!("{:?}", subject);

        assert!(debug.contains("[REDACTED]"));
        assert!(!debug.contains("SENSITIVE_OLD_PASSWORD"));
        assert!(!debug.contains("SENSITIVE_NEW_PASSWORD"));
    }
