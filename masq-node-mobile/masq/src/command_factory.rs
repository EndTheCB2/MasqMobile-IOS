...
use crate::commands::provider_settlement_command::ProviderSettlementCommand;
use crate::commands::receipt_session_command::ReceiptSessionCommand;
...
impl CommandFactory for CommandFactoryReal {
    fn make(&self, pieces: &[String]) -> Result<Box<dyn Command>, CommandFactoryError> {
        let boxed_command: Box<dyn Command> = match pieces[0].as_str() {
            ...
            "provider-settlement" => match ProviderSettlementCommand::new(pieces) {
                Ok(command) => Box::new(command),
                Err(msg) => return Err(CommandSyntax(msg)),
            },
            ...
            "receipt-session" => match ReceiptSessionCommand::new(pieces) {
                Ok(command) => Box::new(command),
                Err(msg) => return Err(CommandSyntax(msg)),
            },
