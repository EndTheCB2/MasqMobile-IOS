...
lazy_static! {
    ...
    static ref MOBILE_LOGGER_HANDLE: Mutex<Option<ReconfigurationHandle>> = Mutex::new(None);
}
...
impl LoggerInitializerWrapper for LoggerInitializerWrapperReal {
    fn init(
        ...
        if crate::mobile_runtime::is_embedded() {
            let mut mobile_handle = MOBILE_LOGGER_HANDLE
                .lock()
                .unwrap_or_else(|poisoned| poisoned.into_inner());
            if let Some(handle) = mobile_handle.as_mut() {
                handle.set_new_spec(log_spec);
                log!(
                    Level::Info,
                    "Reusing the process logger for a restarted embedded mobile Node"
                );
                return;
            }
        }
        ...
        let logger_handle = logger.start().expect("Logging subsystem failed to start");
        if crate::mobile_runtime::is_embedded() {
            *MOBILE_LOGGER_HANDLE
                .lock()
                .unwrap_or_else(|poisoned| poisoned.into_inner()) = Some(logger_handle);
        }
