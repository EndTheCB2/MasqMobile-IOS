...
impl Debug for NonClandestineAttributes {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        f.write_str("NonClandestineAttributes { stream_data: [REDACTED] }")
    }
}
...
impl Debug for RemovedStreamType {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        f.write_str(match self {
            Self::Clandestine => "Clandestine",
            Self::NonClandestine(_) => "NonClandestine",
        })
    }
}
...
impl Debug for RemoveStreamMsg {
    fn fmt(&self, f: &mut Formatter) -> Result<(), std::fmt::Error> {
        write!(
            f,
            "RemoveStreamMsg {{ stream_type: {:?}, socket_data: [REDACTED], sub: <unprintable> }}",
            self.stream_type
        )
    }
}
...
mod tests {
    ...
    #[test]
    fn stream_removal_debug_redacts_socket_and_sequence_metadata() {
        let _system = System::new("stream_removal_debug_redacts_socket_and_sequence_metadata");
        let attributes = NonClandestineAttributes {
            reception_port: 8443,
            sequence_number: 991,
        };
        let stream_type = RemovedStreamType::NonClandestine(attributes.clone());
        let (dispatcher, _, _) = make_recorder();
        let message = RemoveStreamMsg {
            local_addr: "127.0.0.1:4321".parse().unwrap(),
            peer_addr: "203.0.113.61:9876".parse().unwrap(),
            stream_type: stream_type.clone(),
            dispatcher_sub: dispatcher.start().recipient(),
        };

        assert_eq!(
            format!("{:?}", attributes),
            "NonClandestineAttributes { stream_data: [REDACTED] }"
        );
        assert_eq!(format!("{:?}", stream_type), "NonClandestine");
        assert_eq!(
            format!("{:?}", message),
            "RemoveStreamMsg { stream_type: NonClandestine, socket_data: [REDACTED], sub: <unprintable> }"
        );
    }
