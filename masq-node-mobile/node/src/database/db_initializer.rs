...
use masq_lib::constants::{
    ...
    LOWEST_USABLE_INSECURE_PORT, RECEIPT_SESSION_RECOVERY_KEY,
};
...
impl DbInitializerReal {
    ...
    fn create_database_tables(&self, conn: &Connection, external_params: ExternalData) {
        ...
        Self::create_receipt_sequence_checkpoint_table(conn);
        Self::create_receipt_session_authorization_table(conn);
        Self::create_receipt_acknowledgement_outbox_table(conn);
        Self::create_routing_receipt_offer_state_table(conn);
        Self::create_receipt_session_recovery_tables(conn);
        Self::create_receipt_settlement_claim_outbox_table(conn);
        Self::create_provider_settlement_authorization_table(conn);
        Self::create_receipt_settlement_reconciliation_tables(conn);
    }
    ...
    pub fn create_receipt_sequence_checkpoint_table(conn: &Connection) {
        conn.execute(
            "create table receipt_sequence_checkpoint (
        ...
        .expect("Can't create receipt sequence checkpoint table");
    }

    pub fn create_receipt_session_authorization_table(conn: &Connection) {
        conn.execute(
            "create table receipt_session_authorization (
        ...
        .expect("Can't create receipt session authorization table");
    }

    pub fn create_receipt_acknowledgement_outbox_table(conn: &Connection) {
        conn.execute(
        ...
        .expect("Can't create receipt acknowledgement outbox table");
    }

    pub fn create_routing_receipt_offer_state_table(conn: &Connection) {
        conn.execute(
            "create table routing_receipt_offer_state (
        ...
        .expect("Can't create routing receipt offer state table");
    }

    pub fn create_receipt_session_recovery_tables(conn: &Connection) {
        conn.execute(
        ...
        .expect("Can't create receipt session recovery table");
        conn.execute(
        ...
        .expect("Can't create receipt session route recovery table");
    }

    pub fn create_receipt_settlement_claim_outbox_table(conn: &Connection) {
        conn.execute(
        ...
        .expect("Can't create receipt settlement claim outbox table");
    }

    pub fn create_provider_settlement_authorization_table(conn: &Connection) {
        conn.execute(
        ...
        .expect("Can't create provider settlement authorization table");
    }

    pub fn create_receipt_settlement_reconciliation_tables(conn: &Connection) {
        conn.execute(
        ...
        .expect("Can't create receipt settlement claim archive table");
        conn.execute(
        ...
        .expect("Can't create receipt settlement reconciliation checkpoint table");
    }
    ...
    pub fn choose_clandestine_port() -> u16 {
        Self::find_available_clandestine_port(|candidate_port| {
            StdTcpListener::bind(SocketAddr::V4(SocketAddrV4::new(
                Ipv4Addr::from(0),
                candidate_port,
            )))
            .map(|_| ())
        ...
        .unwrap_or_else(|error_kind| {
            panic!(
                "Could not find an available clandestine port between {} and {}: {:?}",
                LOWEST_USABLE_INSECURE_PORT,
                HIGHEST_RANDOM_CLANDESTINE_PORT - 1,
                error_kind
            )
        })
    }

    fn find_available_clandestine_port<F>(mut port_is_available: F) -> Result<u16, ErrorKind>
    where
        F: FnMut(u16) -> Result<(), ErrorKind>,
    {
        let candidate_count = HIGHEST_RANDOM_CLANDESTINE_PORT - LOWEST_USABLE_INSECURE_PORT;
        let starting_offset = SmallRng::from_entropy().gen_range(0..candidate_count);
        let mut last_error_kind = ErrorKind::AddrInUse;

        for offset in 0..candidate_count {
            let candidate_port =
                LOWEST_USABLE_INSECURE_PORT + ((starting_offset + offset) % candidate_count);
            match port_is_available(candidate_port) {
                Ok(()) => return Ok(candidate_port),
                Err(error_kind) => last_error_kind = error_kind,
            }
        }
...
impl Debug for ExternalData {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ExternalData")
            ...
            .field(
                "db_password_opt",
                &self.db_password_opt.as_ref().map(|_| "[REDACTED]"),
            )
            .finish()
    }
}
...
mod tests {
    ...
    fn existing_database_with_correct_version_is_accepted_without_changes() {
        ...
        verify(&mut config_vec, RECEIPT_SESSION_RECOVERY_KEY, None, true);
        verify(
            &mut config_vec,
            "scan_intervals",
    ...
    fn new_database_is_initialized_correctly() {
        ...
        assert_eq!(
            ...
            HashSet::from([
                ...
                "receipt_sequence_checkpoint".to_string(),
                "receipt_session_authorization".to_string(),
                "receipt_acknowledgement_outbox".to_string(),
                "routing_receipt_offer_state".to_string(),
                "receipt_session_recovery".to_string(),
                "receipt_session_route_recovery".to_string(),
                "receipt_settlement_claim_outbox".to_string(),
                "provider_settlement_authorization".to_string(),
                "receipt_settlement_claim_archive".to_string(),
                "receipt_settlement_reconciliation_checkpoint".to_string()
            ]),
        );
        let config_map = extract_configurations(&conn);
        let mut config_vec: Vec<(String, (Option<String>, bool))> =
        ...
        DbInitializationConfig {
            mode: InitializationMode::CreationAndMigration {
                external_data: ExternalData {
                    ...
                    neighborhood_mode: NeighborhoodModeLight::Standard,
                    db_password_opt: Some("SENSITIVE_DB_PASSWORD".to_string()),
                },
            },
    ...
    fn debug_is_implemented_for_db_initialization_config() {
            ...
            config_one_debug.contains(
                    "DbInitializationConfig{init_config: CreationAndMigration { external_data: \
                     ExternalData { chain: BaseMainnet, neighborhood_mode: Standard, db_password_opt: \
                      Some(\"[REDACTED]\") } }, special_conn_setup: Addresses["
                ),
        ...
        assert!(!config_one_debug.contains("SENSITIVE_DB_PASSWORD"));
    }
    ...
    #[test]
    fn find_available_clandestine_port_stops_after_each_candidate_is_rejected() {
        let mut attempted_ports = vec![];

        let result = DbInitializerReal::find_available_clandestine_port(|candidate_port| {
            attempted_ports.push(candidate_port);
        ...
        assert_eq!(
            attempted_ports.len(),
            usize::from(HIGHEST_RANDOM_CLANDESTINE_PORT - LOWEST_USABLE_INSECURE_PORT)
        );
        attempted_ports.sort_unstable();
        attempted_ports.dedup();
        assert_eq!(
            attempted_ports.len(),
            usize::from(HIGHEST_RANDOM_CLANDESTINE_PORT - LOWEST_USABLE_INSECURE_PORT)
        );
    ...
    #[test]
    fn find_available_clandestine_port_returns_the_first_accepted_candidate() {
        let mut attempts = 0;

        let result = DbInitializerReal::find_available_clandestine_port(|_| {
            attempts += 1;
            if attempts == 3 {
                Ok(())
            } else {
