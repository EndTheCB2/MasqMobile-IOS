...
impl Debug for CryptDENull {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.write_str(
            "CryptDENull { private_key: [REDACTED], public_key: [REDACTED], digest: [REDACTED], next_symmetric_key_seed: [REDACTED] }",
        )
    }
}
...
mod tests {
    ...
    #[test]
    fn debug_never_exposes_cryptographic_or_seed_material() {
        let subject = CryptDENull::new(Chain::Dev);
    ...
    fn make_from_str_can_fail_on_base64_syntax() {
        let subject = CryptDENull::new(Chain::Dev);
        let string = "SENSITIVE_INVALID_PRIVATE_KEY!"; // invalid
    ...
    fn fails_to_destringify_public_key_properly() {
        let subject = CRYPTDE_PAIR.main.as_ref();
        let half_key = "SENSITIVE_INVALID_PUBLIC_KEY!((]--$";
        ...
        assert!(!result.unwrap_err().contains(half_key));
    }
