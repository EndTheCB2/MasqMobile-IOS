...
impl RatePack {
    ...
    pub fn routing_charge(&self, payload_size: u64) -> u64 {
        self.routing_service_rate
            .saturating_add(self.routing_byte_rate.saturating_mul(payload_size))
    }

    pub fn exit_charge(&self, payload_size: u64) -> u64 {
        self.exit_service_rate
            .saturating_add(self.exit_byte_rate.saturating_mul(payload_size))
    }
...
pub struct RouteQueryMessage {
    ...
    pub host: Host,
    pub minimum_protocol_capabilities: u64,
    /// Copied into each onion-encrypted billable hop only when bilateral receipts are active.
    pub routing_receipt_request_opt: Option<crate::sub_lib::service_receipt::ReceiptSessionRequest>,
}
...
impl RouteQueryMessage {
    ...
    pub fn require_service_receipt_settlement_v1(mut self) -> Self {
        self.minimum_protocol_capabilities |=
            crate::sub_lib::service_receipt::SERVICE_RECEIPT_SETTLEMENT_V1_CAPABILITY;
        self
    }

    pub fn require_bilateral_service_receipts_v1(
        mut self,
        request: crate::sub_lib::service_receipt::ReceiptSessionRequest,
    ) -> Self {
        self.minimum_protocol_capabilities |=
            crate::sub_lib::service_receipt::SERVICE_RECEIPT_SETTLEMENT_V1_CAPABILITY
                | crate::sub_lib::service_receipt::ROUTING_RECEIPT_V1_CAPABILITY;
        self.routing_receipt_request_opt = Some(request);
        self
    }

    pub fn service_receipt_delivery_route_request(
        provider_public_key: PublicKey,
        payload_size: usize,
    ) -> Self {
        RouteQueryMessage {
            target_key_opt: Some(provider_public_key),
            target_component: Component::Hopper,
            return_component_opt: Some(Component::ProxyServer),
            payload_size,
            host: Host::new("", 0),
...
/// Emitted only after non-empty response data has returned over a correlated MASQ exit route.
...
/// It intentionally carries no destination or stream identity.
#[derive(Clone, Debug, Message, PartialEq, Eq)]
pub struct RouteUseFailedMessage;
...
pub enum NRMetadataChange {
    ...
    RemoveUnreachableHost { hostname: String },
    RecordRouteSuccess { hostname: String, latency_ms: u32 },
}
...
mod tests {
    ...
    fn from_str_complains_about_bad_base_64() {
        ...
        assert_eq!(
            result,
            Err(String::from(
                "Invalid Base64 value for public key; contents redacted"
            ))
        );
        assert!(!result.unwrap_err().contains("bad_key"));
    }
    ...
    fn from_str_complains_about_slash_in_the_key() {
        ...
        assert!(!result
            .unwrap_err()
            .contains("abJ5XvhVbmVyGejkYUkmftF09pmGZGKg/PzRNnWQxFw"));
    }
    ...
    fn from_str_complains_about_plus_in_the_key() {
        ...
        assert!(!result
            .unwrap_err()
            .contains("abJ5XvhVbmVy+GejkYUmftF09pmGZGKgkPzRNnWQxFw"));
    }
    ...
    #[test]
    fn rate_pack_charges_saturate_instead_of_wrapping() {
        let subject = RatePack {
            routing_byte_rate: u64::MAX,
            routing_service_rate: u64::MAX,
            exit_byte_rate: u64::MAX,
            exit_service_rate: u64::MAX,
        };

        assert_eq!(subject.routing_charge(u64::MAX), u64::MAX);
        assert_eq!(subject.exit_charge(u64::MAX), u64::MAX);
    }
    ...
    fn data_indefinite_route_request() {
        ...
        let receipt_capable =
            RouteQueryMessage::data_indefinite_route_request(Host::new("booga.com", 1234), 7500)
                .require_service_receipt_settlement_v1();
        assert_eq!(
