...
impl fmt::Debug for Endpoint {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Endpoint::Key(_) => f.write_str("Endpoint::Key([REDACTED])"),
            Endpoint::Socket(_) => f.write_str("Endpoint::Socket([REDACTED])"),
        }
    }
}
...
#[derive(Clone, Hash, Ord, PartialEq, Eq, PartialOrd)]
...
impl Debug for DispatcherError {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        f.write_str(match self {
            Self::IpAddressUnknown => "IpAddressUnknown",
            Self::StreamConnectError(_) => "StreamConnectError([REDACTED])",
            Self::StreamWriteError(_) => "StreamWriteError([REDACTED])",
            Self::StreamShutdownError(_) => "StreamShutdownError([REDACTED])",
            Self::NeighborhoodPanicked => "NeighborhoodPanicked",
        })
    }
}
...
impl Debug for InboundClientData {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "InboundClientData {{ reception_port: {:?}, last_data: {}, clandestine: {}, sequence_number: {:?}, data_len: {}, client_data: [REDACTED] }}",
...
impl Debug for StreamShutdownMsg {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "StreamShutdownMsg {{ stream_type: {:?}, report_to_counterpart: {}, peer_addr: [REDACTED] }}",
            self.stream_type, self.report_to_counterpart
        )
    }
}
...
mod tests {
    ...
    #[test]
    fn debug_redacts_endpoint_with_utf8_key() {
        let subject = Endpoint::Key(PublicKey::new(b"blah"));

        let result = format!("{:?}", subject);

        assert_eq!(result, String::from("Endpoint::Key([REDACTED])"))
    }

    #[test]
    fn debug_redacts_endpoint_with_non_utf8_key() {
    ...
    #[test]
    fn debug_redacts_endpoint_with_socket() {
        let subject = Endpoint::Socket(SocketAddr::from_str("1.2.3.4:5678").unwrap());

        let result = format!("{:?}", subject);

        assert_eq!(result, String::from("Endpoint::Socket([REDACTED])"))
    }

    #[test]
    fn dispatcher_debug_redacts_client_payload_socket_and_error_details() {
        let inbound = InboundClientData {
            timestamp: SystemTime::now(),
            client_addr: SocketAddr::from_str("203.0.113.81:4567").unwrap(),
            reception_port_opt: Some(8080),
            last_data: false,
            is_clandestine: false,
            sequence_number_opt: Some(7),
            data: b"private inbound marker".to_vec(),
        };
        let shutdown = StreamShutdownMsg {
            peer_addr: SocketAddr::from_str("203.0.113.82:7654").unwrap(),
        ...
        assert_eq!(
            format!("{:?}", inbound),
            "InboundClientData { reception_port: Some(8080), last_data: false, clandestine: false, sequence_number: Some(7), data_len: 22, client_data: [REDACTED] }"
        );
        assert_eq!(
            format!(
                "{:?}",
                DispatcherError::StreamWriteError("private write error marker".to_string())
            ),
            "StreamWriteError([REDACTED])"
        );
        assert_eq!(
            format!("{:?}", shutdown),
            "StreamShutdownMsg { stream_type: NonClandestine, report_to_counterpart: true, peer_addr: [REDACTED] }"
        );
    }
