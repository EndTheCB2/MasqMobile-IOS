...
impl fmt::Debug for WalletKind {
    fn fmt(&self, formatter: &mut Formatter<'_>) -> fmt::Result {
        formatter.write_str(match self {
            WalletKind::Address(_) => "Address([REDACTED])",
            WalletKind::SecretKey(_) => "SecretKey([REDACTED])",
            WalletKind::PublicKey(_) => "PublicKey([REDACTED])",
            WalletKind::Uninitialized => "Uninitialized",
        })
    }
}
...
impl Wallet {
    ...
    pub fn address_opt(&self) -> Option<Address> {
        match &self.kind {
            WalletKind::Address(address) => Some(H160(address.0)),
            WalletKind::PublicKey(public) => Some(H160(*public.address())),
            WalletKind::SecretKey(key_provider) => Some(key_provider.address()),
            WalletKind::Uninitialized => None,
        }
    }

    pub fn address(&self) -> Address {
        self.address_opt()
            .expect("No address for an uninitialized wallet!")
    }
...
mod tests {
    ...
    #[test]
    fn wallet_debug_redacts_all_initialized_identity_material() {
        let raw_secret = vec![0x42; 32];
        let key_provider = Bip32EncryptionKeyProvider::from_raw_secret(&raw_secret).unwrap();
        let public_wallet = Wallet::from(key_provider.public_key());
        let secret_wallet = Wallet::from(key_provider);
        let address_wallet = Wallet::new("0xabcdef0123456789abcdef0123456789abcdef01");

        let secret_debug = format!("{:?}", secret_wallet);

        assert_eq!(secret_debug, "Wallet { kind: SecretKey([REDACTED]) }");
        assert!(!secret_debug.contains(&format!("{:?}", raw_secret)));
        assert_eq!(
            format!("{:?}", public_wallet),
            "Wallet { kind: PublicKey([REDACTED]) }"
        );
        assert_eq!(
            format!("{:?}", address_wallet),
            "Wallet { kind: Address([REDACTED]) }"
        );
    }
