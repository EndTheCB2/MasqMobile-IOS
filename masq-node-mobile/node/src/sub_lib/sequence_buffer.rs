...
pub const MAX_SEQUENCE_REORDER_WINDOW: u64 = 4096;
...
impl fmt::Debug for SequencedPacket {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "SequencedPacket {{ sequence_number: {}, last_data: {}, data_len: {}, packet_data: [REDACTED] }}",
...
impl SequenceBuffer {
    ...
    pub fn push(&mut self, packet: SequencedPacket) {
        if self.sequence_space_exhausted {
            warning!(
                self.logger,
                "Dropping packet with sequence number {} because the sequence space is exhausted",
        ...
        let sequence_gap = packet.sequence_number - self.next_expected_sequence_number;
        if sequence_gap >= MAX_SEQUENCE_REORDER_WINDOW {
            warning!(
                self.logger,
                "Dropping packet with sequence number {} because it is {} positions ahead of the next expected packet; maximum reorder gap is {}",
                packet.sequence_number,
                sequence_gap,
                MAX_SEQUENCE_REORDER_WINDOW - 1
            );
            return;
        }
    ...
    pub fn poll(&mut self) -> Option<SequencedPacket> {
        if self.buffer.is_empty() {
            ...
            if sequence_number_to_pop == self.next_expected_sequence_number {
                ...
                match self.next_expected_sequence_number.checked_add(1) {
                    Some(next) => self.next_expected_sequence_number = next,
                    None => self.sequence_space_exhausted = true,
                }
    ...
    pub fn repush(&mut self, packet: SequencedPacket) {
        let follows_last_poll = if self.sequence_space_exhausted {
            packet.sequence_number == u64::MAX
        } else {
            self.next_expected_sequence_number.checked_sub(1) == Some(packet.sequence_number)
        };
        if !follows_last_poll {
            panic!("improper use of repush")
        } else {
            self.sequence_space_exhausted = false;
...
mod tests {
    ...
    #[test]
    fn sequenced_packet_debug_redacts_payload() {
        let packet = SequencedPacket::new(b"private sequence marker".to_vec(), 5, false);

        assert_eq!(
            format!("{:?}", packet),
            "SequencedPacket { sequence_number: 5, last_data: false, data_len: 23, packet_data: [REDACTED] }"
        );
    }
    ...
    #[test]
    fn sequence_buffer_bounds_future_packets_to_the_reorder_window() {
        init_test_logging();
        let mut subject = SequenceBuffer::new();
        for sequence_number in 1..MAX_SEQUENCE_REORDER_WINDOW {
            subject.push(SequencedPacket::new(vec![], sequence_number, false));
        }

        subject.push(SequencedPacket::new(
            vec![],
            MAX_SEQUENCE_REORDER_WINDOW,
            false,
        ));
        subject.push(SequencedPacket::new(vec![], u64::MAX, false));

        assert_eq!(subject.len(), (MAX_SEQUENCE_REORDER_WINDOW - 1) as usize);
        TestLogHandler::new().exists_log_containing(&format!(
            "WARN: SequenceBuffer: Dropping packet with sequence number {} because it is {} positions ahead of the next expected packet; maximum reorder gap is {}",
            MAX_SEQUENCE_REORDER_WINDOW,
            MAX_SEQUENCE_REORDER_WINDOW,
            MAX_SEQUENCE_REORDER_WINDOW - 1
        ));
    ...
    #[test]
    fn sequence_buffer_exhausts_u64_space_without_wraparound_and_allows_repush() {
        let mut subject = SequenceBuffer::new();
        subject.next_expected_sequence_number = u64::MAX;
        let terminal_packet = SequencedPacket::new(vec![1, 2, 3], u64::MAX, true);
        subject.push(terminal_packet.clone());

        assert_eq!(subject.poll(), Some(terminal_packet.clone()));
        assert!(subject.sequence_space_exhausted);
        subject.push(terminal_packet.clone());
        assert!(subject.is_empty());

        subject.repush(terminal_packet.clone());

        assert!(!subject.sequence_space_exhausted);
        assert_eq!(subject.poll(), Some(terminal_packet));
        assert!(subject.sequence_space_exhausted);
    }
