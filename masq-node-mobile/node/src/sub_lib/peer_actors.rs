...
impl Debug for NewPublicIp {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        f.write_str("NewPublicIp { new_ip: [REDACTED] }")
    }
}

#[cfg(test)]
mod tests {
    use super::NewPublicIp;
    ...
    #[test]
    fn new_public_ip_debug_redacts_the_address() {
        let subject = NewPublicIp {
            new_ip: "203.0.113.241".parse().unwrap(),
        };

        let result = format!("{:?}", subject);

        assert_eq!(result, "NewPublicIp { new_ip: [REDACTED] }");
        assert!(!result.contains("203.0.113.241"));
    }
}
