...
impl CryptDE for CryptDEReal {
    fn encode(&self, key: &PublicKey, data: &PlainData) -> Result<CryptData, CryptdecError> {
        if key.len() != cxsp::PUBLICKEYBYTES + signing::PUBLICKEYBYTES {
            return Err(CryptdecError::InvalidKey(format!(
                "public key has invalid length {}; expected {}",
                key.len(),
                cxsp::PUBLICKEYBYTES + signing::PUBLICKEYBYTES
            )));
        }
    ...
    fn encode_sym(&self, key: &SymmetricKey, data: &PlainData) -> Result<CryptData, CryptdecError> {
        ...
        let cipher_data = {
            match secretbox::Key::from_slice(key.as_slice()) {
                None => {
                    return Err(CryptdecError::InvalidKey(format!(
                        "symmetric key has invalid length {}; expected {}",
                        key.len(),
                        secretbox::KEYBYTES
                    )))
                }
    ...
    fn descriptor_fragment_to_first_contact_public_key(
        ...
        let mut encryption_public_key =
            match base64::decode_config(descriptor_fragment, base64::URL_SAFE_NO_PAD) {
                Ok(v) => v,
                Err(_) => {
                    return Err("Invalid Base64 value for public key; contents redacted".to_string())
                }
            };
    ...
    fn make_from_str(&self, value: &str, chain: Chain) -> Result<Box<dyn CryptDE>, String> {
        ...
        let convert = |s: &str| -> Result<Vec<u8>, String> {
            match base64::decode_config(s, base64::URL_SAFE_NO_PAD) {
                Ok(v) => Ok(v),
                Err(_) => {
                    Err("Serialized CryptDE must have valid Base64; contents redacted".to_string())
                }
            }
        };
...
mod tests {
    ...
    fn make_from_str_can_fail_on_base64_syntax_for_encryption() {
        ...
        let signing_string =
            base64::encode_config(&SIGNING_SECRET_KEY_DATA, base64::URL_SAFE_NO_PAD);
        let sensitive = "SENSITIVE_INVALID_ENCRYPTION_KEY!";
        let string = format!("{},{}", sensitive, signing_string); // invalid
        ...
        assert_eq!(
            error,
            "Serialized CryptDE must have valid Base64; contents redacted".to_string()
        );
    ...
    fn make_from_str_can_fail_on_base64_syntax_for_signing() {
        ...
        let encryption_string =
            base64::encode_config(&ENCRYPTION_SECRET_KEY_DATA, base64::URL_SAFE_NO_PAD);
        let sensitive = "SENSITIVE_INVALID_SIGNING_KEY!";
        let string = format!("{},{}", encryption_string, sensitive); // invalid
    ...
    fn encode_with_invalid_key() {
        ...
        assert_eq!(
            CryptdecError::InvalidKey("public key has invalid length 15; expected 64".to_string()),
            result.err().unwrap()
        );
    }
    ...
    fn encode_sym_with_invalid_key() {
        ...
        assert_eq!(
            CryptdecError::InvalidKey(
                "symmetric key has invalid length 15; expected 32".to_string()
            ),
            result.err().unwrap()
        );
    }
    ...
    #[test]
    fn decode_sym_with_invalid_key_redacts_secret_material() {
        let subject = CryptDEReal::default();
        let sensitive_key = b"SENSITIVE_SHORT_SYMMETRIC_KEY";
        let data = CryptData::new(&[0u8; secretbox::NONCEBYTES + 1]);

        let result = subject.decode_sym(&SymmetricKey::new(sensitive_key), &data);
        let error = result.err().unwrap();

        assert_eq!(
            error,
            CryptdecError::InvalidKey(format!(
                "symmetric key has invalid length {}; expected {}",
                sensitive_key.len(),
                secretbox::KEYBYTES
            ))
        );
        assert!(!format!("{:?}", error).contains("SENSITIVE_SHORT_SYMMETRIC_KEY"));
    }
    ...
    fn fails_to_destringify_bad_base64_public_key_string_properly() {
        let subject = CryptDEReal::default();
        let half_key_string = "SENSITIVE_INVALID_PUBLIC_KEY_((]--$";
        ...
        assert!(!result.unwrap_err().contains(half_key_string));
    }
    ...
    fn fails_to_destringify_short_public_key_string_properly() {
        ...
        assert_eq!(
            result,
            Err(format!("Public key must decode to 32 bytes, not 31"))
        );
        assert!(!result.unwrap_err().contains(&short_half_key_string));
    }
    ...
    fn fails_to_destringify_long_public_key_string_properly() {
        ...
        assert_eq!(
            result,
            Err(format!("Public key must decode to 32 bytes, not 33"))
        );
        assert!(!result.unwrap_err().contains(&long_half_key_string));
    }
