        ...
        E: serde::de::Error,
    {
        if v.len() != sha1::DIGEST_LENGTH {
            return Err(serde::de::Error::custom(format!(
                "can't deserialize StreamKey: expected {} bytes, got {}",
                sha1::DIGEST_LENGTH,
                v.len()
            )));
        }
...
mod tests {
    ...
    #[test]
    fn debug_implementation_redacts_stream_correlation() {
        let subject = StreamKey::make_meaningful_stream_key("These are the times");

        let result = format!("{:?}", subject);

        assert_eq!(result, "[REDACTED]".to_string());
    }

    #[test]
    fn display_implementation_redacts_stream_correlation() {
        let subject = StreamKey::make_meaningful_stream_key("These are the times");
    ...
    #[test]
    fn malformed_serialized_stream_key_error_redacts_bytes() {
        let result = serde_cbor::de::from_slice::<StreamKey>(&[0x43, 0xAA, 0xBB, 0xCC]);

        let error = result.unwrap_err().to_string();
        assert!(error.contains("expected 20 bytes, got 3"));
        assert!(!error.contains("170"));
        assert!(!error.contains("187"));
