...
use crate::sub_lib::blockchain_bridge::ProviderSettlementReconciliationResult;
...
use crate::sub_lib::service_receipt::{ServiceReceiptOfferPayload_0v1, ServiceReceiptPayload_0v1};
...
impl Debug for NoLookupIncipientCoresPackage {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.write_str("NoLookupIncipientCoresPackage { package_data: [REDACTED] }")
    }
}
...
pub struct IncipientCoresPackage {
    pub route: Route,
    pub payload: CryptData,
    /// Hop offers are individually encrypted to the temporary payer-session key. Peripheral
    /// components preserve this opaque capsule across the request/response turn.
    pub routing_receipt_offers: Vec<CryptData>,
}

impl Debug for IncipientCoresPackage {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "IncipientCoresPackage {{ route_hop_count: {}, routing_receipt_offer_count: {}, package_data: [REDACTED] }}",
            self.route.hops.len(),
            self.routing_receipt_offers.len()
        )
    }
}
...
pub enum MessageType {
    ...
    ServiceReceipt(VersionedData<ServiceReceiptPayload_0v1>),
    ServiceReceiptOffer(VersionedData<ServiceReceiptOfferPayload_0v1>),
    MeteredClientResponse(VersionedData<MeteredClientResponsePayload_0v1>),
}

impl Debug for MessageType {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.write_str(match self {
            Self::ClientRequest(_) => "MessageType::ClientRequest([REDACTED])",
            Self::ClientResponse(_) => "MessageType::ClientResponse([REDACTED])",
            Self::Gossip(_) => "MessageType::Gossip([REDACTED])",
            Self::GossipFailure(_) => "MessageType::GossipFailure([REDACTED])",
            Self::DnsResolveFailed(_) => "MessageType::DnsResolveFailed([REDACTED])",
            Self::ServiceReceipt(_) => "MessageType::ServiceReceipt([REDACTED])",
            Self::ServiceReceiptOffer(_) => "MessageType::ServiceReceiptOffer([REDACTED])",
            Self::MeteredClientResponse(_) => "MessageType::MeteredClientResponse([REDACTED])",
        })
    }
}
...
impl Into<MessageTypeLite> for MessageType {
    fn into(self) -> MessageTypeLite {
        match self {
            ...
            MessageType::ServiceReceipt(_) => MessageTypeLite::ServiceReceipt,
            MessageType::ServiceReceiptOffer(_) => MessageTypeLite::ServiceReceiptOffer,
            MessageType::MeteredClientResponse(_) => MessageTypeLite::MeteredClientResponse,
        }
    }
}
...
impl IncipientCoresPackage {
    ...
    pub fn with_routing_receipt_offers(mut self, offers: Vec<CryptData>) -> Self {
        self.routing_receipt_offers = offers;
        self
    }
}
...
impl<T> Debug for ExpiredCoresPackage<T> {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "ExpiredCoresPackage {{ payload_len: {}, routing_receipt_offer_count: {}, package_data: [REDACTED] }}",
            self.payload_len,
            self.routing_receipt_offers.len()
        )
    }
...
impl<T> ExpiredCoresPackage<T> {
    ...
    pub fn new_with_routing_receipt_offers(
        immediate_neighbor: SocketAddr,
        paying_wallet: Option<Wallet>,
        remaining_route: Route,
        payload: T,
...
pub struct HopperConfig {
    ...
    pub crashable: bool,
    pub service_receipt_config_opt: Option<ServiceReceiptConfig>,
}

#[derive(Clone)]
pub struct ServiceReceiptConfig {
    pub data_directory: PathBuf,
    pub chain: masq_lib::blockchains::chains::Chain,
    pub chain_id: u64,
...
pub struct HopperSubs {
    ...
    pub provider_settlement_reconciliation_result:
        Recipient<ProviderSettlementReconciliationResult>,
}
...
mod tests {
    ...
    #[test]
    fn cores_package_debug_redacts_routes_identities_wallets_and_payloads() {
        let no_lookup = NoLookupIncipientCoresPackage {
            public_key: PublicKey::new(b"private no-lookup key"),
            node_addr: NodeAddr::new(&IpAddr::from_str("203.0.113.41").unwrap(), &[4312]),
            payload: CryptData::new(b"private no-lookup payload"),
        };
        let incipient = IncipientCoresPackage {
            route: Route { hops: vec![] },
            payload: CryptData::new(b"private incipient payload"),
            routing_receipt_offers: vec![CryptData::new(b"private routing offer")],
        };
        let message_type =
            make_meaningless_message_type(StreamKey::make_meaningful_stream_key("private stream"));
        let expired = ExpiredCoresPackage::new_with_routing_receipt_offers(
            SocketAddr::from_str("203.0.113.42:9876").unwrap(),
            Some(make_paying_wallet(b"private expired wallet")),
            Route { hops: vec![] },
            "private expired payload".to_string(),
            29,
            vec![CryptData::new(b"private expired offer")],
        );

        assert_eq!(
            format!("{:?}", no_lookup),
            "NoLookupIncipientCoresPackage { package_data: [REDACTED] }"
        );
        assert_eq!(
            format!("{:?}", incipient),
            "IncipientCoresPackage { route_hop_count: 0, routing_receipt_offer_count: 1, package_data: [REDACTED] }"
        );
        assert_eq!(
            format!("{:?}", message_type),
            "MessageType::DnsResolveFailed([REDACTED])"
        );
        assert_eq!(
            format!("{:?}", expired),
            "ExpiredCoresPackage { payload_len: 29, routing_receipt_offer_count: 1, package_data: [REDACTED] }"
        );
    }
    ...
    fn message_type_can_be_converted_in_to_message_type_lite() {
        ...
        let service_receipt =
            MessageType::ServiceReceipt(VersionedData::test_new(dv!(0, 1), vec![]));
        let service_receipt_offer =
            MessageType::ServiceReceiptOffer(VersionedData::test_new(dv!(0, 1), vec![]));
        let metered_client_response =
            MessageType::MeteredClientResponse(VersionedData::test_new(dv!(0, 1), vec![]));
        ...
        let service_receipt_result: MessageTypeLite = service_receipt.into();
        let service_receipt_offer_result: MessageTypeLite = service_receipt_offer.into();
        let metered_client_response_result: MessageTypeLite = metered_client_response.into();
        ...
        assert_eq!(service_receipt_result, MessageTypeLite::ServiceReceipt);
        assert_eq!(
            service_receipt_offer_result,
            MessageTypeLite::ServiceReceiptOffer
        );
        assert_eq!(
            metered_client_response_result,
            MessageTypeLite::MeteredClientResponse
        );
    }

    #[test]
    fn receipt_variant_preserves_existing_wire_messages_and_fails_closed_for_legacy_peers() {
        let existing = MessageType::ClientRequest(VersionedData::test_new(dv!(0, 1), vec![]));
        let existing_bytes = serde_cbor::to_vec(&existing).unwrap();
        assert!(matches!(
            serde_cbor::from_slice::<LegacyMessageType>(&existing_bytes),
            Ok(LegacyMessageType::ClientRequest(_))
        ));

        let receipt = MessageType::ServiceReceipt(VersionedData::test_new(dv!(0, 1), vec![]));
        let receipt_bytes = serde_cbor::to_vec(&receipt).unwrap();
        assert!(serde_cbor::from_slice::<LegacyMessageType>(&receipt_bytes).is_err());
    }
}
