...
impl fmt::Debug for CryptData {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> Result<(), fmt::Error> {
        write!(
            f,
            "CryptData {{ len: {}, data: [REDACTED] }}",
            self.data.len()
        )
    }
}
...
impl fmt::Debug for CryptdecError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> Result<(), fmt::Error> {
        f.write_str(match self {
            Self::EmptyKey => "EmptyKey",
            Self::EmptyData => "EmptyData",
            Self::InvalidKey(_) => "InvalidKey([REDACTED])",
            Self::InvalidSignature(_) => "InvalidSignature([REDACTED])",
            Self::OpeningFailed => "OpeningFailed",
            Self::OtherError(_) => "OtherError([REDACTED])",
        })
    }
}
...
impl fmt::Debug for SerdeCborError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> Result<(), fmt::Error> {
        f.write_str("SerdeCborError([REDACTED])")
    }
}
...
impl fmt::Debug for CodexError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> Result<(), fmt::Error> {
        match self {
            ...
            Self::DeserializationError(_) => f.write_str("DeserializationError([REDACTED])"),
            Self::EncryptionError(error) => write!(f, "EncryptionError({:?})", error),
            Self::DecryptionError(error) => write!(f, "DecryptionError({:?})", error),
            Self::RoutingError(error) => write!(f, "RoutingError({:?})", error),
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn cryptographic_debug_redacts_secrets_plaintext_ciphertext_and_error_details() {
        let private_key = PrivateKey::new(b"private key debug marker");
        let symmetric_key = SymmetricKey::new(b"symmetric key debug marker");
        let crypt_data = CryptData::new(b"ciphertext debug marker");
        let plain_data = PlainData::new(b"plaintext debug marker");
        ...
        assert_eq!(
            format!("{:?}", crypt_data),
            "CryptData { len: 23, data: [REDACTED] }"
        );
        assert_eq!(
            format!("{:?}", plain_data),
            "PlainData { len: 22, data: [REDACTED] }"
        );
        assert_eq!(
            format!(
                "{:?}",
                CryptdecError::InvalidKey("key detail marker".to_string())
            ),
            "InvalidKey([REDACTED])"
        );
        assert_eq!(
            format!(
                "{:?}",
                CryptdecError::OtherError("codec detail marker".to_string())
            ),
            "OtherError([REDACTED])"
        );
    }
    ...
    fn encodex_handles_serialization_error() {
        ...
        assert_eq!(
            format!("{:?}", result),
            "Err(SerializationError([REDACTED]))".to_string()
        );
    }
    ...
    fn decodex_handles_deserialization_error() {
        ...
        assert_eq!(
            format!("{:?}", result),
            "Err(DeserializationError([REDACTED]))".to_string()
        );
    }
}
