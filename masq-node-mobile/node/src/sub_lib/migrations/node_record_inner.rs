    ...
    fn try_from(value: &Value) -> Result<Self, Self::Error> {
        ...
        match value {
            Value::Map(map) => {
                ...
                let mut protocol_capabilities_opt: Option<u64> = None;
                let mut receipt_settlement_contract_opt: Option<Address> = None;
                let mut invalid_receipt_settlement_contract = false;
                map.keys().for_each(|k| {
                    ...
                    match (k, v) {
                        ...
                        (Value::Text(field_name), Value::Integer(field_value)) => {
                            match field_name.as_str() {
                                ...
                                "protocol_capabilities" => match field_value {
                                    n if *n < 0 => (),
                                    n if *n > u64::MAX as i128 => (),
                                    n => protocol_capabilities_opt = Some(*n as u64),
                                },
                                _ => (),
                            }
                        }
                        (Value::Text(field_name), Value::Text(field_value)) => {
                            match field_name.as_str() {
                                ...
                                "receipt_settlement_contract_opt" => {
                                    receipt_settlement_contract_opt = field_value
                                        .strip_prefix("0x")
                                        .and_then(|address| Address::from_str(address).ok())
                                        .filter(|address| *address != Address::zero());
                                    invalid_receipt_settlement_contract =
                                        receipt_settlement_contract_opt.is_none();
                                }
                                _ => (),
                            }
                        }
                        (Value::Text(field_name), Value::Null) => {
                            if field_name == "receipt_settlement_contract_opt" {
                ...
                if invalid_receipt_settlement_contract {
                    return Err(StepError::SemanticError(
                        "Invalid receipt_settlement_contract_opt".to_string(),
                    ));
                }
                Ok(NodeRecordInner_0v1 {
                    ...
                    country_code_opt,
                    protocol_capabilities: protocol_capabilities_opt.unwrap_or(0),
                    receipt_settlement_contract_opt,
                })
            }
...
mod tests {
    ...
    #[test]
    fn node_record_inner_missing_fields_are_rejected_without_panicking() {
        let result = NodeRecordInner_0v1::try_from(&Value::Map(Default::default()));
    ...
    fn can_migrate_from_the_future() {
        ...
        let future_nri = ExampleFutureNRI {
            ...
            receipt_settlement_contract_opt: expected_nri.receipt_settlement_contract_opt,
