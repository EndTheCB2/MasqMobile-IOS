...
mod tests {
    ...
    #[test]
    fn future_gossip_migration_rejects_non_map_without_panicking() {
        let value = Value::Text("not a gossip map".to_string());

        let result = Gossip_0v1::try_from(&value);

        assert_eq!(
            result,
            Err(StepError::SemanticError(
                "Expected Value::Map; found Text(\"not a gossip map\")".to_string()
            ))
        );
    }
