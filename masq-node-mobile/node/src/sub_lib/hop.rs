...
pub struct LiveHop {
    ...
    /// Present only on receipt-capable data routes. Because each `LiveHop` is onion-encrypted for
    /// its current provider, this wallet-delegated authorization is not broadcast to the route.
    #[serde(default)]
    pub routing_receipt_request_opt: Option<ReceiptSessionRequest>,
    pub component: Component,
}

impl std::fmt::Debug for LiveHop {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            formatter,
            "LiveHop {{ public_key: [REDACTED], payer_present: {}, routing_receipt_request_present: {}, component: {:?} }}",
            self.payer.is_some(),
            self.routing_receipt_request_opt.is_some(),
            self.component
        )
    }
}
...
impl LiveHop {
    ...
    pub fn with_routing_receipt_request(mut self, request: Option<ReceiptSessionRequest>) -> Self {
        self.routing_receipt_request_opt = request;
        self
    }
...
mod tests {
    ...
    #[test]
    fn legacy_hop_without_receipt_authorization_defaults_to_unmetered() {
        let cryptde = CRYPTDE_PAIR.main.as_ref();
        let legacy = LegacyLiveHop {
            public_key: PublicKey::new(b"legacy next hop"),
            payer: None,
            component: Component::Hopper,
        };
        let encrypted = encodex(cryptde, cryptde.public_key(), &legacy).unwrap();

        let decoded = LiveHop::decode(cryptde, &encrypted).unwrap();

        assert_eq!(decoded.public_key, legacy.public_key);
        assert_eq!(decoded.component, legacy.component);
        assert_eq!(decoded.routing_receipt_request_opt, None);
    }

    #[test]
    fn live_hop_debug_redacts_route_identity_and_payment_material() {
        let key = PublicKey::new(b"live hop debug identity");
        let hop = LiveHop::new(
            &key,
            Some(
                make_paying_wallet(b"live hop debug wallet")
                    .as_payer(&key, &TEST_DEFAULT_CHAIN.rec().contract),
            ),
            Component::Neighborhood,
        );
        ...
        assert_eq!(
            format!("{:?}", hop),
            "LiveHop { public_key: [REDACTED], payer_present: true, routing_receipt_request_present: false, component: Neighborhood }"
        );
    }
}
