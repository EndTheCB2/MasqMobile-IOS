    ...
    fn determine_mode_and_priv_req(&self, args: &[String]) -> (Mode, bool) {
        if args.contains(&"--dump-config".to_string()) {
            ...
            (
                Mode::Service,
                !args.iter().any(|arg| {
                    arg == "--mobile-proxy-port" || arg.starts_with("--mobile-proxy-port=")
                }),
            )
        }
    }
...
impl Runner for RunnerReal {
    fn run_node(&self, args: &[String], streams: &mut StdStreams<'_>) -> Result<(), RunnerError> {
        ...
        if crate::mobile_runtime::is_embedded() {
            crate::mobile_runtime::register_system(System::current());
        }
...
mod tests {
    ...
    #[test]
    fn mobile_consume_service_does_not_require_a_privileged_process() {
        check_mode(
