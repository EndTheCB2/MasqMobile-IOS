...
use masq_lib::constants::{
    NODE_ALREADY_RUNNING_ERROR, NODE_LAUNCH_ERROR, NODE_NOT_RUNNING_ERROR, UNMARSHAL_ERROR,
};
...
use masq_lib::messages::{
    ...
    UiUndeliveredFireAndForget, UiUnmarshalError,
};
...
impl Handler<NodeFromUiMessage> for Daemon {
    ...
    fn handle(&mut self, msg: NodeFromUiMessage, _ctx: &mut Self::Context) -> Self::Result {
        debug!(
            &self.logger,
            "Handling NodeFromUiMessage from client {} with opcode '{}' and path {:?}; payload redacted",
            msg.client_id,
            msg.body.opcode,
            msg.body.path
        );
...
impl Daemon {
    ...
    fn handle_unexpected_message(&mut self, client_id: u64, body: MessageBody) {
        ...
        let payload = match payload {
            ...
            Err((code, _message)) => {
                let message = format!(
                    "Cannot redirect {} request: error responses are not valid requests",
                    opcode
                );
                warning!(
                    &self.logger,
                    "Daemon rejected {} message from UI {} with error payload code {}",
                    opcode,
                    client_id,
                    code
                );
                let response = match path {
                    Conversation(context_id) => MessageBody {
                        opcode,
                        path: Conversation(context_id),
                        payload: Err((UNMARSHAL_ERROR, message)),
                    },
                    FireAndForget => UiUnmarshalError {
                        message,
                        bad_data: format!("error response payload with code {}", code),
                    }
                    .tmb(0),
                };
                self.respond_to_ui(client_id, response);
                return;
            }
        };
        match self.port_if_node_is_running() {
            ...
            None => self.send_node_is_not_running_error(client_id, opcode, path),
        }
    }
...
mod tests {
    ...
    fn accepts_setup_when_node_is_running_and_returns_existing_setup() {
        ...
        subject_addr
            .try_send(NodeFromUiMessage {
                client_id: 1234,
                body: UiSetupRequest {
                    values: vec![UiSetupRequestValue::new("db-password", sensitive)],
                }
                .tmb(4321),
            })
            .unwrap();
        ...
        log_handler.exists_log_containing("DEBUG: Daemon: Handling NodeFromUiMessage from client 1234 with opcode 'setup' and path Conversation(4321); payload redacted");
        log_handler.exists_no_log_containing(sensitive);
    }
    ...
    #[test]
    fn rejects_conversational_error_payload_without_probing_or_crashing_the_node() {
        let (ui_gateway, _, ui_gateway_recording_arc) = make_recorder();
        let system = System::new("test");
        let process_is_running_params_arc = Arc::new(Mutex::new(vec![]));
        let verifier_tools = VerifierToolsMock::new()
        ...
        subject_addr
            .try_send(NodeFromUiMessage {
                ...
                body: MessageBody {
                    ...
                    payload: Err((123, "response-shaped input".to_string())),
                },
            })
            .unwrap();
        ...
        assert_eq!(
            ...
            &NodeToUiMessage {
                ...
                body: MessageBody {
                    ...
                    payload: Err((
                        UNMARSHAL_ERROR,
                        "Cannot redirect financials request: error responses are not valid requests"
                            .to_string(),
                    )),
                },
            }
        );
        assert!(process_is_running_params_arc.lock().unwrap().is_empty());
    }

    #[test]
    fn rejects_fire_and_forget_error_payload_with_an_unmarshal_error() {
        let (ui_gateway, _, ui_gateway_recording_arc) = make_recorder();
        let system = System::new("test");
        let mut subject = Daemon::new(Box::new(LauncherMock::new()));
        subject.node_ui_port = Some(7777);
        subject.node_process_id = Some(8888);
        subject.verifier_tools = Box::new(VerifierToolsMock::new().process_is_running_result(true));
        let subject_addr = subject.start();
        subject_addr
            .try_send(make_daemon_bind_message(ui_gateway))
            .unwrap();
        ...
        subject_addr
            .try_send(NodeFromUiMessage {
                ...
                body: MessageBody {
                    ...
                    path: FireAndForget,
                    payload: Err((456, "response-shaped input".to_string())),
                },
            })
            .unwrap();
        ...
        assert_eq!(
            ...
            &NodeToUiMessage {
                ...
                body: UiUnmarshalError {
                    message: "Cannot redirect uninventedMessage request: error responses are not valid requests"
                        .to_string(),
                    bad_data: "error response payload with code 456".to_string(),
                }
                .tmb(0),
            }
        );
