    ...
    fn structure_from_string(
        ...
        match parse_result {
            ...
            Err(e) => Err(MasqueradeError::MidLevelDataError(if e.is_syntax() {
                ...
                "Unexpected JSON parsing error; details redacted".to_string()
            })),
        }
    }
    ...
    fn data_vector_from_structure(
        ...
        match (structure.bodyText.clone(), structure.bodyData.clone()) {
            ...
            (None, Some(data)) => match base64::decode(&data[..]) {
                Ok(vec) => Ok(vec),
                Err(_) => Err(MasqueradeError::HighLevelDataError(
                    "Can't decode Base64; contents redacted".to_string(),
                )),
            },
...
mod tests {
    ...
    fn json_masquerader_handles_bad_base64() {
        let sensitive_marker = "SENSITIVE_BASE64_PACKET_FRAGMENT";
        verify_error(
            format!("{{\"bodyData\": \"{}()[]\"}}", sensitive_marker).as_ref(),
            "JsonMasquerader: High-level data error: Can't decode Base64; contents redacted",
        );
        TestLogHandler::new().exists_no_log_containing(sensitive_marker);
    }
