...
impl Handler<NewPublicIp> for Dispatcher {
    ...
    fn handle(&mut self, msg: NewPublicIp, _ctx: &mut Self::Context) -> Self::Result {
        match &self.node_descriptor.node_addr_opt {
            None => warning!(
                self.logger,
                "Received attempt to set public IP while not in Standard mode; address redacted - rejecting"
            ),
            Some(node_addr) => {
...
mod tests {
    ...
    fn new_ip_message_without_node_addr_is_logged_and_ignored() {
        ...
        let index = logs.exists_log_containing("WARN: Dispatcher: Received attempt to set public IP while not in Standard mode; address redacted - rejecting");
        assert!(!logs.get_log_at(index).contains("1.2.3.4"));
    }
}
