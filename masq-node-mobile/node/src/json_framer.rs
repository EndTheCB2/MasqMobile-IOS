...
impl Framer for JsonFramer {
    fn add_data(&mut self, data: &[u8]) {
        let combined_len_opt = self.data_so_far.len().checked_add(data.len());
        if combined_len_opt
            .map(|combined_len| combined_len > MAX_JSON_FRAME_BYTES)
            .unwrap_or(true)
        {
        ...
        self.data_so_far.extend_from_slice(data);
    }
...
mod tests {
    ...
    #[test]
    fn json_framer_discards_an_oversized_incomplete_frame_and_recovers() {
        let mut subject = JsonFramer::new();
        subject.add_data(b"{");
        assert_eq!(subject.take_frame(), None);
        assert_eq!(subject.data_so_far.len(), 1);

        subject.add_data(&vec![b'a'; MAX_JSON_FRAME_BYTES]);

        assert!(subject.data_so_far.is_empty());
        let valid_frame = b"{\"bodyText\":\"recovered\"}";
        subject.add_data(valid_frame);
        assert_eq!(subject.take_frame().unwrap().chunk, valid_frame);
        assert!(subject.data_so_far.is_empty());
    }
}
