...
pub struct PortConfiguration {
    ...
    /// The logical destination port reported to the proxy pipeline. Mobile clients bind an
    /// unprivileged loopback port but still need the existing TLS/CONNECT path for port 443.
    pub reception_port_override: Option<u16>,
}
...
impl PortConfiguration {
    pub fn new(
        ...
        PortConfiguration {
            ...
            is_clandestine,
            reception_port_override: None,
        }
    }

    pub fn with_reception_port(
        discriminator_factories: Vec<Box<dyn DiscriminatorFactory>>,
        is_clandestine: bool,
        reception_port: u16,
    ) -> PortConfiguration {
        PortConfiguration {
            discriminator_factories,
            is_clandestine,
            reception_port_override: Some(reception_port),
        }
    }

    pub fn reception_port(&self, bound_port: u16) -> u16 {
        self.reception_port_override.unwrap_or(bound_port)
    }
}

#[cfg(test)]
mod port_configuration_tests {
    use super::PortConfiguration;

    #[test]
    fn reception_port_defaults_to_the_bound_port() {
        assert_eq!(
            PortConfiguration::new(vec![], false).reception_port(12_345),
            12_345
        );
    }

    #[test]
    fn reception_port_can_be_overridden_for_a_mobile_listener() {
        assert_eq!(
            PortConfiguration::with_reception_port(vec![], false, 443).reception_port(12_345),
            443
        );
    }
}
...
impl RealUser {
    ...
    #[cfg(any(target_os = "ios", target_os = "android"))]
    fn home_dir_from_sudo_user(_sudo_user: String) -> PathBuf {
        PathBuf::from("/")
    }
...
impl BootstrapperConfig {
    ...
    pub fn merge_unprivileged(&mut self, unprivileged: BootstrapperConfig) {
        ...
        self.blockchain_bridge_config
            .receipt_settlement_contract_opt = unprivileged
            .blockchain_bridge_config
...
mod tests {
    ...
    fn merging_unprivileged_config_picks_correct_items() {
        ...
        let receipt_settlement_contract_opt = Some(ethereum_types::Address::from([0x77; 20]));
        ...
        unprivileged_config
            .blockchain_bridge_config
            .receipt_settlement_contract_opt = receipt_settlement_contract_opt;
