...
impl Debug for LiveCoresPackage {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "LiveCoresPackage {{ route_hop_count: {}, payload_len: {}, routing_receipt_offer_count: {}, package_data: [REDACTED] }}",
            self.route.hops.len(),
            self.payload.len(),
...
impl LiveCoresPackage {
    ...
    pub fn into_next_live(
        ...
        let next_live = LiveCoresPackage::new(self.route, self.payload)
            .with_routing_receipt_offers(self.routing_receipt_offers);
    ...
    pub fn from_incipient(
        ...
        Ok((
            LiveCoresPackage::new(route, incipient.payload)
                .with_routing_receipt_offers(incipient.routing_receipt_offers),
            next_hop,
        ))
    }
    ...
    pub fn to_expired(
        ...
        decodex::<MessageType>(payload_cryptde, &self.payload).map(|decoded_payload| {
            ExpiredCoresPackage::new_with_routing_receipt_offers(
                immediate_neighbor_addr,
                top_hop.payer.map(|p| p.wallet),
                self.route.clone(),
                decoded_payload,
                self.payload.len(),
                self.routing_receipt_offers.clone(),
            )
        })
    }
}
...
mod tests {
    ...
    #[test]
    fn live_cores_package_debug_redacts_route_payload_and_receipt_offers() {
        let package = LiveCoresPackage::new(
            Route { hops: vec![] },
            CryptData::new(b"private live payload"),
        )
        .with_routing_receipt_offers(vec![CryptData::new(b"private live offer")]);

        assert_eq!(
            format!("{:?}", package),
            "LiveCoresPackage { route_hop_count: 0, payload_len: 20, routing_receipt_offer_count: 1, package_data: [REDACTED] }"
        );
    }
    ...
    fn live_cores_package_can_be_produced_from_older_live_cores_package() {
        ...
        .unwrap();
        let routing_receipt_offers = vec![CryptData::new(b"opaque hop offer")];
        let subject = LiveCoresPackage::new(route.clone(), encrypted_payload.clone())
            .with_routing_receipt_offers(routing_receipt_offers.clone());

        let (next_hop, next_pkg) = subject.into_next_live(&relay_cryptde).unwrap();

        assert_eq!(next_pkg.routing_receipt_offers, routing_receipt_offers);
    ...
    fn expired_cores_package_can_be_constructed_from_live_cores_package() {
        ...
        let routing_receipt_offers = vec![CryptData::new(b"opaque expired hop offer")];
        let subject = LiveCoresPackage::new(route.clone(), encrypted_payload.clone())
            .with_routing_receipt_offers(routing_receipt_offers.clone());
        ...
        assert_eq!(result.routing_receipt_offers, routing_receipt_offers);
    ...
    fn live_cores_package_serialization_deserialization() {
        ...
        .with_routing_receipt_offers(vec![CryptData::new(&[5, 6, 7])]);
    ...
    #[test]
    fn legacy_live_package_without_receipt_capsule_defaults_to_empty() {
        let legacy = LegacyLiveCoresPackage {
            version: LiveCoresPackage::version(),
            route: make_meaningless_route(&CRYPTDE_PAIR),
            payload: CryptData::new(b"legacy payload"),
        };
        let serialized = serde_cbor::to_vec(&legacy).unwrap();

        let decoded = serde_cbor::from_slice::<LiveCoresPackage>(&serialized).unwrap();

        assert_eq!(decoded.version, legacy.version);
        assert_eq!(decoded.route, legacy.route);
        assert_eq!(decoded.payload, legacy.payload);
        assert!(decoded.routing_receipt_offers.is_empty());
    }
}
