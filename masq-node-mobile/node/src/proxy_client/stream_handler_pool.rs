...
fn should_report_legacy_request(payload_size: usize, receipt_metered: bool) -> bool {
    payload_size > 0 && !receipt_metered
}
...
impl StreamHandlerPoolReal {
    ...
    fn process_package(
        ...
        match Self::find_stream_with_key(&stream_key, &inner_arc) {
            Some(sender_wrapper) => {
                ...
                debug!(
                    logger,
                    "Spawning future to write {} bytes over existing exit stream; destination and stream identity redacted",
                    data_len
                );
                actix::spawn(future);
            }
            None => {
                if payload.sequenced_packet.data.is_empty() {
                    debug!(
                        ...
                        "Empty request payload received for nonexistent exit stream; identity redacted - ignoring"
                    )
                } else {
                    ...
                    debug!(
                        logger,
                        "Spawning future to write {} bytes over new exit stream; destination and stream identity redacted",
                        data_len
                    );
                    actix::spawn(future);
                }
    ...
    fn send_shutdown_signal_to_stream_reader(
        ...
        match reader_shutdown_tx.try_send(()) {
            Ok(()) => {
                debug!(
                    logger,
                    "A shutdown signal was sent to the StreamReader; stream identity redacted."
                );
            }
            Err(_e) => {
                debug!(
                    ...
                    a redacted stream. The channel is already gone."
                );
            }
        }
    }
    ...
    fn clean_up_bad_stream(
        ...
        error!(
            inner.logger,
            "Couldn't process request from CORES package; stream and error details redacted"
        );
        if let Some(stream_senders) = inner.stream_writer_channels.remove(stream_key) {
            debug!(inner.logger, "Removing exit stream writer; peer redacted");
    ...
    fn write_and_tend(
        ...
        debug!(
            logger,
            "Queueing write of {} bytes{} to exit stream; peer and stream identity redacted",
            payload_size,
        ...
        Self::queue_write(payload.sequenced_packet, sender_wrapper.clone()).and_then(move |_| {
            ...
            if should_report_legacy_request(payload_size, receipt_metered) {
            ...
            if last_data {
                match inner.stream_writer_channels.remove(&stream_key) {
                    Some(stream_senders) => {
                        debug!(
                            inner.logger,
                            "Removing StreamWriter and shutting down StreamReader; stream and peer redacted"
                        );
                        Self::send_shutdown_signal_to_stream_reader(
                            stream_senders.reader_shutdown_tx,
                            &stream_key,
                    ...
                    None => {
                        debug!(
                            inner.logger,
                            "Trying to remove StreamWriter and StreamReader for a redacted stream, but it's already gone"
                        )
                    }
                }
            }
    ...
    fn make_stream_with_key(
        ...
        debug!(
            logger,
            "No exit stream exists; resolving destination host [REDACTED]"
        );
    ...
    fn handle_ip(
        ...
        Box::new(
            ...
            .map_err(|_| "Could not establish exit stream; details redacted".to_string()),
        )
    }
    ...
    fn lookup_dns(
        ...
        let stream_key = payload.stream_key;
        let dns_attempt_id_opt = payload.dns_attempt_id_opt;
        ...
        Box::new(
            inner_arc
                ...
                .map_err(move |_| {
                    ...
                    "Could not establish exit stream; details redacted".to_string()
                }),
        )
    }
    ...
    fn handle_lookup_ip(
        ...
        let ip_addrs: Vec<IpAddr> = match lookup_result {
            Err(e) => {
                error!(
                    logger,
                    "Could not resolve destination host; destination and resolver details redacted"
                );
        ...
        if filtered_ip_addrs.is_empty() {
            info!(
                logger,
                "DNS resolution returned no valid address; destination and addresses redacted"
            );
        ...
        debug!(
            logger,
            "DNS resolution returned {} valid address(es); destination and addresses redacted",
            filtered_ip_addrs.len()
        );
        ...
        match result {
            Ok(sender_wrapper) => {
                debug!(
                    logger,
                    "Established exit stream; destination and stream identifiers redacted"
                );
                Ok(sender_wrapper)
            }
            Err(e) => {
                debug!(
                    logger,
                    "Failed to establish exit stream; destination, stream and error details redacted"
                );
        ...
        while let Ok((stream_key, sequence_number)) = self.stream_killer_rx.try_recv() {
            match inner.stream_writer_channels.remove(&stream_key) {
                Some(stream_senders) => {
                    ...
                    debug!(
                        inner.logger,
                        "Killed StreamWriter and StreamReader for a redacted stream and peer; sent server-drop report"
                    )
                }
                None => debug!(
                    inner.logger,
                    "Tried to kill StreamWriter for a redacted stream, but it was already gone"
                ),
            }
        }
    }
        ...
        loop {
            match self.stream_adder_rx.try_recv() {
                ...
                Ok((stream_key, stream_senders)) => {
                    debug!(
                        inner.logger,
                        "Persisting StreamWriter; peer and stream identity redacted"
                    );
                    inner
...
mod tests {
    ...
    #[test]
    fn receipt_metered_requests_do_not_use_legacy_provider_accounting() {
        assert!(should_report_legacy_request(1, false));
        assert!(!should_report_legacy_request(1, true));
        assert!(!should_report_legacy_request(0, false));
    }
    ...
    fn dns_resolution_failure_sends_a_message_to_proxy_client() {
        ...
        assert_eq!(
            &DnsResolveFailure_0v1::for_attempt(stream_key, Some(37)),
    ...
    fn write_failure_for_nonexistent_stream_generates_termination_message() {
        ...
        TestLogHandler::new().exists_log_containing(&format!(
            "DEBUG: {test_name}: A shutdown signal was sent to the StreamReader; stream identity redacted."
        ));
    }
    ...
    fn stream_handler_pool_sends_shutdown_signal_when_last_data_is_true() {
        ...
        TestLogHandler::new().await_log_containing(
            &format!(
                "DEBUG: {test_name}: Removing StreamWriter and shutting down StreamReader; stream and peer redacted"
            ),
            500,
        );
    }
    ...
    fn stream_handler_pool_logs_when_shutdown_channel_is_broken() {
        ...
        TestLogHandler::new().await_log_containing(
            &format!(
                ...
                for a redacted stream. The channel is already gone."
            ),
            500,
        );
    }
    ...
    fn wildcard_ip_resolves_in_dns_failure() {
        ...
        test_log_handler.await_log_containing(&format!("INFO: {test_name}: DNS resolution returned no valid address; destination and addresses redacted"), 10_000);
        test_log_handler.await_log_containing(&format!("ERROR: {test_name}: Couldn't process request from CORES package; stream and error details redacted"), 10_000);
    }
    ...
    fn bad_dns_lookup_produces_log_and_sends_error_response() {
        ...
        TestLogHandler::new().exists_log_containing(
            "ERROR: bad_dns_lookup_produces_log_and_sends_error_response: Could not resolve destination host; destination and resolver details redacted",
        );
    }
    ...
    fn error_from_tx_to_writer_removes_stream() {
        ...
        tlh.await_log_containing("Removing exit stream writer; peer redacted", 1000);
    }
    ...
    fn clean_up_dead_streams_logs_when_the_stream_reader_is_down() {
        ...
        TestLogHandler::new().exists_log_containing(&format!(
            ...
            to the StreamReader for a redacted stream. \
    ...
    fn add_new_streams_works() {
        ...
        tlh.exists_log_containing(&format!(
            "DEBUG: {test_name}: Persisting StreamWriter; peer and stream identity redacted"
        ));
    }
}
