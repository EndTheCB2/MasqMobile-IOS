    ...
    fn poll(&mut self) -> Result<Async<<Self as Future>::Item>, <Self as Future>::Error> {
        ...
        loop {
            if self.shutdown_signal.try_recv().is_ok() {
                info!(self.logger, "Shutting down exit stream; identity redacted");
            ...
            match self.stream.poll_read(&mut buf) {
                ...
                Ok(Async::Ready(0)) => {
                    ...
                    debug!(
                        self.logger,
                        "Exit stream closed: zero-byte read; peer redacted"
                    );
                    self.shutdown();
                    return Ok(Async::Ready(()));
                }
                Ok(Async::Ready(len)) => {
                    ...
                    if self.logger.trace_enabled() {
                        trace!(self.logger, "Read {}-byte exit chunk; peer redacted", len);
                    }
                ...
                Err(e) => {
                    if indicates_dead_stream(e.kind()) {
                        debug!(
                            self.logger,
                            "Exit stream closed; peer and transport error redacted"
                        );
                        self.shutdown();
                        return Err(());
                    } else {
                        ...
                        if consecutive_read_errors >= MAX_CONSECUTIVE_READ_ERRORS {
                            error!(
                                self.logger,
                                "Abandoning exit stream after {} consecutive read errors; peer and transport details redacted",
                        ...
                        warning!(
                            self.logger,
                            "Continuing after exit-stream read error; peer and transport details redacted"
                        );
                    }
                }
            }
...
impl StreamReader {
    pub fn new(
        ...
        let logger = Logger::new("StreamReader");
...
mod tests {
    ...
    fn receiving_0_bytes_kills_stream() {
        ...
        TestLogHandler::new().exists_log_containing(&format!(
            "DEBUG: {test_name}: Exit stream closed: zero-byte read; peer redacted"
        ));
    }
    ...
    fn non_dead_stream_read_errors_log_but_do_not_shut_down() {
        ...
        TestLogHandler::new().exists_log_containing(
            &format!("WARN: {test_name}: Continuing after exit-stream read error; peer and transport details redacted"),
        );
        let proxy_client_recording = proxy_client_recording_arc.lock().unwrap();
        assert_eq!(
            proxy_client_recording.get_record::<InboundServerData>(0),
    ...
    #[test]
    fn stream_reader_bounds_consecutive_non_dead_read_errors_and_kills_stream() {
        init_test_logging();
        let test_name = "stream_reader_bounds_consecutive_non_dead_read_errors_and_kills_stream";
        let mut stream = ReadHalfWrapperMock::new();
        stream.poll_read_results = (0..MAX_CONSECUTIVE_READ_ERRORS)
            .map(|_| (vec![], Err(Error::from(ErrorKind::Other))))
            .collect();
        let (stream_killer, stream_killer_params) = unbounded();
        let mut subject = make_subject();
        subject.stream = Box::new(stream);
        ...
        let stream_key = subject.stream_key;

        let result = subject.poll();

        assert_eq!(result, Err(()));
        assert_eq!(stream_killer_params.try_recv(), Ok((stream_key, 0)));
        assert!(stream_killer_params.try_recv().is_err());
        TestLogHandler::new().exists_log_containing(&format!(
            "ERROR: {test_name}: Abandoning exit stream after {} consecutive read errors; peer and transport details redacted",
    ...
    fn stream_reader_shuts_down_when_it_receives_the_shutdown_signal() {
        ...
        assert_eq!(result, Ok(Async::Ready(())));
        TestLogHandler::new().exists_log_containing(&format!(
            "INFO: {test_name}: Shutting down exit stream; identity redacted"
        ));
    }
