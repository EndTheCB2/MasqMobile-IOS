...
mod tests {
    ...
    fn characterize_route_to_proxy_client() {
        let cryptde = CRYPTDE_PAIR.main.as_ref();
        let key = cryptde.public_key();
        let removed_hop_len = LiveHop::new(&key, None, Component::Hopper)
            .encode(&key, cryptde)
            .unwrap()
            .len();
        ...
        assert_eq!(subject.hops[2].len(), removed_hop_len);
    }
    ...
    fn characterize_route_to_proxy_server() {
        let cryptde = CRYPTDE_PAIR.main.as_ref();
        let key = cryptde.public_key();
        let first_removed_hop_len = LiveHop::new(&key, None, Component::Hopper)
            .encode(&key, cryptde)
            .unwrap()
            .len();
        let second_removed_hop_len = LiveHop::new(&key, None, Component::ProxyClient)
            .encode(&key, cryptde)
            .unwrap()
            .len();
        ...
        assert_eq!(subject.hops[1].len(), first_removed_hop_len);
        assert_eq!(subject.hops[2].len(), second_removed_hop_len);
    }
