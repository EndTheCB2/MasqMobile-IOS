...
impl std::fmt::Debug for StreamWriterKey {
...
impl StreamHandlerPool {
    ...
    fn set_up_stream_reader(
        ...
        debug!(
            self.logger,
            "Setting up {}clandestine StreamReader with reception_port {:?}; local and peer addresses redacted",
    ...
    fn handle_transmit_data_msg(&mut self, msg: TransmitDataMsg) {
        debug!(
            self.logger,
            "Handling order to transmit {} bytes; endpoint redacted",
            msg.data.len()
        );
        ...
        match msg.endpoint.clone() {
            Endpoint::Key(key) => {
                ...
                debug!(self.logger, "Sending key-based node query to Neighborhood");
            ...
            Endpoint::Socket(socket_addr) => {
                ...
                debug!(
                    self.logger,
                    "Translating TransmitDataMsg to socket-based node query response; endpoint redacted"
                );
    ...
    fn handle_remove_stream_msg(&mut self, msg: RemoveStreamMsg) {
        ...
        debug!(
            self.logger,
            "Stream closed; removing writer; local and peer addresses and writer key redacted"
        );
        let report_to_counterpart = match self.stream_writers.remove(&stream_writer_key) {
            None => {
                trace!(
                    self.logger,
                    "While handling RemoveStreamMsg: Stream Writers did not contain an entry; writer key redacted"
                );
                false
            }
            Some(None) => {
                error!(
                    self.logger,
                    "An unpopulated entry in stream_writers was found for a {:?} stream from a client; endpoints redacted. This shouldn't be possible. Investigate!",
                    msg.stream_type
                );
                false
            }
            Some(Some(_sender_wrapper)) => {
                trace!(
                    self.logger,
                    "While handling RemoveStreamMsg: Stream Writers contained an entry and writer; removing; writer key redacted"
                );
                true
            }
        };
        ...
        debug!(
            self.logger,
            "Signaling StreamShutdownMsg to Dispatcher for endpoint-redacted stream with stream type {:?}, {}report to counterpart",
    ...
    fn handle_dispatcher_node_query_response(&mut self, msg: DispatcherNodeQueryResponse) {
        ...
        debug!(
            self.logger,
            "Handling node query response; metadata redacted"
        );
        ...
        if node_addr.ports().is_empty() {
            error!(
                self.logger,
                "Neighborhood returned a NodeDescriptor with no ports; discarding {} bytes",
    ...
    fn extract_node_addr(&self, msg: &DispatcherNodeQueryResponse) -> Result<NodeAddr, String> {
        match msg.result.clone() {
            Some(metadata) => match metadata.node_addr_opt {
                ...
                None => Err("No known IP for neighbor in route; key redacted".to_string()),
            },
            None => Err("No Node found at endpoint; endpoint redacted".to_string()),
        }
    }
        ...
        match tx_box_opt_opt {
            ...
            None => {
                if peer_addr.ip() == localhost() {
                    return Err(format!(
                        "Local connection not found; endpoint redacted. Discarding {} bytes.",
                        msg.context.data.len()
                    ));
                };
    ...
    fn send_packet_on_open_stream(
        ...
        debug!(
            self.logger,
            "Found already-open stream; peer and writer key redacted; using"
        );
        ...
        let packet = if msg.context.sequence_number_opt.is_none() {
            ...
            match masquerader.mask(msg.context.data.as_slice()) {
                ...
                Err(_e) => {
                    return Err(format!(
                        "Masking failed; peer and error redacted. Discarding {} bytes.",
                        msg.context.data.len()
                    ));
                }
            }
        ...
        match tx_box.unbounded_send(packet) {
            Err(_e) => {
                error!(
                    self.logger,
                    "Removing channel to disabled StreamWriter; peer, writer key and error redacted"
                );
        ...
        if msg.context.last_data {
            debug!(
                self.logger,
                "Removing channel to StreamWriter in response to server-drop report; peer and writer key redacted"
            );
            return Ok(true);
        }
        Ok(false)
    ...
    fn delay_packet_for_opening_stream(
        ...
        debug!(
            self.logger,
            "Found stream being opened; peer and writer key redacted; preparing to use"
        );
        // a connection is already in progress. resubmit this message, to give the connection time to complete
        info!(
            self.logger,
            "Connection in progress; peer redacted; resubmitting {} bytes",
            msg.context.data.len()
        );
    ...
    fn open_new_stream_and_recycle_message(
        ...
        debug!(
            self.logger,
            "No existing stream; creating one; peer and writer key redacted"
        );
        ...
        debug!(self.logger, "Beginning connection attempt; peer redacted");
...
impl StreamStartFailureHandler {
    ...
    pub fn handle(self, _err: io::Error) {
        crate::mobile_runtime::report_connection_error(
            crate::mobile_runtime::MobileConnectionError::StreamConnectionFailed,
        );
        error!(
            self.logger,
            "Stream does not exist and could not be connected; peer and error redacted; discarding {} bytes",
            self.msg_data_len
        );
        self.remove_sub
...
mod tests {
    ...
    #[test]
    fn stream_writer_key_formatting_redacts_socket_identity() {
        let subject = StreamWriterKey::from(SocketAddr::from_str("1.2.3.4:5678").unwrap());

        assert_eq!(format!("{}", subject), "[REDACTED]");
        assert_eq!(format!("{:?}", subject), "[REDACTED]");
    }
    ...
    fn stream_handler_pool_writes_data_to_stream_writer() {
        ...
        TestLogHandler::new().await_log_containing(
            "WARN: StreamWriterUnsorted: Continuing after write error; transport error redacted",
            1000,
        );
    ...
    fn handle_remove_stream_msg_handles_stream_waiting_for_connect_scenario() {
        ...
        TestLogHandler::new().exists_log_containing(&format!(
            "ERROR: {}: An unpopulated entry in stream_writers was found for a Clandestine stream from a client; endpoints redacted. This shouldn't be possible. Investigate!",
            test_name
        ));
    }
    ...
    fn when_stream_handler_pool_fails_to_create_nonexistent_stream_for_write_then_it_logs_and_notifies_neighborhood(
        ...
        TestLogHandler::new().await_log_containing("ERROR: Dispatcher: Stream does not exist and could not be connected; peer and error redacted; discarding 5 bytes", 1000);
    ...
    fn transmit_data_msg_handler_finds_ip_from_neighborhood_and_transmits_message() {
        ...
        TestLogHandler::new().exists_log_containing(
            "DEBUG: Dispatcher: Sending key-based node query to Neighborhood",
        );
    }
    ...
    fn node_query_response_handler_does_not_try_to_write_when_neighbor_is_not_found() {
        ...
        TestLogHandler::new().await_log_containing(
            "ERROR: Dispatcher: No Node found at endpoint; endpoint redacted",
            1000,
        );
    }
    ...
    fn node_query_response_handler_does_not_try_to_write_when_neighbor_ip_is_not_known() {
        ...
        TestLogHandler::new().await_log_containing(
            "ERROR: Dispatcher: No known IP for neighbor in route; key redacted",
            1000,
        );
    }
    ...
    fn log_an_error_when_it_fails_to_send_a_packet() {
        ...
        let tlh = TestLogHandler::new();
        tlh.exists_log_containing("ERROR: Dispatcher: Removing channel to disabled StreamWriter; peer, writer key and error redacted");
    }
    ...
    fn node_query_response_handler_sets_counterpart_flag_and_removes_stream_writer_if_last_data_is_true(
        ...
        let tlh = TestLogHandler::new();
        tlh.exists_log_containing("DEBUG: Dispatcher: Removing channel to StreamWriter in response to server-drop report; peer and writer key redacted");
        assert_eq!(
            subject
                .stream_writers
    ...
    #[test]
    fn node_query_response_without_ports_is_discarded_without_stopping_stream_handler_pool() {
        init_test_logging();
        let test_name =
            "node_query_response_without_ports_is_discarded_without_stopping_stream_handler_pool";
        let cryptde = CRYPTDE_PAIR.main.as_ref();
        ...
        TestLogHandler::new().exists_log_containing(&format!(
            "ERROR: {test_name}: Neighborhood returned a NodeDescriptor with no ports; discarding 5 bytes"
        ));
    }
    ...
    fn stream_handler_pool_drops_data_when_masking_fails() {
        ...
        TestLogHandler::new().await_log_containing(
            "Masking failed; peer and error redacted. Discarding 5 bytes.",
            1000,
        );
    }
