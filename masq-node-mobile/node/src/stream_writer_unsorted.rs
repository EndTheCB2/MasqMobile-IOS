                    ...
                    match self.stream.poll_write(&packet.data) {
                        Err(error) => {
                            if indicates_dead_stream(error.kind()) {
                                error!(
                                    self.logger,
                                    "Cannot transmit {} bytes; transport error redacted",
                                    packet.data.len()
                                );
                                return Err(());
                            } else {
...
impl StreamWriterUnsorted {
    pub fn new(
        ...
        StreamWriterUnsorted {
            stream,
            rx_to_write,
            logger: Logger::new("StreamWriterUnsorted"),
            buf: None,
        }
    }
}
...
mod tests {
    ...
    fn stream_writer_bounds_consecutive_non_dead_write_errors() {
        ...
        TestLogHandler::new().exists_log_containing(&format!(
            "ERROR: StreamWriterUnsorted: Abandoning stream after {} consecutive write errors; transport error redacted",
            MAX_CONSECUTIVE_WRITE_ERRORS
        ));
    }
