                    ...
                    let (consuming_wallet_private_key_opt, consuming_wallet_address_opt) = {
                        ...
                        Ok(Some (private_key_hex)) => {
                            let private_key_bytes = match private_key_hex.from_hex::<Vec<u8>>() {
                                Ok(bytes) => bytes,
                                Err(e) => panic! ("Database corruption: consuming wallet private key cannot be converted from hexadecimal: {:?}", e),
                            };
                            let key_pair = match Bip32EncryptionKeyProvider::from_raw_secret(private_key_bytes.as_slice()) {
                                Ok(pair) => pair,
                                Err(e) => panic!("Database corruption: consuming wallet private key has invalid length or format: {:?}", e),
                            };
...
mod tests {
    ...
    #[test]
    #[should_panic(
        expected = "Database corruption: consuming wallet private key cannot be converted from hexadecimal: Invalid character 'L' at position 0"
    )]
    ...
    #[test]
    #[should_panic(
        expected = "Database corruption: consuming wallet private key has invalid length or format: \"Number of bytes of the secret differs from 32: 4\""
    )]
