...
impl NodeConfigurator<BootstrapperConfig> for NodeConfiguratorStandardPrivileged {
    fn configure(
        ...
        let mobile_proxy_port_opt = value_m!(multi_config, "mobile-proxy-port", u16);
        if mobile_proxy_port_opt.is_some()
            && value_m!(multi_config, "neighborhood-mode", String).as_deref()
                != Some("consume-only")
        {
            return Err(ConfiguratorError::required(
                "mobile-proxy-port",
                "--mobile-proxy-port requires --neighborhood-mode consume-only",
            ));
        }
        establish_port_configurations(&mut bootstrapper_config, mobile_proxy_port_opt);
...
pub fn establish_port_configurations(
    config: &mut BootstrapperConfig,
    mobile_proxy_port_opt: Option<u16>,
) {
    if let Some(mobile_proxy_port) = mobile_proxy_port_opt {
        config.port_configurations.insert(
            mobile_proxy_port,
            PortConfiguration::with_reception_port(
                vec![
...
mod tests {
    ...
    #[test]
    fn privileged_configuration_uses_one_mobile_tls_listener_in_consume_only_mode() {
        let _guard = EnvironmentGuard::new();
        running_test();
        let _clap_guard = ClapGuard::new();
        let subject = NodeConfiguratorStandardPrivileged::new();
        ...
        let config = subject
            .configure(&make_simplified_multi_config([
                "--mobile-proxy-port",
                "4443",
                "--neighborhood-mode",
                "consume-only",
        ...
        assert_eq!(config.port_configurations.len(), 1);
        let mobile_listener = config.port_configurations.get(&4443).unwrap();
        assert_eq!(mobile_listener.reception_port_override, Some(TLS_PORT));
        assert!(!mobile_listener.is_clandestine);
    }

    #[test]
    fn privileged_configuration_rejects_a_mobile_listener_outside_consume_only_mode() {
        let _guard = EnvironmentGuard::new();
        running_test();
        let _clap_guard = ClapGuard::new();
        let subject = NodeConfiguratorStandardPrivileged::new();
