...
pub(crate) const MAX_INITIAL_NEIGHBORS: usize = 10;
...
pub trait UnprivilegedParseArgsConfiguration {
    ...
    fn unprivileged_parse_args(
        ...
        unprivileged_config
            ...
            value_m!(multi_config, "receipt-settlement-contract", String).map(|address| {
                Address::from_str(&address[2..]).expect("validated settlement contract address")
            });
...
pub fn get_wallets(
    ...
    let consuming_wallet_opt = consuming_opt.map(|consuming_private_key| {
        let key_bytes = consuming_private_key
            .from_hex::<Vec<u8>>()
            .unwrap_or_else(|_| {
                panic!("Wallet corruption: consuming wallet private key is not hexadecimal")
            });
        let key_pair = Bip32EncryptionKeyProvider::from_raw_secret(key_bytes.as_slice())
            .unwrap_or_else(|_| {
                panic!(
                    "Wallet corruption: consuming wallet private key has invalid length or format"
                )
            });
...
pub fn make_neighborhood_config<T: UnprivilegedParseArgsConfiguration + ?Sized>(
    ...
    let unvalidated_neighbor_configs: Vec<NodeDescriptor> = {
    ...
    let neighbor_configs =
        validate_initial_neighbor_set(unvalidated_neighbor_configs, desired_chain)?;
...
fn validate_initial_neighbor_set(
    neighbors: Vec<NodeDescriptor>,
    desired_chain: Chain,
) -> Result<Vec<NodeDescriptor>, ConfiguratorError> {
    if neighbors.len() > MAX_INITIAL_NEIGHBORS {
        return Err(ConfiguratorError::required(
            "neighbors",
            &format!(
                "At most {} initial neighbors are allowed; received {}",
                MAX_INITIAL_NEIGHBORS,
                neighbors.len()
            ),
        ));
    }
    ...
    for neighbor in &neighbors {
        if neighbor.blockchain != desired_chain {
            return Err(ConfiguratorError::required(
                "neighbors",
                "Every initial neighbor must use the configured chain",
            ));
        }
        if neighbor.node_addr_opt.is_none() {
            return Err(ConfiguratorError::required(
                "neighbors",
                "Every initial neighbor must include an IP address and at least one port",
            ));
        }
        if !public_keys.insert(neighbor.encryption_public_key.clone()) {
            return Err(ConfiguratorError::required(
                "neighbors",
                "Initial neighbors must use unique public keys",
            ));
        }
    }
...
mod tests {
    ...
    fn make_initial_neighbor(index: u8, chain: Chain) -> NodeDescriptor {
        NodeDescriptor {
            blockchain: chain,
            encryption_public_key: PublicKey::new(&[index; 32]),
            node_addr_opt: Some(NodeAddr::new(
                &IpAddr::V4(Ipv4Addr::new(10, 0, 0, index.saturating_add(1))),
    ...
    #[test]
    fn initial_neighbor_validation_accepts_ten_unique_descriptors() {
        let neighbors = (0..MAX_INITIAL_NEIGHBORS)
            .map(|index| make_initial_neighbor(index as u8, Chain::EthRopsten))
            .collect::<Vec<_>>();

        let result = validate_initial_neighbor_set(neighbors.clone(), Chain::EthRopsten);

        assert_eq!(result, Ok(neighbors));
    }

    #[test]
    fn initial_neighbor_validation_rejects_more_than_ten_descriptors() {
        let neighbors = (0..=MAX_INITIAL_NEIGHBORS)
            .map(|index| make_initial_neighbor(index as u8, Chain::EthRopsten))
            .collect::<Vec<_>>();

        let result = validate_initial_neighbor_set(neighbors, Chain::EthRopsten);

        assert_eq!(
            result,
            Err(ConfiguratorError::required(
                "neighbors",
                "At most 10 initial neighbors are allowed; received 11",
            ))
        );
    }

    #[test]
    fn initial_neighbor_validation_rejects_duplicate_public_keys() {
        let first = make_initial_neighbor(1, Chain::EthRopsten);
        let mut duplicate_identity = first.clone();
        duplicate_identity.node_addr_opt = Some(NodeAddr::new(
            &IpAddr::V4(Ipv4Addr::new(10, 0, 0, 99)),
            &[19_999],
        ));

        let result =
            validate_initial_neighbor_set(vec![first, duplicate_identity], Chain::EthRopsten);
    ...
    #[test]
    fn initial_neighbor_validation_rejects_wrong_chain_and_missing_address() {
        let wrong_chain = validate_initial_neighbor_set(
            vec![make_initial_neighbor(1, Chain::EthMainnet)],
            Chain::EthRopsten,
        );
        let mut missing_address = make_initial_neighbor(2, Chain::EthRopsten);
        missing_address.node_addr_opt = None;
        let missing_address_result =
            validate_initial_neighbor_set(vec![missing_address], Chain::EthRopsten);
