...
impl StreamWriterSorted {
    pub fn new(
        ...
        StreamWriterSorted {
            stream,
            rx_to_write,
            logger: Logger::new("StreamWriterSorted"),
    ...
    fn write_from_buffer_to_stream(&mut self) -> WriteBufferStatus {
        ...
        loop {
            ...
            match packet_opt {
                Some(packet) => match self.stream.poll_write(&packet.data) {
                    Err(error) => {
                        if indicates_dead_stream(error.kind()) {
                            error!(
                                self.logger,
                                "Error writing {} bytes; peer and transport error redacted",
                                packet.data.len()
                            );
                            ...
                            if consecutive_write_errors >= MAX_CONSECUTIVE_WRITE_ERRORS {
                                error!(
                                    self.logger,
                                    "Abandoning stream after {} consecutive write errors; transport error redacted",
                            ...
                            warning!(
                                self.logger,
                                "Continuing after write error; transport error redacted"
                            );
                    ...
                    Ok(Async::Ready(len)) => {
                        ...
                        if len != packet.data.len() {
                            ...
                            debug!(self.logger, "Shutting down stream to client in response to server-drop report; peer redacted");
...
mod tests {
    ...
    fn stream_writer_logs_and_returns_err_when_it_gets_a_dead_stream_error() {
        ...
        TestLogHandler::new().await_log_containing(
            "Error writing 5 bytes; peer and transport error redacted",
            1000,
        );
    }
    ...
    fn stream_writer_logs_error_and_continues_when_it_gets_a_non_dead_stream_error() {
        ...
        TestLogHandler::new().exists_log_containing(
            "WARN: StreamWriterSorted: Continuing after write error; transport error redacted",
        );
    ...
    fn stream_writer_bounds_consecutive_non_dead_write_errors() {
        ...
        let writer = (0..MAX_CONSECUTIVE_WRITE_ERRORS)
            .fold(WriteHalfWrapperMock::new(), |writer, _| {
                writer.poll_write_result(Err(io::Error::from(ErrorKind::Other)))
            });
        let write_params = writer.poll_write_params.clone();
        let peer_addr = SocketAddr::from_str("1.2.3.4:5678").unwrap();
        let mut subject = StreamWriterSorted::new(Box::new(writer), peer_addr, rx);
        ...
        TestLogHandler::new().exists_log_containing(&format!(
            "ERROR: StreamWriterSorted: Abandoning stream after {} consecutive write errors; transport error redacted",
            MAX_CONSECUTIVE_WRITE_ERRORS
        ));
    }
