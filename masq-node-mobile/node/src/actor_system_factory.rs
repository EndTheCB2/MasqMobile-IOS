...
use super::proxy_server::receipt_session::ReceiptSessionConfig;
use super::proxy_server::receipt_session_recovery::ReceiptSessionRecoveryStoreReal;
...
use crate::accountant::db_access_objects::receipt_acknowledgement_outbox_dao::ReceiptAcknowledgementOutboxDaoReal;
...
use crate::sub_lib::hopper::{HopperConfig, ServiceReceiptConfig};
...
impl ActorSystemFactoryTools for ActorSystemFactoryToolsReal {
    fn prepare_initial_messages(
        ...
        let proxy_client_subs_opt = if !config.neighborhood_config.mode.is_consume_only() {
            Some(
                actor_factory.make_and_start_proxy_client(ProxyClientConfig {
                    ...
                    receipt_session_validation_opt: if config
                        .neighborhood_config
                        .mode
                        .is_decentralized()
                    {
                        config
                            ...
                            .map(|settlement_contract| {
                                crate::sub_lib::proxy_client::ReceiptSessionValidation {
                                    chain_id: config
                                        .blockchain_bridge_config
                                        .chain
                                        .rec()
        ...
        let hopper_subs = actor_factory.make_and_start_hopper(HopperConfig {
            ...
            is_decentralized: config.neighborhood_config.mode.is_decentralized(),
            crashable: is_crashable(&config),
            service_receipt_config_opt: if config.neighborhood_config.mode.is_decentralized() {
                config
                    .blockchain_bridge_config
                    .receipt_settlement_contract_opt
                    .map(|settlement_contract| ServiceReceiptConfig {
                        data_directory: config.data_directory.clone(),
                        chain: config.blockchain_bridge_config.chain,
                        chain_id: config.blockchain_bridge_config.chain.rec().num_chain_id,
                        settlement_contract,
                        payout_wallet_address: config.earning_wallet.address(),
                    })
            } else {
                None
            },
        ...
        if crate::mobile_runtime::is_embedded() {
            crate::mobile_runtime::register_neighborhood_ui(
                peer_actors.neighborhood.from_ui_message_sub.clone(),
            );
            crate::mobile_runtime::register_neighborhood_config(
                peer_actors.neighborhood.config_change_msg_sub.clone(),
            );
...
impl ActorSystemFactoryToolsReal {
    ...
    fn notify_of_public_ip_change(
        ...
        let failed_recipient_count = new_ip_recipients
            .iter()
            .filter(|recipient| {
                recipient
        ...
        if failed_recipient_count > 0 {
            warning!(
                ...
                "Could not deliver public IP update to {} recipient(s); address redacted",
    ...
    fn handle_housekeeping_thread_error(error: AutomapError) {
        warning!(
            Logger::new("ActorSystemFactory"),
            "Automap housekeeping failure; mapping will be retried: {:?}",
            error
        );
    }
    ...
    fn start_automap(
        ...
        if let NeighborhoodMode::Standard(node_addr, _, _) = &config.neighborhood_config.mode {
            ...
            let change_recipients = new_ip_recipients.clone();
            let change_handler = move |change: AutomapChange| match change {
                AutomapChange::NewIp(new_public_ip) => {
                    Self::notify_of_public_ip_change(&change_recipients, new_public_ip);
                }
...
impl ActorFactory for ActorFactoryReal {
    fn make_and_start_dispatcher(
        ...
        crate::mobile_runtime::expect_actor_arbiter();
        let arbiter = Arbiter::builder().stop_system_on_panic(true);
        let addr: Addr<Dispatcher> = arbiter.start(move |_| {
            crate::mobile_runtime::register_actor_arbiter(Arbiter::current());
            Dispatcher::new(node_descriptor, cryptde_pair_thread, crashable)
        });
        (
    ...
    fn make_and_start_proxy_server(&self, config: &BootstrapperConfig) -> ProxyServerSubs {
        ...
        let cryptde_pair_thread = self.cryptde_pair.clone();
        let crashable = is_crashable(config);
        let receipt_session_config_opt = if is_decentralized {
            config
                .blockchain_bridge_config
                .receipt_settlement_contract_opt
                .and_then(|settlement_contract| {
                    config
                        .consuming_wallet_opt
                        .as_ref()
                        .map(|wallet| ReceiptSessionConfig {
                            chain: config.blockchain_bridge_config.chain,
                            chain_id: config.blockchain_bridge_config.chain.rec().num_chain_id,
                            settlement_contract,
        ...
        let receipt_acknowledgement_outbox_opt = receipt_session_config_opt.as_ref().map(|_| {
            let connection = connection_or_panic(
                &DbInitializerReal::default(),
                config.data_directory.as_path(),
                DbInitializationConfig::panic_on_migration(),
            );
            Box::new(ReceiptAcknowledgementOutboxDaoReal::new(connection))
        });
        let receipt_session_recovery_store_opt =
            receipt_session_config_opt.as_ref().and_then(|_| {
                let password = match config.db_password_opt.as_ref() {
                    Some(password) => password,
                    None => {
                        warning!(
                        self.logger,
                        "Receipt sessions require a database password for secure crash recovery"
                    );
                        return None;
                    }
                };
                ...
                match ReceiptSessionRecoveryStoreReal::new(
                    &factory,
                    password,
                    config.blockchain_bridge_config.chain,
                ) {
                    Ok(store) => Some(Box::new(store) as Box<_>),
                    Err(error) => {
                        error!(
                            self.logger,
                            "Could not initialize encrypted receipt-session recovery: {}", error
                        );
                        None
                    }
                }
        ...
        let addr: Addr<ProxyServer> = arbiter.start(move |_| {
            ...
            if let Some(config) = receipt_session_config_opt {
                proxy_server.enable_recoverable_receipt_sessions(
                    config,
                    receipt_session_recovery_store_opt,
                );
            }
            if let Some(outbox) = receipt_acknowledgement_outbox_opt {
                proxy_server.enable_receipt_acknowledgement_outbox(outbox);
            }
            proxy_server
        });
    ...
    fn make_and_start_hopper(&self, config: HopperConfig) -> HopperSubs {
        crate::mobile_runtime::expect_actor_arbiter();
        let arbiter = Arbiter::builder().stop_system_on_panic(true);
        let addr: Addr<Hopper> = arbiter.start(move |_| {
            crate::mobile_runtime::register_actor_arbiter(Arbiter::current());
            Hopper::new(config)
        });
    ...
    fn make_and_start_neighborhood(&self, config: &BootstrapperConfig) -> NeighborhoodSubs {
        ...
        let addr: Addr<Neighborhood> = arbiter.start(move |_| {
            crate::mobile_runtime::register_actor_arbiter(Arbiter::current());
            Neighborhood::new(cryptde_pair_thread, &config_clone)
        });
    ...
    fn make_and_start_ui_gateway(&self, config: &BootstrapperConfig) -> UiGatewaySubs {
        ...
        crate::mobile_runtime::expect_actor_arbiter();
        let arbiter = Arbiter::builder().stop_system_on_panic(true);
        let addr: Addr<UiGateway> = arbiter.start(move |_| {
            crate::mobile_runtime::register_actor_arbiter(Arbiter::current());
            ui_gateway
        });
    ...
    fn make_and_start_stream_handler_pool(
        ...
        crate::mobile_runtime::expect_actor_arbiter();
        let arbiter = Arbiter::builder().stop_system_on_panic(true);
        let addr: Addr<StreamHandlerPool> = arbiter.start(move |_| {
            crate::mobile_runtime::register_actor_arbiter(Arbiter::current());
    ...
    fn make_and_start_proxy_client(&self, config: ProxyClientConfig) -> ProxyClientSubs {
        crate::mobile_runtime::expect_actor_arbiter();
        let arbiter = Arbiter::builder().stop_system_on_panic(true);
        let addr: Addr<ProxyClient> = arbiter.start(move |_| {
            crate::mobile_runtime::register_actor_arbiter(Arbiter::current());
            ProxyClient::new(config)
        });
    ...
    fn make_and_start_configurator(&self, config: &BootstrapperConfig) -> ConfiguratorSubs {
        ...
        let arbiter = Arbiter::builder().stop_system_on_panic(true);
        let cryptde_pair_thread = self.cryptde_pair.clone();
        let addr: Addr<Configurator> = arbiter.start(move |_| {
            crate::mobile_runtime::register_actor_arbiter(Arbiter::current());
            Configurator::new(data_directory, cryptde_pair_thread, crashable)
        });
        ConfiguratorSubs {
...
mod tests {
    ...
    fn make_and_start_actors_sends_bind_messages() {
        ...
        let config = BootstrapperConfig {
            ...
            blockchain_bridge_config: BlockchainBridgeConfig {
                ...
                receipt_settlement_contract_opt: None,
                gas_price: 1,
            },
    ...
    fn prepare_initial_messages_generates_the_correct_messages() {
        ...
        let config = BootstrapperConfig {
            ...
            blockchain_bridge_config: BlockchainBridgeConfig {
                blockchain_service_url_opt: None,
                chain: TEST_DEFAULT_CHAIN,
                receipt_settlement_contract_opt: Some(ethereum_types::Address::from([0x77; 20])),
                gas_price: 1,
            },
            port_configurations: HashMap::new(),
            db_password_opt: None,
        ...
        assert_eq!(
            hopper_config
                .service_receipt_config_opt
                .as_ref()
                .map(|config| config.settlement_contract),
            Some(ethereum_types::Address::from([0x77; 20]))
        );
        ...
        assert_eq!(
            proxy_client_config
                .receipt_session_validation_opt
                .as_ref()
                .map(|config| config.settlement_contract),
            Some(ethereum_types::Address::from([0x77; 20]))
        );
    ...
    #[test]
    fn change_handler_propagates_public_ip_change_without_stopping_the_node() {
        let system = System::new("test");
        let mut config = BootstrapperConfig::default();
        ...
        let (first_recorder, _, first_recording) = make_recorder();
        let (second_recorder, _, second_recording) = make_recorder();

        subject.start_automap(
            &config,
            Box::new(PersistentConfigurationMock::new()),
            vec![
                first_recorder.start().recipient(),
                second_recorder.start().recipient(),
            ],
        );
        ...
        for recording in [first_recording, second_recording] {
            check_new_ip_message(&recording, IpAddr::from_str("1.2.3.4").unwrap(), 0);
            check_new_ip_message(&recording, IpAddr::from_str("1.2.3.5").unwrap(), 1);
        }
    }

    #[test]
    fn public_ip_notification_tolerates_a_disconnected_recipient() {
        init_test_logging();
        let system = System::new("test");
        ...
        let failed_recipient_count = ActorSystemFactoryToolsReal::notify_of_public_ip_change(
            &[recipient],
            IpAddr::from_str("203.0.113.241").unwrap(),
        );

        assert_eq!(failed_recipient_count, 1);
        let logs = TestLogHandler::new();
        let index = logs.exists_log_containing(
            "WARN: ActorSystemFactory: Could not deliver public IP update to 1 recipient(s); address redacted",
        );
        assert!(!logs.get_log_at(index).contains("203.0.113.241"));
    }
    ...
    #[test]
    fn start_automap_change_handler_keeps_running_after_remapping_error() {
        init_test_logging();
        let mut subject = ActorSystemFactoryToolsReal::new();
        let make_params_arc = Arc::new(Mutex::new(vec![]));
        ...
        change_handler(AutomapChange::Error(AutomapError::TemporaryMappingError(
            "SENSITIVE_ROUTER_REMAP_DETAIL".to_string(),
        )));

        let logs = TestLogHandler::new();
        let index = logs.exists_log_containing(
            "WARN: ActorSystemFactory: Automap housekeeping failure; mapping will be retried: TemporaryMappingError([REDACTED])",
        );
        assert!(!logs
            .get_log_at(index)
            .contains("SENSITIVE_ROUTER_REMAP_DETAIL"));
    }
