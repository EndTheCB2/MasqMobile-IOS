...
impl WebSocketSupervisorReal {
    fn redacted_bad_data(message: &str) -> String {
        format!("<redacted malformed UI message: {} bytes>", message.len())
    }
    ...
    fn handle_text_message(
        ...
        match UiTrafficConverter::new_unmarshal_from_ui(message, client_id) {
            ...
            Err(Critical(e)) => {
                let redacted_bad_data = Self::redacted_bad_data(message);
                error!(
                    logger,
    ...
    fn handle_close_message(
        ...
        let client_id = match locked_inner.client_id_by_socket_addr.remove(&socket_addr) {
            None => match locked_inner.socket_addr_by_client_id.iter().find_map(
                |(client_id, registered_socket_addr)| {
                    (*registered_socket_addr == socket_addr).then_some(*client_id)
                },
            ) {
                Some(client_id) => {
                    warning!(
                        logger,
                        "UI at {} had a partial client registry during disconnect; completing idempotent cleanup for client ID {}",
                        socket_addr,
                        client_id
                    );
                    client_id
                ...
                None => {
                    warning!(
                        logger,
                        "Ignoring duplicate or incomplete disconnect from UI at {}; the client is already absent",
                        socket_addr
                    );
                    return err::<(), ()>(());
                }
    ...
    fn emergency_client_removal(client_id: u64, inner_arc: &Arc<Mutex<WebSocketSupervisorInner>>) {
        let mut locked_inner = inner_arc.lock().expect("WebSocketSupervisor is poisoned");
        let client_was_present = locked_inner.client_by_id.remove(&client_id).is_some();
        let socket_was_present = locked_inner
            .socket_addr_by_client_id
            .remove(&client_id)
            .is_some();
        let reverse_registration_count = locked_inner
            .client_id_by_socket_addr
            .values()
            .filter(|registered_client_id| **registered_client_id == client_id)
            .count();
        locked_inner
            .client_id_by_socket_addr
            .retain(|_, registered_client_id| *registered_client_id != client_id);
        if !client_was_present || !socket_was_present || reverse_registration_count != 1 {
            warning!(
                Logger::new("WebSocketSupervisor"),
                "Client {} registry was already partial or absent during emergency removal; idempotent cleanup completed",
                client_id
            );
        }
    }
    ...
    fn close_connection(
        ...
        let mut client = match locked_inner.client_by_id.remove(&client_id) {
            ...
            None => {
                warning!(
                    logger,
                    "UI at {} (client ID {}) was already absent before close acknowledgement; idempotent cleanup completed",
                    socket_addr,
                    client_id
                );
                return;
        ...
        if !socket_was_present {
            warning!(
                logger,
                "UI at {} (client ID {}) had no forward socket registration during close; cleanup continued",
                socket_addr,
                client_id
            );
        }
...
mod tests {
    ...
    fn logs_badly_formatted_json_and_returns_unmarshal_error() {
        ...
        let secret = "SENSITIVE_VALUE_SHOULD_NOT_LEAK";
        let bad_json = format!("}}: I am badly-formatted JSON with {} :{{", secret);
        ...
        assert_eq!(
            ...
            UiUnmarshalError {
                ...
                bad_data: WebSocketSupervisorReal::redacted_bad_data(&bad_json),
            }
        );
        TestLogHandler::new().exists_no_log_containing(secret)
    }
    ...
    fn bad_one_way_message_is_logged_and_returns_error() {
        ...
        assert_eq!(
            ...
            UiUnmarshalError {
                message: expected_traffic_conversion_message,
                bad_data: WebSocketSupervisorReal::redacted_bad_data(bad_message_json),
            }
        )
    }
    ...
    #[test]
    fn emergency_client_removal_is_idempotent_and_repairs_partial_registries() {
        init_test_logging();
        let mut inner = make_ordinary_inner();
        let socket_addr = SocketAddr::new(IpAddr::V4(Ipv4Addr::new(1, 2, 3, 5)), 55555);
        inner
            .client_by_id
            .insert(7, Box::new(ClientWrapperMock::new()));
        inner.socket_addr_by_client_id.insert(7, socket_addr);
        inner.client_id_by_socket_addr.insert(socket_addr, 7);
        let inner_arc = Arc::new(Mutex::new(inner));

        WebSocketSupervisorReal::emergency_client_removal(7, &inner_arc);
        WebSocketSupervisorReal::emergency_client_removal(7, &inner_arc);

        let locked_inner = inner_arc.lock().unwrap();
        assert!(locked_inner.client_by_id.is_empty());
        assert!(locked_inner.socket_addr_by_client_id.is_empty());
        assert!(locked_inner.client_id_by_socket_addr.is_empty());
        TestLogHandler::new().exists_log_containing(
            "WARN: WebSocketSupervisor: Client 7 registry was already partial or absent during emergency removal; idempotent cleanup completed",
        );
    }

    #[test]
    fn close_connection_tolerates_an_already_removed_client_and_repairs_stale_links() {
        init_test_logging();
        let mut inner = make_ordinary_inner();
        let socket_addr = SocketAddr::new(IpAddr::V4(Ipv4Addr::new(1, 2, 3, 6)), 56666);
        inner.socket_addr_by_client_id.insert(8, socket_addr);
        inner.client_id_by_socket_addr.insert(socket_addr, 8);

        WebSocketSupervisorReal::close_connection(
            &mut inner,
            8,
            socket_addr,
            &Logger::new("idempotent_close_test"),
        );

        assert!(inner.client_by_id.is_empty());
        assert!(inner.socket_addr_by_client_id.is_empty());
        assert!(inner.client_id_by_socket_addr.is_empty());
        TestLogHandler::new().exists_log_containing(
            "WARN: idempotent_close_test: UI at 1.2.3.6:56666 (client ID 8) was already absent before close acknowledgement; idempotent cleanup completed",
        );
    }

    #[test]
    fn close_message_recovers_a_missing_reverse_registration() {
        init_test_logging();
        let mut inner = make_ordinary_inner();
        let socket_addr = SocketAddr::new(IpAddr::V4(Ipv4Addr::new(1, 2, 3, 7)), 57777);
        let mock_client = ClientWrapperMock::new()
            .send_result(Ok(()))
            .flush_result(Ok(()));
        inner.client_by_id.insert(9, Box::new(mock_client));
        inner.socket_addr_by_client_id.insert(9, socket_addr);
        let inner_arc = Arc::new(Mutex::new(inner));

        let _ = WebSocketSupervisorReal::handle_close_message(
            &inner_arc,
            &Logger::new("partial_close_test"),
            socket_addr,
        );
        ...
        assert!(locked_inner.client_id_by_socket_addr.is_empty());
        TestLogHandler::new().exists_log_containing(
            "WARN: partial_close_test: UI at 1.2.3.7:57777 had a partial client registry during disconnect; completing idempotent cleanup for client ID 9",
        );
    }
