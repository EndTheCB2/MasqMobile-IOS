    ...
    fn deserialization_validator_with_crash_request_handler(
        ...
        match &message_body.payload {
            Ok(payload) => match serde_json::from_str::<UiCrashRequest>(payload) {
                ...
                Err(e) if e.is_syntax() => Some((e.to_string(), payload.len())),
            ...
            Err((code, _)) => {
                error!(
                    self.logger,
                    "Received a message of '{}' opcode with error payload code {}; contents redacted",
                    message_body.opcode,
                    code
                );
                None
...
impl Handler<NodeFromUiMessage> for UiGateway {
    ...
    fn handle(&mut self, msg: NodeFromUiMessage, _ctx: &mut Self::Context) -> Self::Result {
        ...
        if let Some((error, payload_len)) =
            self.deserialization_validator_with_crash_request_handler(msg.body.clone())
        {
            warning!(
                self.logger,
                "Deserialization error: {}; payload contents redacted ({} bytes)",
                error,
                payload_len
            );
            return;
...
mod tests {
    ...
    #[test]
    fn syntactically_bad_json_is_caught_without_logging_payload_contents() {
        init_test_logging();
        let (accountant, _, accountant_recording_arc) = make_recorder();
        let subject = UiGateway::new(
            &UiGatewayConfig {
        ...
        let payload = format!("some bad JSON containing {}", secret);
        ...
        let expected_log = format!("WARN: UiGateway: Deserialization error: expected value at line 1 column 1; payload contents redacted ({} bytes)", payload_len);
        log_handler.exists_log_containing(&expected_log);
        log_handler.exists_no_log_containing(secret)
    }
    ...
    fn deserialization_validator_logs_payload_errors() {
        ...
        let msg_body = MessageBody {
            ...
            payload: Err((1234, "SENSITIVE_VALUE_SHOULD_NOT_LEAK".to_string())),
        };
        let subject = UiGateway::new(&UiGatewayConfig { ui_port: 123 }, false);
        ...
        assert_eq!(result, None);
        let log_handler = TestLogHandler::new();
        log_handler.exists_log_containing("ERROR: UiGateway: Received a message of 'whatever' opcode with error payload code 1234; contents redacted");
        log_handler.exists_no_log_containing("SENSITIVE_VALUE_SHOULD_NOT_LEAK");
    }
