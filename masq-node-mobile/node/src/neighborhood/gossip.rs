...
impl TryFrom<&Value> for GossipNodeRecord {
    ...
    fn try_from(value: &Value) -> Result<Self, Self::Error> {
        ...
        match serde_cbor::de::from_slice::<Self>(&reserialized) {
            Ok(gnr) => Ok(gnr),
            Err(e) => Err(StepError::SemanticError(format!(
                "Invalid GossipNodeRecord: {:?}",
                e
            ))),
        }
    }
...
impl Gossip_0v1 {
        ...
        T: Into<DotGossipEndpoint>,
    {
        ...
        let inners_and_addrs: Vec<(NodeRecordInner_0v1, Option<NodeAddr>)> = self
            ...
            .iter()
            .filter_map(|gnr| {
                NodeRecordInner_0v1::try_from(gnr)
                    .ok()
                    .map(|nri| (nri, gnr.node_addr_opt.clone()))
            })
            .collect();
...
mod tests {
    ...
    #[test]
    fn gossip_node_record_rejects_invalid_value_without_panicking() {
        let result = GossipNodeRecord::try_from(&Value::Integer(42));
...
00f0:   64 65 5f 6f  70 74 62 41  55 75 70 72  6f 74 6f 63   de_optbAUuprotoc
0100:   6f 6c 5f 63  61 70 61 62  69 6c 69 74  69 65 73 00   ol_capabilities.
0110:   78 1f 72 65  63 65 69 70  74 5f 73 65  74 74 6c 65   x.receipt_settle
0120:   6d 65 6e 74  5f 63 6f 6e  74 72 61 63  74 5f 6f 70   ment_contract_op
0130:   74 f6                                                t.",
	        "\n\tsignature:
Length: 24 (0x18) bytes
0000:   01 02 03 04  d2 55 c9 e0  92 ef 47 ee  2b d7 8b 4d   .....U....G.+..M
0010:   d6 3d a3 7d  f2 7c c4 de                             .=.}.|.."
        );

        assert_eq!(result, expected);
        assert!(!result.contains("Address(0x546900db8d6e0937497133d1ae6fdf5f4b75bcd0)"));
    }
...
0000:   04 03 02 01                                          ....",
    ...
    #[test]
    fn to_dot_graph_omits_non_deserializable_peer_records_without_panicking() {
        let valid_node = make_node_record_cc(1234, true, "AU");
        let db = db_from_node(&valid_node);
        let malformed_record = GossipNodeRecord {
            signed_data: PlainData::new(&[1, 2, 3, 4]),
            signature: CryptData::new(&[4, 3, 2, 1]),
            node_addr_opt: None,
        };
        let gossip = Gossip_0v1::new(vec![
            GossipNodeRecord::from((&db, valid_node.public_key(), true)),
            malformed_record,
        ]);
        ...
        assert_string_contains(&result, "AQIDBA");
        assert_string_contains(&result, "\"src\" -> \"dest\" [arrowhead=empty]");
        assert!(!result.contains("non-deserializable"));
    }
