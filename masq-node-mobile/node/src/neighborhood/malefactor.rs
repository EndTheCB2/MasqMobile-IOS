...
impl Display for Malefactor {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "Malefactor {{ public_key_present: {}, ip_address_present: {}, earning_wallet_present: {}, consuming_wallet_present: {}, timestamp: [REDACTED], reason: [REDACTED] }}",
            self.public_key_opt.is_some(),
            self.ip_address_opt.is_some(),
            self.earning_wallet_opt.is_some(),
            self.consuming_wallet_opt.is_some()
        )
    }
}

impl Debug for Malefactor {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        Display::fmt(self, f)
    }
}
...
impl PartialEq for Malefactor {
    ...
    fn eq(&self, other: &Self) -> bool {
        ...
        let plus_fudge_factor = self.timestamp.saturating_add(*FUDGE_FACTOR);
        let minus_fudge_factor = self.timestamp.saturating_sub(*FUDGE_FACTOR);
        equal && (other.timestamp >= minus_fudge_factor && other.timestamp <= plus_fudge_factor)
    }
}
...
mod tests {
    ...
    fn displays_public_key() {
        ...
        assert_eq!(
            string,
            "Malefactor { public_key_present: true, ip_address_present: false, earning_wallet_present: false, consuming_wallet_present: false, timestamp: [REDACTED], reason: [REDACTED] }".to_string()
        );
    }
    ...
    fn displays_ip_address() {
        ...
        assert_eq!(
            string,
            "Malefactor { public_key_present: false, ip_address_present: true, earning_wallet_present: false, consuming_wallet_present: false, timestamp: [REDACTED], reason: [REDACTED] }".to_string()
        );
    }
    ...
    fn displays_earning_wallet() {
        ...
        assert_eq!(
            string,
            "Malefactor { public_key_present: false, ip_address_present: false, earning_wallet_present: true, consuming_wallet_present: false, timestamp: [REDACTED], reason: [REDACTED] }".to_string()
        );
    }
    ...
    fn displays_consuming_wallet() {
        ...
        assert_eq!(
            string,
            "Malefactor { public_key_present: false, ip_address_present: false, earning_wallet_present: false, consuming_wallet_present: true, timestamp: [REDACTED], reason: [REDACTED] }".to_string()
        );
    }
    ...
    fn displays_all_fields() {
        ...
        assert_eq!(
            string,
            "Malefactor { public_key_present: true, ip_address_present: true, earning_wallet_present: true, consuming_wallet_present: true, timestamp: [REDACTED], reason: [REDACTED] }".to_string()
        );
        assert_eq!(format!("{:?}", malefactor), string);
        assert!(!string.contains("Qm9vZ2E"));
        assert!(!string.contains("12.34.56.78"));
        assert!(!string.contains("Bad Smell"));
        assert!(!string.contains("2024-06-01"));
    }
