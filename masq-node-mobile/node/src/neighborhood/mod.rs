...
use crate::neighborhood::gossip::{AccessibleGossipRecord, Gossip_0v1};
...
use masq_lib::messages::{
    ExitLocation, FromMessageBody, ToMessageBody, UiConnectionStage, UiConnectionStatusReason,
    UiConnectionStatusRequest, UiGetNeighborhoodGraphRequest, UiGetNeighborhoodGraphResponse,
...
pub const MAX_ROUTE_LATENCY_SAMPLE_MS: u32 = 60_000;
pub const ROUTE_LATENCY_UNDESIRABILITY_PER_MS: i64 = 1_000;
const ROUTE_LATENCY_EWMA_OLD_WEIGHT: u64 = 3;
const ROUTE_LATENCY_EWMA_TOTAL_WEIGHT: u64 = 4;
pub const ZZ_COUNTRY_CODE_STRING: &str = "ZZ";
pub const DEFAULT_PREALLOCATION_VEC: usize = 10;
pub const CONSECUTIVE_ROUTE_FAILURES_BEFORE_DEMOTION: usize = 3;
pub const ROUTE_READINESS_TTL: Duration = Duration::from_secs(5 * 60);
...
pub struct Neighborhood {
    ...
    topology_route_available: bool,
    accountant_started: bool,
    consecutive_route_failures: usize,
    route_readiness_generation: u64,
    route_readiness_ttl: Duration,
    route_readiness_proof_is_stale: bool,
    route_readiness_expiration_handle_opt: Option<SpawnHandle>,
    chain: Chain,
    crashable: bool,
...
impl Neighborhood {
    fn apply_route_readiness_lease_renewal(&mut self, ctx: &mut Context<Self>) {
        ...
        if let Some(expiration_handle) = self.route_readiness_expiration_handle_opt.take() {
            let _ = ctx.cancel_future(expiration_handle);
        }
        self.route_readiness_generation = self.route_readiness_generation.wrapping_add(1);
        let route_readiness_generation = self.route_readiness_generation;
        let route_readiness_ttl = self.route_readiness_ttl;
        self.start_accountant_if_needed();
        ...
        let expiration_handle = ctx.run_later(route_readiness_ttl, move |actor, _ctx| {
            actor.route_readiness_expiration_handle_opt = None;
...
impl Handler<RouteUseFailedMessage> for Neighborhood {
    type Result = ();

    fn handle(&mut self, _msg: RouteUseFailedMessage, _ctx: &mut Self::Context) -> Self::Result {
        self.consecutive_route_failures = self.consecutive_route_failures.saturating_add(1);
        if self.consecutive_route_failures < CONSECUTIVE_ROUTE_FAILURES_BEFORE_DEMOTION
            || self.overall_connection_status.stage() != OverallConnectionStage::RouteFound
        {
            return;
        }
        self.topology_route_available = false;
        self.route_readiness_proof_is_stale = false;
        let fallback_stage = if self
            .neighborhood_database
            .root()
            .full_neighbor_keys(&self.neighborhood_database)
            .is_empty()
...
impl Handler<ExpiredCoresPackage<GossipFailure_0v1>> for Neighborhood {
    ...
    fn handle(
        ...
        if let Some(retry_peer) = self.handle_gossip_failure(msg.immediate_neighbor, msg.payload) {
            if self.overall_connection_status.stage() == OverallConnectionStage::NotConnected {
                self.send_ask_about_debut_gossip_message(ctx, retry_peer);
            }
        }
    }
}
...
impl Handler<RemoveNeighborMessage> for Neighborhood {
    ...
    fn handle(&mut self, msg: RemoveNeighborMessage, _ctx: &mut Self::Context) -> Self::Result {
        ...
        match self.neighborhood_database.remove_neighbor(public_key) {
            Err(_) => error!(
                self.logger,
                "Could not remove requested neighbor; identity and details redacted"
            ),
            Ok(db_changed) => {
                if db_changed {
                    self.invalidate_route_after_neighbor_loss();
                    self.gossip_to_neighbors();
                    info!(
                        self.logger,
                        "Removed requested neighbor; public key redacted"
                    )
                }
            }
        }
    ...
    fn handle(&mut self, msg: ConnectionProgressMessage, ctx: &mut Self::Context) -> Self::Result {
            ...
            .get_connection_progress_to_modify(&msg)
        {
            ...
            Err(_) => {
                trace!(
                    self.logger,
                    "Found unnecessary connection progress message; peer details redacted"
                );
            }
        }
    }
    ...
    fn handle(&mut self, msg: AskAboutDebutGossipMessage, ctx: &mut Self::Context) -> Self::Result {
        ...
        let retry_target_in_use = node_descriptor
            ...
            .map(|node_addr| {
                self.overall_connection_status
                    ...
                    .any(|connection_progress| {
                        connection_progress.initial_node_descriptor != *node_descriptor
                            && connection_progress.current_peer_addr == node_addr.ip_addr()
                    })
            })
            .unwrap_or(false);
        let mut schedule_retry_for_peer = None;
        let mut retry_descriptor = None;
            ...
            .get_connection_progress_by_desc(node_descriptor)
        {
            if msg.prev_connection_progress == *current_connection_progress {
                if matches!(
                    current_connection_progress.connection_stage,
                    ConnectionStage::TcpConnectionEstablished
                ) {
                    // No change, hence no response was received. Schedule a separate retry tick
                    // whose snapshot must still match this new terminal state.
                    if OverallConnectionStatus::update_connection_stage(
                ...
                ) && connection_is_not_established
                {
                    if all_entry_attempts_were_terminal {
                        ...
                        schedule_retry_for_peer =
                            Some(current_connection_progress.current_peer_addr)
                    } else if current_connection_progress.retry_initial_node(&self.logger) {
                        retry_descriptor =
                            Some(current_connection_progress.initial_node_descriptor.clone())
                    }
                }
            }
        } else {
            trace!(
                self.logger,
                "Received an AskAboutDebutGossipMessage for an unknown node descriptor; identity redacted - ignoring"
            )
        }
        ...
        if let Some(node_descriptor) = retry_descriptor {
            let gossip = self
                .gossip_producer
...
impl Handler<UpdateNodeRecordMetadataMessage> for Neighborhood {
    ...
    fn handle(
        ...
        let public_key = msg.public_key;
        let node_record = match self.neighborhood_database.node_by_key_mut(&public_key) {
            Some(node_record) => node_record,
            None => {
                warning!(
                    self.logger,
                    "Ignoring metadata update for unknown Node; identity redacted"
                );
                return;
            }
        };
        match msg.metadata_change {
            NRMetadataChange::AddUnreachableHost { hostname } => {
                debug!(
                    self.logger,
                    "Marking a destination host unreachable for a Node; identities redacted"
                );
                node_record.metadata.unreachable_hosts.insert(hostname);
            }
            NRMetadataChange::RemoveUnreachableHost { hostname } => {
                if node_record.metadata.unreachable_hosts.remove(&hostname) {
                    debug!(
                        self.logger,
                        "Clearing a recovered destination host for a Node; identities redacted"
                    );
                }
            }
            NRMetadataChange::RecordRouteSuccess {
                ...
                let bounded_sample = latency_ms.min(MAX_ROUTE_LATENCY_SAMPLE_MS);
                node_record.metadata.route_latency_ewma_ms_opt = Some(
                    node_record
                        .metadata
                        .route_latency_ewma_ms_opt
                        .map(|previous| {
                            ((previous as u64 * ROUTE_LATENCY_EWMA_OLD_WEIGHT
                                + bounded_sample as u64)
                                / ROUTE_LATENCY_EWMA_TOTAL_WEIGHT)
                                as u32
                        })
                        .unwrap_or(bounded_sample),
                );
            }
        }
    }
...
impl Neighborhood {
    pub fn new(cryptde_pair: CryptDEPair, config: &BootstrapperConfig) -> Self {
        ...
        let receipt_settlement_contract_opt = neighborhood_mode
            .is_decentralized()
            .then(|| {
                config
                    .blockchain_bridge_config
                    .receipt_settlement_contract_opt
            })
            .flatten();
        let root = neighborhood_database.root_mut();
        root.inner.protocol_capabilities = if receipt_settlement_contract_opt.is_some() {
            crate::sub_lib::service_receipt::CURRENT_PROTOCOL_CAPABILITIES
        } else {
            0
        };
        root.inner.receipt_settlement_contract_opt = receipt_settlement_contract_opt;
        root.regenerate_signed_gossip(cryptde_pair.main.as_ref());
        ...
        Neighborhood {
            ...
            db_patch_size,
            overall_connection_status,
            topology_route_available: false,
            accountant_started: false,
            consecutive_route_failures: 0,
            route_readiness_generation: 0,
            route_readiness_ttl: ROUTE_READINESS_TTL,
            route_readiness_proof_is_stale: false,
            route_readiness_expiration_handle_opt: None,
    ...
    fn handle_start_message(&mut self) {
        ...
        if let Some(entry_nodes) = crate::mobile_runtime::take_pending_entry_nodes() {
            self.replace_initial_neighbors_for_mobile_retry(entry_nodes);
        } else if self.overall_connection_status.stage() == OverallConnectionStage::NotConnected {
            self.overall_connection_status
                .progress
                .iter_mut()
                .for_each(|progress| progress.reset_initial_node_for_user_retry(&self.logger));
        }
        self.send_debut_gossip_to_all_initial_descriptors();
    }

    fn replace_initial_neighbors_for_mobile_retry(&mut self, entry_nodes: Vec<String>) {
        let parsed = entry_nodes
            .iter()
            .map(|descriptor| {
                NodeDescriptor::try_from((self.cryptde.as_ref(), descriptor.as_str()))
            })
            .collect::<Result<Vec<_>, _>>();
        let descriptors = match parsed {
            Ok(descriptors)
                if !descriptors.is_empty()
                    && descriptors
                        .iter()
                        .all(|descriptor| descriptor.blockchain == self.chain) =>
            {
                descriptors
            }
            _ => {
                warning!(
                    self.logger,
                    "Ignoring invalid refreshed entry-node descriptors; identities redacted"
                );
                crate::mobile_runtime::report_connection_error(
                    crate::mobile_runtime::MobileConnectionError::StreamConnectionFailed,
                );
                return;
            }
        };

        let previous_stage = self.overall_connection_status.stage();
        self.overall_connection_status = OverallConnectionStatus::new(descriptors);
        self.overall_connection_status.stage = previous_stage;
        self.topology_route_available = false;
        self.route_readiness_proof_is_stale = false;
        self.consecutive_route_failures = 0;
        ...
        debug!(
            self.logger,
            "Replaced initial Nodes for an explicit mobile retry; identities redacted"
        );
    }

    fn set_mobile_connection_stage(&mut self, stage: OverallConnectionStage) {
        if let Some(node_to_ui_recipient) = self.node_to_ui_recipient_opt.as_ref() {
            self.overall_connection_status
                .update_ocs_stage_and_send_message_to_ui(stage, node_to_ui_recipient, &self.logger);
        } else {
            self.overall_connection_status.stage = stage;
            crate::mobile_runtime::report_route_stage(stage as u8);
        }
    }
    ...
    fn handle_new_public_ip(&mut self, msg: NewPublicIp) {
        ...
        info!(
            self.logger,
            "Changed public IP; old and new addresses redacted"
        );
    }
    ...
    fn handle_route_query_message(&mut self, msg: RouteQueryMessage) -> Option<RouteQueryResponse> {
        let debug_msg_opt = self
            .logger
            .debug_enabled()
            .then(|| "destination host [REDACTED]".to_string());
        ...
        match route_result {
            ...
            Err(_msg) => {
                error!(self.logger, "Unsatisfied route query; details redacted");
                None
            }
        }
    }
        ...
        match msg.change {
            ConfigChange::UpdateWallets(wallet_pair) => {
                if self.consuming_wallet_opt != Some(wallet_pair.consuming_wallet.clone()) {
                    info!(self.logger, "Consuming Wallet has been updated");
    ...
    fn send_debut_gossip_to_descriptor(
            ...
            .is_err()
        {
            error!(
                self.logger,
                "Could not send Debut Gossip; destination and error details redacted"
            );
            return;
        }
        debug!(self.logger, "Debut Gossip sent; destination redacted");
        trace!(
            self.logger,
            "Sent Debut Gossip with {} record(s); topology redacted",
            debut_gossip.node_records.len()
        )
    }

    fn log_incoming_gossip(&self, incoming_gossip: &Gossip_0v1, _gossip_source: SocketAddr) {
        trace!(
            self.logger,
            "Received Gossip with {} record(s); source and topology redacted",
    ...
    fn handle_gossip(
        ...
        if agrs.len() < record_count {
            ...
            error!(
                self.logger,
                "Received non-deserializable Gossip; source redacted"
            );
        ...
        if agrs.iter().any(signature_invalid) {
            ...
            error!(
                self.logger,
                "Received Gossip with invalid signature; source redacted"
            );
            self.announce_gossip_handling_completion(record_count);
            return;
        }
    ...
    fn handle_gossip_failure(
        &mut self,
        failure_source: SocketAddr,
        failure: GossipFailure_0v1,
    ) -> Option<IpAddr> {
        let position = match self
            .overall_connection_status
            .get_peer_addrs()
            .iter()
            .position(|peer_addr| *peer_addr == failure_source.ip())
        {
            None => {
                warning!(
                    self.logger,
                    "Ignoring GossipFailure from unknown Node; source and reason redacted"
                );
                return None;
            }
        ...
        warning!(
            self.logger,
            "A Node refused Debut: {}; identity redacted",
            failure
        );
        let refusal_is_retryable = !matches!(failure, GossipFailure_0v1::ManualRejection);
        ...
        if !refusal_is_retryable {
            info!(self.logger, "Not retrying Node because its owner manually rejected the Debut; identity redacted.");
            return None;
        }
        Some(failure_source.ip())
    }
    ...
    fn handle_agrs(
        ...
        if acceptance_results.is_empty() {
            trace!(self.logger, "Gossip ignored; source redacted");
            ...
            acceptance_results.into_iter().for_each(|acceptance_result| {
                match acceptance_result {
                    ...
                    GossipAcceptanceResult::Ban(_reason) => {
                        ...
                        warning!(self.logger, "Malefactor detected, but malefactor bans are not yet implemented; identity and reason redacted - ignoring");
                        self.handle_gossip_ignored(&ignored_node_name, gossip_record_count);
                    }
                }
            });
    ...
    fn curate_past_neighbors(&mut self, neighbor_keys: &HashSet<PublicKey>) {
        if let Some(db_password) = &self.db_password_opt {
            ...
            debug!(
                self.logger,
                "Saving neighbor list with {} descriptor(s); identities redacted",
                ...
                .set_past_neighbors(node_descriptors_opt, db_password)
            {
                ...
                Err(_) => error!(
                    self.logger,
                    "Could not persist immediate-neighbor changes; error details redacted"
                ),
            };
        } else {
            info!(
    ...
    fn check_connectedness(&mut self) {
        if !self.topology_route_available {
            self.search_for_a_new_route();
        }
    }

    fn invalidate_route_after_neighbor_loss(&mut self) {
        self.topology_route_available = false;
        self.route_readiness_proof_is_stale = false;
        let fallback_stage = if self
            .neighborhood_database
        ...
        if self.overall_connection_status.stage() == OverallConnectionStage::RouteFound
            || fallback_stage == OverallConnectionStage::NotConnected
        {
            if let Some(node_to_ui_recipient) = self.node_to_ui_recipient_opt.as_ref() {
                self.overall_connection_status
                    .update_ocs_stage_and_send_message_to_ui(
                ...
                self.overall_connection_status.stage = fallback_stage;
            }
        }
        if self.connected_signal_opt.is_some() && self.node_to_ui_recipient_opt.is_some() {
            self.check_connectedness();
        }
    }

    fn start_accountant_if_needed(&mut self) {
        if self.accountant_started {
            return;
        }
        ...
        self.accountant_started = true;
    }

    fn expire_route_readiness_lease(&mut self, route_readiness_generation: u64) -> bool {
        if route_readiness_generation != self.route_readiness_generation
            || self.overall_connection_status.stage() != OverallConnectionStage::RouteFound
        {
            return false;
        }
        ...
        self.route_readiness_proof_is_stale = true;
        if let Some(node_to_ui_recipient) = self.node_to_ui_recipient_opt.as_ref() {
            self.overall_connection_status
                .update_ocs_stage_and_send_message_to_ui(
                    fallback_stage,
        ...
        debug!(
            self.logger,
            "End-to-end route-readiness proof expired after {} seconds; retaining the known topology and awaiting fresh correlated response data.",
            self.route_readiness_ttl.as_secs()
        );
        true
    }
    ...
    fn search_for_a_new_route(&mut self) {
        ...
        if self.handle_route_query_message(msg).is_some() {
            ...
            debug!(
                &self.logger,
                "The topology check has found a {}-hop route; awaiting correlated response data before reporting RouteFound.",
            ...
            debug!(
                &self.logger,
                "The topology check still can't find a good route."
            );
        }
    }
    ...
    fn gossip_to_neighbor(&self, neighbor: &PublicKey, gossip: Gossip_0v1) {
        ...
        let route = match self.create_single_hop_route(neighbor) {
            Ok(route) => route,
            Err(_) => {
                error!(
                    self.logger,
                    "Could not create Gossip route; destination and error details redacted - skipping"
                );
                return;
            }
        };
        let package = match IncipientCoresPackage::new(
            ...
            Err(_) => {
                error!(
                    self.logger,
                    "Could not encrypt Gossip; destination and error details redacted - skipping"
                );
                return;
            }
        };
        info!(
            self.logger,
            "Sending update Gossip about {} Nodes; destination redacted", gossip_len
        );
        let hopper = match self.hopper_opt.as_ref() {
            Some(hopper) => hopper,
            None => {
                error!(
                    self.logger,
                    "Could not send Gossip: Hopper is unbound; destination redacted - skipping"
                );
                return;
            }
        };
        if hopper.try_send(package).is_err() {
            error!(
                self.logger,
                "Could not send Gossip: Hopper is dead; destination redacted - skipping"
            );
            return;
        }
        trace!(
            self.logger,
            "Sent update Gossip about {} Nodes; topology redacted",
            gossip_len
        );
    }

    fn create_single_hop_route(&self, destination: &PublicKey) -> Result<Route, String> {
        Route::one_way(
            RouteSegment::new(
    ...
    fn make_round_trip_route(
        ...
        let minimum_protocol_capabilities = request_msg.minimum_protocol_capabilities;
        let routing_receipt_request_opt = request_msg.routing_receipt_request_opt;
        ...
        let anticipated_response_payload_len =
            Self::anticipated_response_payload_len(request_msg.payload_size);
        ...
        self.compose_route_query_response_with_receipts(
            over,
            back,
            host,
    ...
    fn anticipated_response_payload_len(request_payload_len: usize) -> usize {
        request_payload_len.saturating_mul(RESPONSE_UNDESIRABILITY_FACTOR)
    }
    ...
    fn compose_route_query_response(
        ...
        self.compose_route_query_response_with_receipts(over, back, host, None)
    }

    fn compose_route_query_response_with_receipts(
        &mut self,
        over: RouteSegment,
        back: RouteSegment,
        host: Host,
        routing_receipt_request_opt: Option<crate::sub_lib::service_receipt::ReceiptSessionRequest>,
    ) -> Result<RouteQueryResponse, String> {
        ...
        Ok(RouteQueryResponse {
            route: Route::round_trip_with_receipts(
                over,
                back,
                self.cryptde.as_ref(),
    ...
    fn make_route_segment(
        ...
        hostname: &str,
        minimum_protocol_capabilities: u64,
        ...
        match route_opt {
            None => Err(format!(
                "Couldn't find any routes: at least {}-hop to {:?}; node identities redacted",
                minimum_hop_count, target_component
            )),
    ...
    fn compute_undesirability(
        ...
        match undesirability_type {
            UndesirabilityType::Relay => Self::charge_to_undesirability(
                node_record.inner.rate_pack.routing_charge(payload_size),
            ),
            UndesirabilityType::ExitRequest(hostname) => {
                let exit_undesirability = Self::charge_to_undesirability(
                    ...
                    .contains(hostname)
                {
                    trace!(
                        logger,
                        "An exit Node failed to reach destination host [REDACTED] during ExitRequest; identity redacted; Undesirability: {} + {} + {} = {}",
                        exit_undesirability,
                        UNREACHABLE_HOST_PENALTY,
                        country_undesirability,
                        exit_undesirability
                            .saturating_add(UNREACHABLE_HOST_PENALTY)
                            .saturating_add(country_undesirability)
                    );
                    UNREACHABLE_HOST_PENALTY
                } else {
                    0i64
                ...
                exit_undesirability
                    .saturating_add(unreachable_host_undesirability)
                    .saturating_add(country_undesirability)
                    .saturating_add(Self::route_latency_undesirability(node_record))
            }
            UndesirabilityType::ExitAndRouteResponse => {
                let total_charge = node_record
                    .inner
                    .rate_pack
                    .exit_charge(payload_size)
                    .saturating_add(node_record.inner.rate_pack.routing_charge(payload_size));
                Self::charge_to_undesirability(total_charge)
                    .saturating_add(Self::route_latency_undesirability(node_record))
            }
        }
    }

    fn route_latency_undesirability(node_record: &NodeRecord) -> i64 {
        node_record
            .metadata
            .route_latency_ewma_ms_opt
            .map(|latency_ms| {
                i64::from(latency_ms).saturating_mul(ROUTE_LATENCY_UNDESIRABILITY_PER_MS)
            })
            .unwrap_or(0)
    }

    fn charge_to_undesirability(charge: u64) -> i64 {
    ...
    fn find_best_route_segment<'a>(
        ...
        prefix.push(source);
        let computed_routes = self.routing_engine(
            vec![source],
            initial_undesirability,
            target_opt,
            minimum_hops,
        ...
        Self::choose_uniform_minimum_route(
            computed_routes,
    ...
    fn choose_uniform_minimum_route<'a, R: rand::Rng + ?Sized>(
        computed_routes: Vec<ComputedRouteSegment<'a>>,
        minimum_undesirability: i64,
        rng: &mut R,
    ) -> Option<Vec<&'a PublicKey>> {
        computed_routes
            .into_iter()
            .filter(|route| route.undesirability == minimum_undesirability)
            .choose(rng)
    ...
    fn routing_guts<'a>(
        ...
        let receipt_capability_required = minimum_protocol_capabilities
            & crate::sub_lib::service_receipt::SERVICE_RECEIPT_SETTLEMENT_V1_CAPABILITY
            != 0;
        let required_settlement_contract_opt = self
            .neighborhood_database
            .root()
            .inner
            .receipt_settlement_contract_opt;
        ...
        previous_node
            ...
            .filter(|node_record| {
                let capabilities_match = node_record.inner.protocol_capabilities
                    & minimum_protocol_capabilities
                    == minimum_protocol_capabilities;
                let settlement_contract_matches = if receipt_capability_required {
                    required_settlement_contract_opt
                        .map(|required| {
                            node_record.inner.receipt_settlement_contract_opt == Some(required)
                        })
                        .unwrap_or(false)
                } else {
                    true
                };
                capabilities_match && settlement_contract_matches
            })
    ...
    fn compute_new_undesirability(
        ...
        undesirability.saturating_add(node_undesirability)
    }
    ...
    fn create_exit_location_response(
        ...
        crate::mobile_runtime::report_available_exit_countries(
            self.user_exit_preferences.db_countries.clone(),
        );
    ...
    fn handle_gossip_reply(
            ...
            .is_err()
        {
            error!(
                self.logger,
                "Could not send Gossip reply; destination and error details redacted"
            );
            return;
        }
        trace!(
            self.logger,
            "Sent Gossip reply with {} record(s); topology redacted",
    ...
    fn handle_gossip_failed(
            ...
            .is_err()
        {
            error!(
                self.logger,
                "Could not send Gossip failure; destination and error details redacted"
            );
            return;
        }
        trace!(
            self.logger,
            "Sent Gossip failure; destination and reason redacted"
        );
    }
    ...
    fn send_no_lookup_package(
        ...
        .map_err(|error| format!("Could not create no-lookup package: {}", error))?;
        self.hopper_no_lookup_opt
            .as_ref()
            .ok_or_else(|| "No-lookup Hopper is unbound".to_string())?
            .try_send(package)
            .map_err(|_| "No-lookup Hopper is unavailable".to_string())
    }
    ...
    fn handle_stream_shutdown_msg(&mut self, msg: StreamShutdownMsg) {
        ...
        let initial_connection_closed = {
            let progress = self
                .overall_connection_status
                .get_connection_progress_by_ip(msg.peer_addr.ip());
            match progress {
                Ok(progress) => {
        ...
        if initial_connection_closed {
            crate::mobile_runtime::report_connection_error(
                crate::mobile_runtime::MobileConnectionError::StreamConnectionFailed,
            );
            let another_initial_connection_is_active = self
                .overall_connection_status
                .progress
                .iter()
                .any(|progress| {
        ...
        let neighbor_key = match self.neighborhood_database.node_by_ip(&msg.peer_addr.ip()) {
            None => {
                warning!(self.logger, "Received shutdown notification for an unknown peer; identity redacted - ignoring");
                return;
            }
    ...
    fn handle_connection_status_message(&self, client_id: u64, context_id: u64) {
        ...
        let reason = if self.route_readiness_proof_is_stale {
            Some(UiConnectionStatusReason::RouteProofStale)
        } else {
            self.overall_connection_status.ui_connection_status_reason()
        };
        let message = NodeToUiMessage {
            target: MessageTarget::ClientId(client_id),
            body: UiConnectionStatusResponse { stage, reason }.tmb(context_id),
        };
    ...
    fn remove_neighbor(&mut self, neighbor_key: &PublicKey, _peer_addr: &SocketAddr) {
        match self.neighborhood_database.remove_neighbor(neighbor_key) {
            Err(e) => panic!("Node suddenly disappeared: {:?}", e),
            Ok(true) => {
                debug!(self.logger, "Received shutdown notification for a known peer: removing neighborship; identity redacted");
                self.invalidate_route_after_neighbor_loss();
                self.gossip_to_neighbors()
            }
            Ok(false) => {
                debug!(self.logger, "Received shutdown notification for a known non-neighbor; identity redacted - ignoring");
            }
        };
    }
...
mod tests {
    ...
    use crate::neighborhood::overall_connection_status::ConnectionStageErrors::{
        DebutRejected, NoGossipResponseReceived, PassLoopFound, TcpConnectionFailed,
    };
    use crate::neighborhood::overall_connection_status::{
    ...
    use crate::sub_lib::neighborhood::{
        AskAboutDebutGossipMessage, ConfigChange, ConfigChangeMsg, NeighborhoodMode,
        RatePackLimits, WalletPair,
    };
    ...
    fn constants_have_correct_values() {
        ...
        assert_eq!(MAX_ROUTE_LATENCY_SAMPLE_MS, 60_000u32);
        assert_eq!(ROUTE_LATENCY_UNDESIRABILITY_PER_MS, 1_000i64);
    ...
    #[test]
    fn receipt_capabilities_require_an_explicit_settlement_contract() {
        use crate::sub_lib::service_receipt::CURRENT_PROTOCOL_CAPABILITIES;
        use ethereum_types::Address;

        let neighborhood_config = NeighborhoodConfig {
            mode: NeighborhoodMode::Standard(
                NodeAddr::new(&make_ip(1), &[1234]),
                vec![],
                rate_pack(100),
            ),
            min_hops: Hops::ThreeHops,
        };
        let mut config = bc_from_nc_plus(
            neighborhood_config,
            make_wallet("earning"),
            None,
            "receipt_capabilities_require_an_explicit_settlement_contract",
        );

        let disabled = Neighborhood::new(N_CRYPTDE_PAIR.clone(), &config);
        assert_eq!(
            disabled
                .neighborhood_database
                .root()
        ...
        config
            .blockchain_bridge_config
            .receipt_settlement_contract_opt = Some(Address::from([0x77; 20]));
        let enabled = Neighborhood::new(N_CRYPTDE_PAIR.clone(), &config);
        assert_eq!(
            enabled
                .neighborhood_database
                .root()
    ...
    fn neighborhood_logs_with_trace_if_it_receives_a_cpm_with_an_unknown_peer_addr() {
        ...
        TestLogHandler::new().exists_log_containing(
            "TRACE: Neighborhood: Found unnecessary connection progress message; peer details redacted",
        );
    }
    ...
    pub fn neighborhood_handles_connection_progress_message_with_tcp_connection_established() {
        ...
        cpm_recipient
            .try_send(connection_progress_message.clone())
            .unwrap();
    ...
    fn ask_about_debut_gossip_message_handles_timeout_in_case_no_response_is_received() {
        ...
        let notify_later_params = Arc::new(Mutex::new(vec![]));
        subject.tools.notify_later_ask_about_gossip =
            Box::new(NotifyLaterHandleMock::default().notify_later_params(&notify_later_params));
        subject.tools.ask_about_gossip_interval = Duration::from_millis(10);
        ...
        let node_descriptor_for_actor = node_descriptor.clone();
        let assertions = Box::new(move |actor: &mut Neighborhood| {
            assert_eq!(
                actor.overall_connection_status,
                OverallConnectionStatus {
                    ...
                    progress: vec![ConnectionProgress {
                        initial_node_descriptor: node_descriptor_for_actor,
                        current_peer_addr: node_ip_addr,
        ...
        assert_eq!(system.run(), 0);
        assert_eq!(
            *notify_later_params.lock().unwrap(),
            vec![(
                AskAboutDebutGossipMessage {
    ...
    #[test]
    fn failed_connection_retry_resets_to_and_contacts_the_initial_node() {
        let (initial_peer, node_descriptor) = make_node(1);
        let passed_peer = make_ip(2);
        let mut subject = make_subject_from_node_descriptor(
            &node_descriptor,
            "failed_connection_retry_resets_to_and_contacts_the_initial_node",
        );
        let failed_progress = ConnectionProgress {
            initial_node_descriptor: node_descriptor.clone(),
            current_peer_addr: passed_peer,
            connection_stage: ConnectionStage::Failed(NoGossipResponseReceived),
        };
        subject.overall_connection_status.progress[0] = failed_progress.clone();
        let (hopper, _, hopper_recording) = make_recorder();
        let system = System::new("failed_connection_retry");
        let addr = subject.start();
        addr.try_send(BindMessage {
            peer_actors: peer_actors_builder().hopper(hopper).build(),
        })
        .unwrap();
        addr.try_send(AskAboutDebutGossipMessage {
            prev_connection_progress: failed_progress,
        })
        .unwrap();

        let assertions = Box::new(move |actor: &mut Neighborhood| {
            assert_eq!(
                actor.overall_connection_status.progress[0].connection_stage,
                ConnectionStage::StageZero
            );
            assert_eq!(
                actor.overall_connection_status.progress[0].current_peer_addr,
                initial_peer
            );
        });
        addr.try_send(AssertionsMessage { assertions }).unwrap();
        ...
        assert_eq!(
            recording
                .get_record::<NoLookupIncipientCoresPackage>(0)
                .node_addr,
            node_descriptor.node_addr_opt.unwrap()
        );
    }

    #[test]
    fn failed_connection_retry_is_suppressed_after_another_peer_connects() {
        let (_initial_peer, node_descriptor) = make_node(1);
        let mut subject = make_subject_from_node_descriptor(
            &node_descriptor,
            "failed_connection_retry_is_suppressed_after_another_peer_connects",
        );
        let failed_progress = ConnectionProgress {
            initial_node_descriptor: node_descriptor,
            current_peer_addr: make_ip(2),
            connection_stage: ConnectionStage::Failed(TcpConnectionFailed),
        };
        subject.overall_connection_status.progress[0] = failed_progress.clone();
        subject.overall_connection_status.stage = OverallConnectionStage::ConnectedToNeighbor;
        let (hopper, _, hopper_recording) = make_recorder();
        let system = System::new("suppress_redundant_connection_retry");
        let addr = subject.start();
        addr.try_send(BindMessage {
            peer_actors: peer_actors_builder().hopper(hopper).build(),
        })
        .unwrap();
        addr.try_send(AskAboutDebutGossipMessage {
            prev_connection_progress: failed_progress.clone(),
        })
        .unwrap();

        let assertions = Box::new(move |actor: &mut Neighborhood| {
            assert_eq!(actor.overall_connection_status.progress[0], failed_progress);
        });
        addr.try_send(AssertionsMessage { assertions }).unwrap();
        System::current().stop();
        assert_eq!(system.run(), 0);
    ...
    pub fn neighborhood_logs_with_trace_if_it_receives_ask_about_debut_message_from_unknown_descriptor(
        ...
        assert_eq!(system.run(), 0);
        TestLogHandler::new().exists_log_containing(
            "TRACE: Neighborhood: Received an AskAboutDebutGossipMessage for an unknown node descriptor; identity redacted - ignoring",
        );
    }
    ...
    #[test]
    fn late_neighborship_progress_does_not_demote_validated_route_readiness() {
        let system =
            System::new("late_neighborship_progress_does_not_demote_validated_route_readiness");
        let (peer_ip, descriptor) = make_node(1);
        let mut subject = make_subject_from_node_descriptor(
            &descriptor,
            "late_neighborship_progress_does_not_demote_validated_route_readiness",
        );
        subject.overall_connection_status.stage = OverallConnectionStage::RouteFound;
        subject.topology_route_available = true;
        subject
            .overall_connection_status
            .get_connection_progress_by_ip(peer_ip)
            .unwrap()
            .connection_stage = ConnectionStage::TcpConnectionEstablished;
        let (ui_gateway, _, ui_recording_arc) = make_recorder();
        subject.node_to_ui_recipient_opt = Some(ui_gateway.start().recipient());
        let addr = subject.start();

        addr.try_send(ConnectionProgressMessage {
            peer_addr: peer_ip,
            event: ConnectionProgressEvent::StandardGossipReceived,
        })
        .unwrap();
        ...
        System::current().stop();
        assert_eq!(system.run(), 0);
        assert_eq!(ui_recording_arc.lock().unwrap().len(), 0);
    }

    #[test]
    fn gossip_failure_from_unknown_node_is_ignored() {
        init_test_logging();
        let mut subject = make_standard_subject();
        let descriptors_before: Vec<NodeDescriptor> = subject
            .overall_connection_status
            .iter_initial_node_descriptors()
            .cloned()
            .collect();

        subject.handle_gossip_failure(
            SocketAddr::new(make_ip(42), 1234),
            GossipFailure_0v1::NoNeighbors,
        );

        let descriptors_after: Vec<NodeDescriptor> = subject
            .overall_connection_status
            .iter_initial_node_descriptors()
            .cloned()
            .collect();
        ...
        TestLogHandler::new().exists_log_containing(
            "WARN: Neighborhood: Ignoring GossipFailure from unknown Node; source and reason redacted",
        );
    }

    #[test]
    fn gossip_failure_from_current_pass_target_marks_only_matching_progress_for_retry() {
        init_test_logging();
        let cryptde: &dyn CryptDE = N_CRYPTDE_PAIR.main.as_ref();
        let original_node = make_node_record(3456, true);
        let other_node = make_node_record(4567, true);
        let original_descriptor =
            NodeDescriptor::from((&original_node, Chain::EthRopsten, cryptde));
        let other_descriptor = NodeDescriptor::from((&other_node, Chain::EthRopsten, cryptde));
        let original_ip = original_descriptor
            .node_addr_opt
            .as_ref()
            .unwrap()
            .ip_addr();
        let pass_target = make_ip(42);
        let mut subject = Neighborhood::new(
            N_CRYPTDE_PAIR.clone(),
            &bc_from_nc_plus(
                NeighborhoodConfig {
                    mode: NeighborhoodMode::Standard(
                        NodeAddr::new(&make_ip(99), &[5678]),
                        vec![original_descriptor.clone(), other_descriptor.clone()],
                        rate_pack(100),
                    ),
                    min_hops: MIN_HOPS_FOR_TEST,
                },
                make_wallet("earning"),
                None,
                "gossip_failure_from_current_pass_target_marks_only_matching_progress_for_retry",
            ),
        );
        let connection_progress = subject
            .overall_connection_status
            .get_connection_progress_by_ip(original_ip)
            .unwrap();
        assert!(OverallConnectionStatus::update_connection_stage(
            connection_progress,
            ConnectionProgressEvent::TcpConnectionSuccessful,
            &subject.logger,
        ));
        ...
        assert_eq!(
            subject.handle_gossip_failure(
                SocketAddr::new(pass_target, 1234),
        ...
        assert_eq!(
            subject
                ...
                .iter()
                .map(|progress| (
                    progress.initial_node_descriptor.clone(),
                    progress.current_peer_addr,
                    progress.connection_stage.clone(),
                ))
                .collect::<Vec<_>>(),
            vec![
                (
                    original_descriptor,
                    pass_target,
                    ConnectionStage::Failed(DebutRejected),
                ),
                (
                    other_descriptor.clone(),
                    other_descriptor.node_addr_opt.unwrap().ip_addr(),
                    ConnectionStage::StageZero,
                ),
            ]
        );
        TestLogHandler::new().exists_log_containing(
            "WARN: Neighborhood: A Node refused Debut: Node owner manually rejected your Debut; identity redacted",
        );
    }

    #[test]
    fn gossip_failures_retry_only_transient_refusals_without_stopping_the_neighborhood() {
        init_test_logging();
        let cryptde: &dyn CryptDE = N_CRYPTDE_PAIR.main.as_ref();
        let earning_wallet = make_wallet("earning");
        ...
        let one_neighbor_descriptor =
            NodeDescriptor::from((&one_neighbor_node, Chain::EthRopsten, cryptde));
        let another_neighbor_descriptor =
            NodeDescriptor::from((&another_neighbor_node, Chain::EthRopsten, cryptde));
        let mut subject = Neighborhood::new(
            N_CRYPTDE_PAIR.clone(),
            &bc_from_nc_plus(
                NeighborhoodConfig {
                    mode: NeighborhoodMode::Standard(
                        this_node_addr.clone(),
                        vec![
                            one_neighbor_descriptor.clone(),
                            another_neighbor_descriptor.clone(),
                        ],
                        rate_pack(100),
                    ),
                    min_hops: MIN_HOPS_FOR_TEST,
                ...
                earning_wallet.clone(),
                None,
                "gossip_failures_retry_only_transient_refusals_without_stopping_the_neighborhood",
            ),
        );
        let notify_later_params = Arc::new(Mutex::new(vec![]));
        subject.tools.notify_later_ask_about_gossip =
        ...
        let assertions = Box::new(|actor: &mut Neighborhood| {
            assert_eq!(actor.overall_connection_status.progress.len(), 2);
            assert!(actor
                .overall_connection_status
                .progress
                .iter()
                .all(
                    |progress| progress.connection_stage == ConnectionStage::Failed(DebutRejected)
                ));
        });
        addr.try_send(AssertionsMessage { assertions }).unwrap();
        System::current().stop();
        ...
        assert_eq!(
            *notify_later_params.lock().unwrap(),
            vec![(
                AskAboutDebutGossipMessage {
                    prev_connection_progress: ConnectionProgress {
                        initial_node_descriptor: one_neighbor_descriptor.clone(),
                        current_peer_addr: one_neighbor_descriptor.node_addr_opt.unwrap().ip_addr(),
                        connection_stage: ConnectionStage::Failed(DebutRejected),
                    },
                },
                Duration::from_millis(10),
            ),]
        ...
        tlh.exists_log_containing("WARN: Neighborhood: A Node refused Debut: No neighbors for Introduction or Pass; identity redacted");
        tlh.exists_log_containing("WARN: Neighborhood: A Node refused Debut: Node owner manually rejected your Debut; identity redacted");
        tlh.exists_log_containing(
            "INFO: Neighborhood: Not retrying Node because its owner manually rejected the Debut; identity redacted.",
        );
        tlh.exists_no_log_containing("shutting down");
    }
    ...
    fn route_query_responds_with_none_when_asked_for_two_hop_one_way_route_without_consuming_wallet(
        ...
        let sub: Recipient<RouteQueryMessage> = addr.recipient::<RouteQueryMessage>();
        let msg =
            RouteQueryMessage::data_indefinite_route_request(Host::new("booga.com", 1234), 20000);
        ...
        assert_handling_of_config_change_msg(
            ...
            |subject: &Neighborhood| {
                ...
                let _ = TestLogHandler::new()
                    .exists_log_containing("INFO: ConfigChange: Consuming Wallet has been updated");
            },
        );
        assert_handling_of_config_change_msg(
            ConfigChangeMsg {
    ...
    */
    ...
    fn complete_routes_exercise() {
        ...
        let route_opt = subject.find_best_route_segment(
            p,
            None,
            2,
            10000,
    ...
    */
    ...
    fn route_optimization_by_serving_rates() {
        ...
        let mut intervals = Vec::with_capacity(7);

        // Use the fastest of repeated identical searches. Wall-clock timing in the parallel suite
        // includes arbitrary scheduler pauses; a real algorithm regression slows every sample.
        for _ in 0..7 {
            ...
            let route = subject
                ...
                .unwrap();
            intervals.push(before.elapsed());
            assert_eq!(route, expected_route);
        }

        let interval = intervals.into_iter().min().unwrap();
        assert!(
            interval.as_millis() <= 100,
            "Should have calculated the fastest route in <=100ms, but was {}ms",
            interval.as_millis()
        );
    }
    ...
    */
    ...
    #[test]
    fn route_query_requires_receipt_capability_and_matching_contract_on_every_selected_peer() {
        use ethereum_types::Address;

        let mut subject = make_standard_subject();
        let root_key = subject.neighborhood_database.root().public_key().clone();
        let relay_key = subject
            .neighborhood_database
            .add_node(make_node_record(4567, true))
            .unwrap()
            .clone();
        let exit_key = subject
            .neighborhood_database
            .add_node(make_node_record(5678, true))
            .unwrap()
            .clone();
        subject
            .neighborhood_database
            .add_arbitrary_full_neighbor(&root_key, &relay_key);
        subject
            .neighborhood_database
            .add_arbitrary_full_neighbor(&relay_key, &exit_key);
        let capability = crate::sub_lib::service_receipt::SERVICE_RECEIPT_SETTLEMENT_V1_CAPABILITY;
        let settlement_contract = Address::from([0x77; 20]);
        [&root_key, &relay_key, &exit_key].iter().for_each(|key| {
            let record = subject.neighborhood_database.node_by_key_mut(key).unwrap();
            record.inner.protocol_capabilities = capability;
            record.inner.receipt_settlement_contract_opt = Some(settlement_contract);
        });

        assert_eq!(
            subject.find_best_route_segment(
                &root_key,
                Some(&exit_key),
                2,
                10_000,
            ...
            Some(vec![&root_key, &relay_key, &exit_key])
        );

        subject
            .neighborhood_database
            .node_by_key_mut(&relay_key)
            .unwrap()
            .inner
            .receipt_settlement_contract_opt = Some(Address::from([0x88; 20]));
        assert_eq!(
            subject.find_best_route_segment(
                &root_key,
                Some(&exit_key),
        ...
        let relay = subject
            ...
            .node_by_key_mut(&relay_key)
            .unwrap();
        relay.inner.receipt_settlement_contract_opt = Some(settlement_contract);
        relay.inner.protocol_capabilities = 0;
        assert_eq!(
            subject.find_best_route_segment(
                &root_key,
                Some(&exit_key),
    ...
    #[test]
    fn computing_undesirability_saturates_extreme_charges() {
        let mut node_record = make_node_record(1234, false);
        node_record.inner.rate_pack = RatePack::new(u64::MAX, u64::MAX, u64::MAX, u64::MAX);
        node_record
            .metadata
            .unreachable_hosts
            .insert("hostname.com".to_string());
        node_record.metadata.country_undesirability = u32::MAX;
        let subject = make_standard_subject();
        let logger = Logger::new("computing_undesirability_saturates_extreme_charges");

        assert_eq!(
            Neighborhood::compute_undesirability(
                &node_record,
                u64::MAX,
        ...
        assert_eq!(
            Neighborhood::compute_undesirability(
                &node_record,
                u64::MAX,
                UndesirabilityType::ExitRequest("hostname.com"),
                &logger,
            ),
            i64::MAX
        );
        ...
        assert_eq!(
            subject.compute_new_undesirability(
                &node_record,
                i64::MAX - 1,
                None,
                1,
    ...
    #[test]
    fn uniform_minimum_route_choice_reaches_every_exact_tie_and_excludes_worse_routes() {
        use rand::rngs::SmallRng;
        use rand::SeedableRng;

        let first = PublicKey::from(&b"first"[..]);
        let second = PublicKey::from(&b"second"[..]);
        let worse = PublicKey::from(&b"worse"[..]);
        let mut selected = std::collections::BTreeSet::new();

        for seed in 0..128u64 {
            let candidates = vec![
                ComputedRouteSegment::new(vec![&first], 100),
                ComputedRouteSegment::new(vec![&second], 100),
                ComputedRouteSegment::new(vec![&worse], 101),
            ];
            let mut rng = SmallRng::seed_from_u64(seed);
            let route = Neighborhood::choose_uniform_minimum_route(candidates, 100, &mut rng)
                .expect("two exact-minimum routes should qualify");
            selected.insert(route[0].clone());
        }

        assert_eq!(selected.len(), 2);
        assert!(selected.contains(&first));
        assert!(selected.contains(&second));
        assert!(!selected.contains(&worse));
    }

    #[test]
    fn anticipated_response_payload_len_saturates_instead_of_wrapping() {
        assert_eq!(Neighborhood::anticipated_response_payload_len(0), 0);
        assert_eq!(Neighborhood::anticipated_response_payload_len(42), 42_000);
        assert_eq!(
            Neighborhood::anticipated_response_payload_len(
                usize::MAX / RESPONSE_UNDESIRABILITY_FACTOR + 1
            ),
            usize::MAX
        );
    }
    ...
    #[test]
    fn computing_undesirability_adds_local_latency_only_for_exit_service() {
        let mut node_record = make_node_record(2345, false);
        node_record.metadata.route_latency_ewma_ms_opt = Some(250);
        let subject = make_standard_subject();
        let base = 1_000_000;
        let payload_size = 1_000;

        let exit = subject.compute_new_undesirability(
            &node_record,
            base,
            None,
            0,
        ...
        let relay = subject.compute_new_undesirability(
            &node_record,
            base,
            None,
            1,
        ...
        assert_eq!(
            exit,
            base + node_record.rate_pack().exit_charge(payload_size) as i64 + 250_000
        );
        assert_eq!(
            relay,
            base + node_record.rate_pack().routing_charge(payload_size) as i64
        );
    }
    ...
    fn computing_undesirability_works_for_exit_on_over_leg_for_blacklisted_host() {
        ...
        TestLogHandler::new().exists_log_containing(
            "TRACE: Neighborhood: An exit Node failed to reach destination host [REDACTED] during ExitRequest; identity redacted; Undesirability: 2350745 + 100000000 + 0 = 102350745",
        );
    }
    ...
    #[test]
    fn neighborhood_keeps_processing_no_lookup_gossip_when_hopper_is_unbound() {
        init_test_logging();
        let test_name = "neighborhood_keeps_processing_no_lookup_gossip_when_hopper_is_unbound";
        let subject_node = make_global_cryptde_node_record(5555, true, &N_CRYPTDE_PAIR);
        let target_node = make_node_record(1111, true);
        let target_node_addr = target_node.node_addr_opt().unwrap();
        let mut subject = neighborhood_from_nodes(&subject_node, None, &N_CRYPTDE_PAIR);
        subject.logger = Logger::new(test_name);

        subject.handle_gossip_reply(
            Gossip_0v1::new(vec![]),
            target_node.public_key(),
            &target_node_addr,
        );
        subject.handle_gossip_failed(
            GossipFailure_0v1::NoSuitableNeighbors,
            target_node.public_key(),
            &target_node_addr,
        );
        ...
        let tlh = TestLogHandler::new();
        tlh.exists_log_containing(&format!(
            "ERROR: {test_name}: Could not send Gossip reply; destination and error details redacted"
        ));
        tlh.exists_log_containing(&format!(
            "ERROR: {test_name}: Could not send Gossip failure; destination and error details redacted"
        ));
        tlh.exists_no_log_containing(&format!("TRACE: {test_name}: Sent Gossip"));
    }
    ...
    #[test]
    fn correlated_route_use_promotes_readiness_and_starts_accountant_once() {
        let system =
            System::new("correlated_route_use_promotes_readiness_and_starts_accountant_once");
        let (ui_gateway, _, ui_recording_arc) = make_recorder();
        let (accountant, _, accountant_recording_arc) = make_recorder();
        let mut subject = make_standard_subject();
        subject.overall_connection_status.stage = OverallConnectionStage::ConnectedToNeighbor;
        ...
        subject_addr.try_send(BindMessage { peer_actors }).unwrap();
        subject_addr.try_send(RouteUseSucceededMessage).unwrap();
        subject_addr.try_send(RouteUseSucceededMessage).unwrap();
        subject_addr
            .try_send(AssertionsMessage {
                assertions: Box::new(|neighborhood: &mut Neighborhood| {
                    assert!(neighborhood.topology_route_available);
                    assert!(neighborhood.accountant_started);
                    assert_eq!(neighborhood.route_readiness_generation, 2);
                    assert!(neighborhood.route_readiness_expiration_handle_opt.is_some());
                    assert_eq!(
                        neighborhood.overall_connection_status.stage(),
                        OverallConnectionStage::RouteFound
                    );
        ...
        System::current().stop();
        assert_eq!(system.run(), 0);
        let ui_recording = ui_recording_arc.lock().unwrap();
        assert_eq!(ui_recording.len(), 1);
        assert_eq!(
            ui_recording.get_record::<NodeToUiMessage>(0),
            &NodeToUiMessage {
                target: MessageTarget::AllClients,
                body: UiConnectionChangeBroadcast {
    ...
    #[test]
    fn route_readiness_lease_is_scheduled_and_expires_without_discarding_topology() {
        let system = System::new(
            "route_readiness_lease_is_scheduled_and_expires_without_discarding_topology",
        );
        let (ui_gateway, _, ui_recording_arc) = make_recorder();
        let ui_gateway = ui_gateway.system_stop_conditions(match_lazily_every_type_id!(
            NodeToUiMessage,
            NodeToUiMessage
        ));
        ...
        let mut subject = make_standard_subject();
        let root_key = subject.neighborhood_database.root().public_key().clone();
        let neighbor = make_node_record(4568, true);
        let neighbor_key = neighbor.public_key().clone();
        subject.neighborhood_database.add_node(neighbor).unwrap();
        subject
            .neighborhood_database
            .add_arbitrary_full_neighbor(&root_key, &neighbor_key);
        subject.overall_connection_status.stage = OverallConnectionStage::ConnectedToNeighbor;
        subject.route_readiness_ttl = Duration::from_millis(1);
        let subject_addr = subject.start();
        let peer_actors = peer_actors_builder()
            .ui_gateway(ui_gateway)
            .accountant(accountant)
        ...
        assert_eq!(
            ui_recording.get_record::<NodeToUiMessage>(1),
            &NodeToUiMessage {
                target: MessageTarget::AllClients,
                body: UiConnectionChangeBroadcast {
                    stage: UiConnectionStage::ConnectedToNeighbor
    ...
    #[test]
    fn only_the_current_readiness_lease_can_expire() {
        let mut subject = make_standard_subject();
        let root_key = subject.neighborhood_database.root().public_key().clone();
        let neighbor = make_node_record(4569, true);
        let neighbor_key = neighbor.public_key().clone();
        subject.neighborhood_database.add_node(neighbor).unwrap();
        subject
            .neighborhood_database
        ...
        subject.overall_connection_status.stage = OverallConnectionStage::RouteFound;
        subject.topology_route_available = true;
        subject.route_readiness_generation = 2;
        subject.node_to_ui_recipient_opt = None;

        assert!(!subject.expire_route_readiness_lease(1));
        assert_eq!(
            subject.overall_connection_status.stage(),
            OverallConnectionStage::RouteFound
        );
        assert!(subject.topology_route_available);

        assert!(subject.expire_route_readiness_lease(2));
        assert_eq!(
            subject.overall_connection_status.stage(),
            OverallConnectionStage::ConnectedToNeighbor
        );
        assert!(subject.topology_route_available);
        assert!(subject.route_readiness_proof_is_stale);
    }

    #[test]
    fn repeated_route_failures_demote_readiness_and_success_resets_the_threshold() {
        let system = System::new(
            "repeated_route_failures_demote_readiness_and_success_resets_the_threshold",
        );
        let (ui_gateway, _, ui_recording_arc) = make_recorder();
        let mut subject = make_standard_subject();
        let root_key = subject.neighborhood_database.root().public_key().clone();
        let neighbor = make_node_record(4567, true);
        let neighbor_key = neighbor.public_key().clone();
        subject.neighborhood_database.add_node(neighbor).unwrap();
        subject
            .neighborhood_database
        ...
        subject_addr
            .try_send(AssertionsMessage {
                assertions: Box::new(|neighborhood: &mut Neighborhood| {
                    assert!(!neighborhood.topology_route_available);
                    assert_eq!(
                        neighborhood.consecutive_route_failures,
                        CONSECUTIVE_ROUTE_FAILURES_BEFORE_DEMOTION
                    );
                    assert_eq!(
                        neighborhood.overall_connection_status.stage(),
                        OverallConnectionStage::ConnectedToNeighbor
    ...
    fn neighborhood_ignores_gossip_if_it_receives_a_pass_target_which_is_a_part_of_a_different_connection_progress(
        ...
        TestLogHandler::new().exists_log_containing("Gossip ignored; source redacted");
    }

    fn assert_topology_check(hops: Hops) {
        assert_topology_check_with_neighbor_changes_specified(hops, false);
        assert_topology_check_with_neighbor_changes_specified(hops, true);
    }

    fn assert_topology_check_with_neighbor_changes_specified(hops: Hops, neighbor_changes: bool) {
        init_test_logging();
        ...
        TestLogHandler::new().exists_log_containing(&format!(
            "DEBUG: {}: The topology check has found a {}-hop route; awaiting correlated response data before reporting RouteFound.",
            test_name, hops as usize
        ));
    }

    #[test]
    fn topology_check_for_different_hops_does_not_claim_end_to_end_readiness() {
        assert_topology_check(Hops::OneHop);
        assert_topology_check(Hops::TwoHops);
        assert_topology_check(Hops::ThreeHops);
        assert_topology_check(Hops::FourHops);
        assert_topology_check(Hops::FiveHops);
        assert_topology_check(Hops::SixHops);
    }
    ...
    fn neighborhood_logs_when_min_hops_route_can_not_be_made() {
        ...
        let tlh = TestLogHandler::new();
        tlh.exists_log_containing(&format!(
            "DEBUG: {test_name}: The topology check still can't find a good route.",
        ));
        tlh.exists_no_log_containing(&format!(
            "DEBUG: {test_name}: The topology check has found a 5-hop route"
        ));
    }
    ...
    fn neighborhood_logs_error_when_past_neighbors_update_fails_for_another_reason() {
        ...
        TestLogHandler::new().exists_log_containing(
            "ERROR: Neighborhood: Could not persist immediate-neighbor changes; error details redacted",
        );
    }
    ...
    fn handle_new_public_ip_changes_public_ip_and_country_code_nothing_else() {
        ...
        TestLogHandler::new().exists_log_containing(
            "INFO: Neighborhood: Changed public IP; old and new addresses redacted",
        );
    }
    ...
    fn neighborhood_sends_from_gossip_producer_when_acceptance_introductions_are_not_provided() {
        ...
        let tlh = TestLogHandler::new();
        let info_index = tlh.exists_log_containing(
            "INFO: Neighborhood: Sending update Gossip about 0 Nodes; destination redacted",
        );
        let trace_index = tlh.exists_log_containing(
            "TRACE: Neighborhood: Sent update Gossip about 0 Nodes; topology redacted",
        );
        let info_log = tlh.get_log_at(info_index);
        let trace_log = tlh.get_log_at(trace_index);
        assert!(!info_log.contains(&format!("{}", full_neighbor.public_key())));
        assert!(!info_log.contains(&format!("{}", half_neighbor.public_key())));
        assert!(!trace_log.contains("1.2.3.4"));
        assert!(!trace_log.contains("2.3.4.5"));
    }
    ...
    #[test]
    fn neighborhood_skips_gossip_to_an_unencryptable_neighbor_without_stopping() {
        init_test_logging();
        let test_name = "neighborhood_skips_gossip_to_an_unencryptable_neighbor_without_stopping";
        let subject_node = make_global_cryptde_node_record(5555, true, &N_CRYPTDE_PAIR);
        let mut subject = neighborhood_from_nodes(&subject_node, None, &N_CRYPTDE_PAIR);
        subject.logger = Logger::new(test_name);
        let (hopper, _, hopper_recording) = make_recorder();
        subject.hopper_opt = Some(
            peer_actors_builder()
                .hopper(hopper)
                .build()
                .hopper
        ...
        subject.gossip_to_neighbor(&PublicKey::new(&[]), Gossip_0v1::new(vec![]));

        System::current().stop();
        system.run();
        assert_eq!(hopper_recording.lock().unwrap().len(), 0);
        TestLogHandler::new().exists_log_containing(&format!(
            "ERROR: {test_name}: Could not create Gossip route; destination and error details redacted"
        ));
    }
    ...
    fn neighborhood_complains_about_inability_to_ban_when_gossip_acceptor_requests_it() {
        ...
        assert_eq!(0, hopper_recording.len());
        let tlh = TestLogHandler::new();
        tlh.exists_log_containing("WARN: Neighborhood: Malefactor detected, but malefactor bans are not yet implemented; identity and reason redacted - ignoring");
    }
    ...
    fn neighborhood_does_not_accept_gossip_if_a_record_is_non_deserializable() {
        ...
        TestLogHandler::new().exists_log_containing(
            "ERROR: Neighborhood: Received non-deserializable Gossip; source redacted",
        );
    }
    ...
    fn neighborhood_does_not_accept_gossip_if_a_record_signature_is_invalid() {
        ...
        // No panic means that subject didn't try to invoke the GossipAcceptorMock: test passes!
        TestLogHandler::new().exists_log_containing(
            "ERROR: Neighborhood: Received Gossip with invalid signature; source redacted",
        );
    }

    #[test]
    fn neighborhood_logs_received_gossip_without_source_or_topology() {
        init_test_logging();
        let cryptde = N_CRYPTDE_PAIR.main.as_ref();
        let this_node = NodeRecord::new_for_tests(
            &cryptde.public_key(),
        ...
        thread::spawn(move || {
            ...
            let subject = Neighborhood::new(
                ...
                &bc_from_nc_plus(
                    ...
                    None,
                    "neighborhood_logs_received_gossip_without_source_or_topology",
                ),
            );
        ...
        let tlh = TestLogHandler::new();
        let log_index = tlh.await_log_containing(
            "Received Gossip with 2 record(s); source and topology redacted",
            5000,
        );
        let received_log = tlh.get_log_at(log_index);
        assert!(!received_log.contains("BAYFBw"));
        assert!(!received_log.contains("AQMCBA"));
        assert!(!received_log.contains("4.6.5.7"));
        assert!(!received_log.contains(&format!("{}", cryptde.public_key())));
    }
    ...
    fn node_gossips_to_neighbors_on_startup() {
        ...
        let log_index = logs.exists_log_containing(
            "DEBUG: node_gossips_to_neighbors_on_startup: Debut Gossip sent; destination redacted",
        );
        assert!(!logs
            .get_log_at(log_index)
            .contains(&format!("{:?}", debut_target)));
    }
    ...
    */
    ...
    #[test]
    fn losing_the_last_neighbor_revokes_validated_route_readiness() {
        let system = System::new("losing_the_last_neighbor_revokes_validated_route_readiness");
        let (ui_gateway, _, ui_recording_arc) = make_recorder();
        let mut subject = make_standard_subject();
        let root_key = subject.neighborhood_database.root().public_key().clone();
        let neighbor = make_node_record(7654, true);
        let neighbor_key = neighbor.public_key().clone();
        subject.neighborhood_database.add_node(neighbor).unwrap();
        subject
            .neighborhood_database
        ...
        subject_addr.try_send(BindMessage { peer_actors }).unwrap();
        subject_addr
            .try_send(RemoveNeighborMessage {
                public_key: neighbor_key,
            })
            .unwrap();
        subject_addr
            .try_send(AssertionsMessage {
    ...
    fn make_round_trip_route_returns_error_when_no_non_next_door_neighbor_found() {
        ...
        assert_eq!(
            Err(format!(
                "Couldn't find any routes: at least {}-hop to ProxyClient; node identities redacted",
                min_hops as usize
            )),
            result
        );
    ...
    fn make_round_trip_succeeds_when_it_finds_non_next_door_neighbor_exit_node() {
        ...
        let actual_keys: Vec<PublicKey> = match hops.as_slice() {
            [hop, exit, hop_back, origin, empty] => vec![
                decodex::<LiveHop>(N_CRYPTDE_PAIR.main.as_ref(), hop)
                    .expect("hop")
                    .public_key,
                ...
                decodex::<LiveHop>(&next_door_neighbor_cryptde, origin)
                    .expect("origin")
                    .public_key,
                decodex::<LiveHop>(N_CRYPTDE_PAIR.main.as_ref(), empty)
                    .expect("empty")
                    .public_key,
            ],
            l => panic!(
    ...
    */
    ...
    fn node_record_metadata_message_is_handled_properly() {
        ...
        let assertions = Box::new(move |actor: &mut Neighborhood| {
            ...
            TestLogHandler::new().exists_log_matching(
                "DEBUG: Neighborhood: Marking a destination host unreachable for a Node; identities redacted"
            );
        });
        addr.try_send(AssertionsMessage { assertions }).unwrap();
        System::current().stop();
    ...
    #[test]
    fn node_record_metadata_remove_clears_recovered_host() {
        init_test_logging();
        let subject_node = make_global_cryptde_node_record(1345, true, &N_CRYPTDE_PAIR);
        let public_key = PublicKey::from(&b"exit_node"[..]);
        let unreachable_host = String::from("facebook.com");
        ...
        let _ = subject.neighborhood_database.add_node(node_record);
        let addr = subject.start();
        let system = System::new("node_record_metadata_remove_clears_recovered_host");
        ...
        let assertions = Box::new(move |actor: &mut Neighborhood| {
            ...
            assert!(!updated_node_record
                ...
                .contains(&unreachable_host));
            TestLogHandler::new().exists_log_matching(
                "DEBUG: Neighborhood: Clearing a recovered destination host for a Node; identities redacted"
            );
        });
        addr.try_send(AssertionsMessage { assertions }).unwrap();
        System::current().stop();
    ...
    #[test]
    fn node_record_metadata_records_a_bounded_identifier_free_latency_ewma() {
        let subject_node = make_global_cryptde_node_record(1345, true, &N_CRYPTDE_PAIR);
        let public_key = PublicKey::from(&b"exit_node"[..]);
        let unreachable_host = String::from("recovered.example");
        let node_record_inputs = NodeRecordInputs {
            earning_wallet: make_wallet("earning"),
            rate_pack: rate_pack(100),
            accepts_connections: true,
        ...
        let addr = subject.start();
        let system =
            System::new("node_record_metadata_records_a_bounded_identifier_free_latency_ewma");

        addr.try_send(UpdateNodeRecordMetadataMessage {
            public_key: public_key.clone(),
            metadata_change: NRMetadataChange::RecordRouteSuccess {
                hostname: unreachable_host.clone(),
                latency_ms: 400,
            },
        })
        ...
        let assertions = Box::new(move |actor: &mut Neighborhood| {
            ...
            assert!(!updated
                ...
                .unreachable_hosts
                .contains(&unreachable_host));
            assert_eq!(updated.metadata.route_latency_ewma_ms_opt, Some(15_375));
        });
        addr.try_send(AssertionsMessage { assertions }).unwrap();
        System::current().stop();
        assert_eq!(system.run(), 0);
    ...
    fn handle_stream_shutdown_handles_socket_addr_with_unknown_ip() {
        ...
        let hopper_recording = hopper_recording_arc.lock().unwrap();
        assert_eq!(hopper_recording.len(), 0);
        TestLogHandler::new().exists_log_containing("WARN: Neighborhood: Received shutdown notification for an unknown peer; identity redacted - ignoring");
    }

    #[test]
    fn initial_peer_shutdown_demotes_stale_mobile_connection_status() {
        let initial_node = make_node_record(3123, true);
        let initial_node_addr = initial_node.node_addr_opt().unwrap();
        let root_node = make_global_cryptde_node_record(1345, true, &N_CRYPTDE_PAIR);
        let mut subject = neighborhood_from_nodes(&root_node, Some(&initial_node), &N_CRYPTDE_PAIR);
        subject.overall_connection_status.stage = OverallConnectionStage::ConnectedToNeighbor;

        subject.handle_stream_shutdown_msg(StreamShutdownMsg {
            peer_addr: SocketAddr::new(initial_node_addr.ip_addr(), initial_node_addr.ports()[0]),
            stream_type: RemovedStreamType::Clandestine,
            report_to_counterpart: true,
        });
        ...
        assert!(matches!(
            subject.overall_connection_status.progress[0].connection_stage,
            ConnectionStage::Failed(TcpConnectionFailed)
        ));
    }

    #[test]
    fn mobile_retry_replaces_stale_initial_nodes_without_restarting_actix() {
        let old_node = make_node_record(3123, true);
        let fresh_node = make_node_record(4123, true);
        let fresh_addr = fresh_node.node_addr_opt().unwrap().ip_addr();
        let root_node = make_global_cryptde_node_record(1345, true, &N_CRYPTDE_PAIR);
        let mut subject = neighborhood_from_nodes(&root_node, Some(&old_node), &N_CRYPTDE_PAIR);
        subject.overall_connection_status.stage = OverallConnectionStage::ConnectedToNeighbor;
        let descriptor =
            NodeDescriptor::from((&fresh_node, subject.chain, subject.cryptde.as_ref()))
                .to_string(subject.cryptde.as_ref());

        subject.replace_initial_neighbors_for_mobile_retry(vec![descriptor]);

        assert_eq!(subject.overall_connection_status.progress.len(), 1);
        assert_eq!(
            subject.overall_connection_status.progress[0].current_peer_addr,
            fresh_addr
        );
        assert_eq!(
            subject.overall_connection_status.stage(),
    ...
    fn handle_stream_shutdown_handles_already_inactive_node() {
        ...
        let hopper_recording = hopper_recording_arc.lock().unwrap();
        assert_eq!(hopper_recording.len(), 0);
        TestLogHandler::new().exists_log_containing("DEBUG: Neighborhood: Received shutdown notification for a known non-neighbor; identity redacted - ignoring");
    }
    ...
    fn handle_stream_shutdown_handles_existing_socket_addr() {
        ...
        TestLogHandler::new().exists_log_containing("DEBUG: Neighborhood: Received shutdown notification for a known peer: removing neighborship; identity redacted");
    }
    ...
    fn shutdown_instruction_generates_log() {
        ...
        subject.handle_shutdown_order(1234, UiShutdownRequest {});
    }
    ...
    fn connection_status_message_is_handled_properly_for_connected_to_neighbor() {
        ...
        assert_eq!(
            ...
            Some(NodeToUiMessage {
                ...
                body: UiConnectionStatusResponse {
                    stage: stage.into(),
                    reason: Some(UiConnectionStatusReason::RouteNotReady),
                }
                .tmb(context_id),
            })
        )
    ...
    #[test]
    fn connection_status_message_reports_an_expired_route_proof() {
        let stage = OverallConnectionStage::ConnectedToNeighbor;
        let client_id = 1237;
        let context_id = 4324;

        let message_opt = connection_status_message_received_by_ui_with_staleness(
            stage,
            true,
            client_id,
            context_id,
            "connection_status_message_reports_an_expired_route_proof",
        );
    ...
    pub struct GossipAcceptorMock {
        handle_params: Arc<
            ...
            >,
        >,
        handle_results: RefCell<Vec<Vec<GossipAcceptanceResult>>>,
    }
    ...
    impl GossipAcceptorMock {
        ...
        pub fn handle_result(self, result: Vec<GossipAcceptanceResult>) -> GossipAcceptorMock {
    ...
    fn connection_status_message_received_by_ui(
        ...
        connection_status_message_received_by_ui_with_staleness(
            stage, false, client_id, context_id, test_name,
        )
    }

    fn connection_status_message_received_by_ui_with_staleness(
        stage: OverallConnectionStage,
        route_readiness_proof_is_stale: bool,
        client_id: u64,
        context_id: u64,
        ...
        subject.route_readiness_proof_is_stale = route_readiness_proof_is_stale;
        let (ui_gateway, _, ui_gateway_recording_arc) = make_recorder();
        let subject_addr = subject.start();
        let peer_actors = peer_actors_builder().ui_gateway(ui_gateway).build();
        subject_addr.try_send(BindMessage { peer_actors }).unwrap();
