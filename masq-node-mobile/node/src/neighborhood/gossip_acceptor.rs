...
fn try_send_connection_progress(
    neighborhood_metadata: &NeighborhoodMetadata,
    message: ConnectionProgressMessage,
    logger: &Logger,
    gossip_kind: &str,
) {
    if neighborhood_metadata
        .cpm_recipient
        .try_send(message)
        .is_err()
    {
        warning!(
            logger,
            "Could not queue {} connection progress: Neighborhood mailbox is unavailable",
            gossip_kind
        );
    }
}
...
impl GossipHandler for DebutHandler {
    ...
    fn handle(
        ...
        if let Some(preferred_key) = self.find_more_appropriate_neighbor(database, &source_agr) {
            debug!(self.logger, "DebutHandler is commissioning Pass to a more appropriate neighbor; identities redacted");
        ...
        let lcn_key = match Self::find_least_connected_half_neighbor_excluding(
            ...
            None => {
                debug!(self.logger, "Neighbor count at maximum with no non-common neighbor; DebutHandler is ignoring debut; identities redacted");
        ...
        debug!(
            self.logger,
            "DebutHandler is commissioning Pass to the least-connected neighbor; identities redacted"
        );
...
impl DebutHandler {
    ...
    fn try_accept_debut(
        ...
        match database.add_half_neighbor(&debut_node_key) {
            ...
            Ok(true) => {
                ...
                trace!(
                    self.logger,
                    "Current database contains {} Nodes; topology redacted",
                    database.keys().len()
                );
                if Self::should_not_make_another_introduction(database, debuting_agr) {
                    if Self::counts_us_among_neighbors(debuting_agr, database) {
                        debug!(
                                    self.logger,
                                    "Can't send introduction, but the debutant already knows us; sending Standard Gossip; identities redacted",
                                );
                        ...
                        match GossipAcceptorReal::make_debut_triple(database, debuting_agr) {
                            Ok((redebut_gossip, public_key, node_addr)) => {
                                debug!(
                                    self.logger,
                                    "Can't send introduction; sending redebut instead; identities redacted"
                                );
                            ...
                            Err(_) => {
                                debug!(
                                    self.logger,
                                    "Can't send either introduction or redebut; sending Standard Gossip instead; identities and error details redacted"
                                );
                                Ok(GossipAcceptanceResult::Accepted)
                            }
                        }
                    ...
                    match self.try_to_make_introduction(database, debuting_agr, gossip_source) {
                        ...
                        IntroductionAttempt::Failure => {
                            trace!(
                                self.logger,
                                "DebutHandler database contains {} Nodes; topology redacted",
                                database.keys().len(),
                            );
                            if Self::counts_us_among_neighbors(debuting_agr, database) {
                                debug!(
                                    self.logger,
                                    "DebutHandler has no one to introduce and debutant already has us as a neighbor; accepting and sending Standard Gossip; identities redacted",
                                );
                                Ok(GossipAcceptanceResult::Accepted)
                            } else {
                                debug!(
                                    self.logger,
                                    "DebutHandler has no one to introduce and debutant does not have us as a neighbor; debuting back; identities redacted",
                                );
    ...
    fn try_to_make_introduction(
            ...
            Self::find_least_connected_full_neighbor_excluding(database, debuting_agr)
        {
            ...
            debug!(
                self.logger,
                "DebutHandler commissioning Introduction; identities redacted"
            );
            IntroductionAttempt::Success(
...
impl GossipHandler for PassHandler {
    ...
    fn handle(
        ...
        let logger = Logger::new("PassHandler");
        ...
        match database.node_by_key_mut(&introducer_key) {
            Some(existing_introducer_ref) => {
                if existing_introducer_ref.version() < introducer.inner.version {
                    debug!(
                        self.logger,
                        "Updating obsolete introducer from version {} to version {}; identity redacted",
                    ...
                    debug!(
                        self.logger,
                        "Preserving existing introducer at version {}; identity redacted",
                        existing_introducer_ref.version()
                    );
                    return Ok(false);
                }
            ...
            None => {
                ...
                debug!(
                    self.logger,
                    "Adding introducer to database; identity redacted"
                );
                database
...
impl GossipHandler for StandardGossipHandler {
    ...
    fn handle(
        ...
        let gossip_source_agr = match agrs.iter().find(|agr| {
            ...
            None => {
                let message =
                    "Standard gossip sender omitted its self-record; identity redacted".to_string();
...
impl StandardGossipHandler {
        ...
        let neighbors = if current_node_key == root_node.public_key() {
            ...
            match agrs.get(current_node_key) {
                ...
                None => {
                    ...
                    trace!(
                        self.logger,
                        "While computing patch no Node record was found; public key redacted"
                    );
                    return;
                }
            }
    ...
    fn extract_malefactors(
        ...
        let (valid_agrs, bans) = agrs.into_iter().fold((vec![], vec![]), |so_far, agr| {
            ...
            match GossipAcceptorReal::validate_incoming_agr(
                &agr,
                "Standard Gossip record".to_string(),
                &self.rate_pack_limits,
                &self.logger,
            ) {
    ...
    fn identify_non_introductory_new_and_obsolete_nodes(
        ...
        database: &mut NeighborhoodDatabase,
        agrs: Vec<AccessibleGossipRecord>,
    ) -> (Vec<AccessibleGossipRecord>, Vec<AccessibleGossipRecord>) {
    ...
    fn add_new_nodes(
        ...
        agrs.into_iter().for_each(|agr| {
            ...
            trace!(
                self.logger,
                "Discovered new Node with {} full neighbor(s); identities redacted",
                node_record.full_neighbor_keys(database).len()
            );
            database
    ...
    fn update_obsolete_nodes(
        ...
        agrs.into_iter().for_each(|agr| {
            ...
            trace!(
                self.logger,
                "Updating Node from v{} to v{}; identity redacted",
    ...
    fn update_database_record(
        ...
        match existing_node_record.update(agr) {
            ...
            Err(_) => {
                error!(
                    self.logger,
                    "Failed to update Node from v{} to v{}; identity and error details redacted",
    ...
    fn handle(
        ...
        let (qualification, handler_ref) = self
            ...
            .map(|(qualification, handler_ref)| {
                if let Qualification::Unmatched(_) = &qualification {
                    trace!(
                        self.logger,
                        "Gossip did not qualify for {}; source and reason details redacted",
...
mod tests {
    ...
    struct SaturatedConnectionProgressMailbox;

    impl Actor for SaturatedConnectionProgressMailbox {
        type Context = Context<Self>;
    }

    impl Handler<ConnectionProgressMessage> for SaturatedConnectionProgressMailbox {
        type Result = ();

        fn handle(
            &mut self,
            _msg: ConnectionProgressMessage,
    ...
    fn introducee_that_fails_validation_is_rejected() {
        ...
        let message = "Introducee rejected due to rate pack limit violation: ConfiguratorError { param_errors: [ParamError { parameter: \"rate-pack\", reason: \"Value of routing_byte_rate (0) is below the minimum allowed (100)\" }, ParamError { parameter: \"rate-pack\", reason: \"Value of routing_service_rate (0) is below the minimum allowed (100)\" }, ParamError { parameter: \"rate-pack\", reason: \"Value of exit_byte_rate (0) is below the minimum allowed (100)\" }, ParamError { parameter: \"rate-pack\", reason: \"Value of exit_service_rate (0) is below the minimum allowed (100)\" }] }".to_string();
        assert_eq!(
            handle_result,
            vec![GossipAcceptanceResult::Ban(Malefactor::new(
    ...
    fn standard_gossip_handler_can_handle_node_for_which_agr_is_not_found_while_computing_patch() {
        ...
        */
        ...
        TestLogHandler::new().exists_log_containing(&format!(
            "TRACE: {}: While computing patch no Node record was found; public key redacted",
            test_name
        ));
    }
    ...
    fn standard_gossip_that_fails_validation_is_rejected() {
        ...
        */
        ...
        let message = format!(
            "Standard Gossip record rejected due to rate pack limit violation: {}",
            analysis
        );
        assert_eq!(
            handle_result,
    ...
    fn pass_is_properly_handled() {
        ...
        TestLogHandler::new().exists_log_containing(&format!(
            "TRACE: {}: Gossip did not qualify for DebutHandler; source and reason details redacted",
            test_name
        ));
    }

    #[test]
    fn pass_handler_continues_when_connection_progress_mailbox_is_full() {
        init_test_logging();
        let test_name = "pass_handler_continues_when_connection_progress_mailbox_is_full";
        let system = System::new(test_name);
        let cpm_recipient = SaturatedConnectionProgressMailbox::create(|ctx| {
            ctx.set_mailbox_capacity(1);
            SaturatedConnectionProgressMailbox
        })
        .recipient();
        let root_node = make_node_record(1234, true);
        let mut db = db_from_node(&root_node);
        ...
        Arbiter::spawn(future::lazy(move || {
            let filler = ConnectionProgressMessage {
                peer_addr: IpAddr::from_str("1.2.3.4").unwrap(),
            ...
            match &result[0] {
                GossipAcceptanceResult::Reply(_, key, node_addr) => {
                    assert_eq!(key, pass_target.public_key());
                    assert_eq!(node_addr, pass_target.node_addr_opt().as_ref().unwrap());
                }
        ...
        TestLogHandler::new().exists_log_containing(
            "WARN: PassHandler: Could not queue Pass Gossip connection progress: Neighborhood mailbox is unavailable",
        );
    }
    ...
    #[test]
    fn handles_pass_target_timestamp_from_the_future_without_panicking() {
        let cryptde = GA_CRYPTDE_PAIR.main.as_ref();
        let root_node = make_node_record(1234, true);
        let mut db = db_from_node(&root_node);
        let subject = PassHandler::new();
        ...
        let system = System::new("handles_pass_target_timestamp_from_the_future_without_panicking");
        let (cpm_recipient, recording_arc) = make_cpm_recipient();
        let mut neighborhood_metadata = make_default_neighborhood_metadata();
        neighborhood_metadata.cpm_recipient = cpm_recipient;
        let initial_timestamp = SystemTime::now();
