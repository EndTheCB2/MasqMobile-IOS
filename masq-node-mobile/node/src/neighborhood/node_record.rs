...
use crate::sub_lib::service_receipt::SERVICE_RECEIPT_SETTLEMENT_V1_CAPABILITY;
...
pub struct NodeRecordInner_0v1 {
    ...
    /// Signed, forward-compatible feature bits. Old peers ignore this additive CBOR field; new
    /// peers default it to zero when reading records produced before capability advertisement.
    #[serde(default)]
    pub protocol_capabilities: u64,
    /// Signed settlement-network identity. Receipt capability without a nonzero verifier is
    /// deliberately inert, and older records deserialize to `None`.
    #[serde(default)]
    pub receipt_settlement_contract_opt: Option<Address>,
}
...
impl NodeRecordInner_0v1 {
    pub fn supports_service_receipt_settlement_v1(&self) -> bool {
        self.protocol_capabilities & SERVICE_RECEIPT_SETTLEMENT_V1_CAPABILITY != 0
            && self
...
pub struct NodeRecordMetadata {
    ...
    // Local-only, bounded first-response latency. This metadata is neither signed nor gossiped and
    // deliberately carries no hostname, stream, wallet or timestamp.
    pub route_latency_ewma_ms_opt: Option<u32>,
    //TODO introduce a reliability score #583
...
impl NodeRecordMetadata {
    pub fn new() -> NodeRecordMetadata {
        NodeRecordMetadata {
            ...
            route_latency_ewma_ms_opt: None,
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn signed_capabilities_are_backward_compatible_in_both_directions() {
        #[derive(serde_derive::Deserialize, serde_derive::Serialize)]
        struct LegacyNodeRecordInner {
            public_key: PublicKey,
            earning_wallet: Wallet,
            rate_pack: RatePack,
            neighbors: BTreeSet<PublicKey>,
            accepts_connections: bool,
            routes_data: bool,
            version: u32,
            country_code_opt: Option<String>,
        }

        let mut modern = make_node_record(2233, true);
        modern.inner.protocol_capabilities =
            crate::sub_lib::service_receipt::CURRENT_PROTOCOL_CAPABILITIES;
        modern.inner.receipt_settlement_contract_opt = Some(Address::from([0x77; 20]));
        modern.resign();
        assert!(modern.inner.supports_service_receipt_settlement_v1());
        let cryptde = CryptDENull::from(modern.public_key(), TEST_DEFAULT_CHAIN);
        let legacy_view =
            serde_cbor::from_slice::<LegacyNodeRecordInner>(modern.signed_gossip().as_slice())
                .unwrap();
        assert_eq!(legacy_view.public_key, *modern.public_key());
        assert!(cryptde.verify_signature(
            modern.signed_gossip(),
            modern.signature(),
            modern.public_key(),
        ));

        let legacy_signed_data = PlainData::from(
            serde_cbor::to_vec(&legacy_view).expect("legacy record serialization failed"),
        );
        let legacy_record = GossipNodeRecord {
            signature: cryptde.sign(&legacy_signed_data).unwrap(),
            signed_data: legacy_signed_data,
            node_addr_opt: modern.node_addr_opt(),
        };
        let upgraded = NodeRecordInner_0v1::try_from(&legacy_record).unwrap();
        assert_eq!(upgraded.protocol_capabilities, 0);
        assert_eq!(upgraded.receipt_settlement_contract_opt, None);
        assert!(!upgraded.supports_service_receipt_settlement_v1());
    }
