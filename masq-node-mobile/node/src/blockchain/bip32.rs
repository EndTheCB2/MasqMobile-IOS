...
impl std::fmt::Debug for Bip32EncryptionKeyProvider {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter.write_str("Bip32EncryptionKeyProvider([REDACTED])")
    }
}
...
mod tests {
    ...
    #[test]
    fn debug_never_exposes_raw_secret_material() {
        let raw_secret = vec![0x42; Bip32EncryptionKeyProvider::SECRET_KEY_LENGTH];
        let subject = Bip32EncryptionKeyProvider::from_raw_secret(&raw_secret).unwrap();

        let debug_rendering = format!("{:?}", subject);

        assert_eq!(debug_rendering, "Bip32EncryptionKeyProvider([REDACTED])");
        assert!(!debug_rendering.contains(&format!("{:?}", raw_secret)));
    }
