...
use crate::accountant::db_access_objects::receipt_checkpoint_dao::{ConfirmedSettlementClaim, SettlementChainObservation};
...
use web3::types::{Address, BlockId, Log, H256, U256, FilterBuilder, TransactionReceipt, BlockNumber};
use web3::Web3;
...
const SETTLEMENT_CONTRACT_ABI: &str = indoc!(
    r#"[{
    "inputs":[{"name":"claimId","type":"bytes32"}],
    "name":"settledCumulativeChargeWei",
...
impl BlockchainInterface for BlockchainInterfaceWeb3 {
    ...
    ) -> Box<dyn Future<Item = RetrievedBlockchainTransactions, Error = BlockchainInterfaceError>>
    {
        ...
        Box::new(
            lower_level_interface.get_block_number().then(move |rpc_block_number_result| {
                ...
                debug!(
                    logger,
                    "Retrieving transactions from start block: {:?} to end block: {:?}; chain_id: {} contract: {:#x}",
                    start_block_number,
                    end_block_number,
                    num_chain_id,
                    contract_address
                ...
                lower_level_interface.get_transaction_logs(filter)
                    .then(move |logs_result| {
                        match &logs_result {
                            Ok(logs) => trace!(
                                logger,
                                "Transaction logs retrieval completed with {} entries",
                                logs.len()
                            ),
                            Err(_) => trace!(logger, "Transaction logs retrieval failed"),
                        }
    ...
    fn settlement_claims_at_confirmation_depth(
        &self,
        settlement_contract: Address,
        claim_ids: Vec<[u8; 32]>,
        confirmation_depth: u64,
    ) -> Box<dyn Future<Item = SettlementChainObservation, Error = BlockchainInterfaceError>> {
        let web3 = Web3::new(self.transport.clone());
        let web3_for_block = web3.clone();
        let immutable_block = web3
            .eth()
            .block_number()
            .map_err(|error| BlockchainInterfaceError::QueryFailed(error.to_string()))
            .and_then(move |latest| {
                let latest_block_number = latest.as_u64();
                match latest_block_number.checked_sub(confirmation_depth) {
                    None => FutureEither::A(future::err(BlockchainInterfaceError::QueryFailed(
                        format!(
                            "latest block {} is lower than confirmation depth {}",
                            latest_block_number, confirmation_depth
                        ),
                    ))),
                    Some(observed_block_number) => FutureEither::B(
                        web3_for_block
                            .eth()
                            ...
                            .map_err(|error| {
                                BlockchainInterfaceError::QueryFailed(error.to_string())
                            })
                            .and_then(move |block_opt| {
                                let block = block_opt.ok_or_else(|| {
                                    BlockchainInterfaceError::QueryFailed(format!(
                                        "confirmation block {} is unavailable",
                                        observed_block_number
                                    ))
                                })?;
                                if block.number.map(|number| number.as_u64())
                                    != Some(observed_block_number)
                                {
                                    return Err(BlockchainInterfaceError::InvalidResponse);
                                }
                                let observed_block_hash = block
                                    .hash
                                    .ok_or(BlockchainInterfaceError::InvalidResponse)?;
                                Ok((
                                    latest_block_number,
                                    observed_block_number,
                                    observed_block_hash,
        ...
        let web3_for_claims = web3.clone();
        let chain_id = self.chain.rec().num_chain_id;
        Box::new(immutable_block.and_then(
            move |(latest_block_number, observed_block_number, observed_block_hash)| {
                let contract = Contract::from_json(
                    web3_for_claims.eth(),
                    settlement_contract,
                    SETTLEMENT_CONTRACT_ABI.as_bytes(),
                )
                .expect("settlement contract ABI is invalid");
                let queries = claim_ids.into_iter().map(move |claim_id| {
                    contract
                        .clone()
                        .query::<U256, _, _, _>(
                            "settledCumulativeChargeWei",
                            H256(claim_id),
                            None,
                            Options::default(),
                            BlockId::Hash(observed_block_hash),
                        )
                        .map_err(|error| BlockchainInterfaceError::QueryFailed(error.to_string()))
                        .and_then(move |amount| {
                            if amount.bits() > 128 {
                                ...
                                Ok(ConfirmedSettlementClaim {
                                    claim_id,
                                    cumulative_charge_wei: amount.low_u128(),
                                })
                            }
                        })
                });
                future::join_all(queries).map(move |claims| SettlementChainObservation {
                    chain_id,
                    settlement_contract,
                    confirmation_depth,
                    latest_block_number,
                    observed_block_number,
                    observed_block_hash: observed_block_hash.0,
                    claims,
                })
            },
        ))
...
impl BlockchainInterfaceWeb3 {
    ...
    fn handle_transaction_logs(
            ...
            .any(|log| log.topics.len() < 2 || log.data.0.len() > 32)
        {
            warning!(
                logger,
                "Invalid response from blockchain server ({} log entries)",
                logs.len()
            );
            Err(BlockchainInterfaceError::InvalidResponse)
        } else {
            ...
            debug!(logger, "Retrieved {} transactions", transactions.len());
...
mod tests {
    ...
    #[test]
    fn settlement_reader_binds_claim_calls_to_the_confirmation_deep_block_hash() {
        ...
        let server = MBCSBuilder::new(port)
            .ok_response("0x64", 1)
            .ok_response(block, 1)
            .ok_response(format!("0x{:064x}", 100), 1)
            .ok_response(format!("0x{:064x}", 42), 1)
            .start();
        let subject = make_blockchain_interface_web3(port);
        let settlement_contract = Address::from([0x88; 20]);
        let claim_ids = vec![[0xa1; 32], [0xa2; 32]];

        let observation = subject
            .settlement_claims_at_confirmation_depth(settlement_contract, claim_ids.clone(), 12)
            .wait()
            .unwrap();

        assert_eq!(observation.chain_id, subject.get_chain().rec().num_chain_id);
        assert_eq!(observation.settlement_contract, settlement_contract);
        assert_eq!(observation.latest_block_number, 100);
        assert_eq!(observation.observed_block_number, 88);
        assert_eq!(observation.observed_block_hash, [0x11; 32]);
        assert_eq!(
            observation.claims,
            vec![
        ...
        assert!(claim_requests
            .iter()
            .all(|request| request.contains(&block_hash)));
    }
    ...
    fn assert_on_retrieves_transactions(
        ...
        TestLogHandler::new().exists_log_containing(&format!("DEBUG: {test_case}: Retrieving transactions {expected_log}; chain_id: 137 contract: 0xee9a352f6aac4af1a5b9f467f6a93e0ffbe9dd35"));
    }
    ...
    fn process_transaction_receipts_works() {
        ...
        assert_eq!(result.get(&tx_hbt_1).unwrap(), &Err(
                AppRpcError::Remote(
        ...
        assert_eq!(
            result.get(&tx_hbt_5).unwrap(),
            &Ok(StatusReadFromReceiptCheck::Reverted)
        );
        assert_eq!(
