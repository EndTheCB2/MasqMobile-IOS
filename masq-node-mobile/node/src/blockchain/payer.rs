...
impl fmt::Debug for Payer {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.write_str("Payer { wallet: [REDACTED], proof: [REDACTED] }")
    }
}
...
mod tests {
    ...
    #[test]
    fn payer_debug_redacts_wallet_and_signature() {
        let secret = "abacadaba0deadbeefcafefeedbabeface812deadbeefcafefeedbabefaceea7"
            .from_hex::<Vec<u8>>()
            .unwrap();
        let payer = make_payer(&secret, &SubPublicKey::new(b"payer debug public key"));

        assert_eq!(
            format!("{:?}", payer),
            "Payer { wallet: [REDACTED], proof: [REDACTED] }"
        );
    }
}
