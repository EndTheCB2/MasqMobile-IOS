...
use crate::sub_lib::accountant::ReportRoutingServicesConsumedMessage;
    ...
    fn handle(&mut self, msg: ScanForNewPayables, ctx: &mut Self::Context) -> Self::Result {
        ...
        match scheduling_hint {
            ...
            ScanReschedulingAfterEarlyStop::DoNotSchedule => {
                trace!(
                    self.logger,
                    "No early rescheduling is needed for the new-payable scan"
                )
            }
        }
    }
...
impl Handler<ScanError> for Accountant {
    ...
    fn handle(&mut self, scan_error: ScanError, ctx: &mut Self::Context) -> Self::Result {
        error!(
            self.logger,
            "Received an error from the {:?} scan; details redacted", scan_error.scan_type
        );
...
impl Handler<ReportRoutingServicesConsumedMessage> for Accountant {
    type Result = ();

    fn handle(
        &mut self,
        msg: ReportRoutingServicesConsumedMessage,
        _ctx: &mut Self::Context,
    ) -> Self::Result {
        self.handle_report_routing_services_consumed_message(msg);
    }
}
...
impl Accountant {
    ...
    pub fn make_subs_from(addr: &Addr<Accountant>) -> AccountantSubs {
        AccountantSubs {
            ...
            report_routing_services_consumed: recipient!(
                addr,
                ReportRoutingServicesConsumedMessage
            ),
        ...
        if !self.our_wallet(wallet) {
            match self.receivable_dao
                ...
                Err(ReceivableDaoError::SignConversion(_)) => error!(
                    self.logger,
                    "Overflow error recording service provided: service rate {}, byte rate {}, payload size {}. Skipping",
                    service_rate,
                    byte_rate,
                    payload_size
                ),
                Err(e) => panic!("Was recording services provided but hit a fatal database error: {:?}", e)
            };
        } else {
            warning!(
                self.logger,
                "Declining to record a receivable against our own wallet for services we provided"
            );
        }
    }
        ...
        if !self.our_wallet(wallet) {
            match self.payable_dao
                ...
                Err(PayableDaoError::SignConversion(_)) => error!(
                    self.logger,
                    "Overflow error recording consumed services: total charge {}, service rate {}, byte rate {}, payload size {}. Skipping",
                    total_charge,
                    service_rate,
                    byte_rate,
                    payload_size
                ),
                Err(e) => panic!("Recording services consumed has hit fatal database error: {:?}", e)
            };
        } else {
            warning!(
                self.logger,
                "Declining to record a payable against our own wallet for service we provided"
            );
        }
    }
    ...
    fn handle_config_change_msg(&mut self, msg: ConfigChangeMsg) {
        if let ConfigChange::UpdateWallets(wallet_pair) = msg.change {
            if self.earning_wallet != wallet_pair.earning_wallet {
                info!(self.logger, "Earning Wallet has been updated");
    ...
    fn handle_report_routing_service_provided_message(
        ...
        trace!(
            self.logger,
            "Charging {} wei for routing of {} bytes",
            total_charge,
            msg.payload_size
        );
    ...
    fn handle_report_exit_service_provided_message(
        ...
        trace!(
            self.logger,
            "Charging {} wei for exit service for {} bytes at {} per service and {} per byte",
            total_charge,
            msg.payload_size,
            msg.service_rate,
            msg.byte_rate
    ...
    fn handle_report_services_consumed_message(&mut self, msg: ReportServicesConsumedMessage) {
        ...
        trace!(
            self.logger,
            "MsgId {}: Accruing {} wei of debt for consuming {} exited bytes",
            msg_id,
            total_charge,
            msg.exit.payload_size
        );
        ...
        self.record_routing_services_consumed(
            ReportRoutingServicesConsumedMessage {
                timestamp: msg.timestamp,
                payload_size: msg.routing_payload_size,
                routing: msg.routing,
            },
            msg_id,
        );
    ...
    fn handle_report_routing_services_consumed_message(
        &mut self,
        msg: ReportRoutingServicesConsumedMessage,
    ) {
        let msg_id = self.msg_id();
        self.record_routing_services_consumed(msg, msg_id);
    }

    fn record_routing_services_consumed(
        &mut self,
        msg: ReportRoutingServicesConsumedMessage,
        msg_id: u32,
    ) {
        msg.routing.iter().for_each(|routing_service| {
            ...
            trace!(
                self.logger,
                "MsgId {}: Accruing {} wei of debt for consuming {} routed bytes",
                msg_id,
                total_charge,
                msg.payload_size
            );
    ...
    fn register_new_pending_sent_tx(&self, msg: RegisterNewPendingPayables) {
        ...
        match self.sent_payable_dao.insert_new_records(&sent_txs) {
            Ok(_) => debug!(
                self.logger,
                "Registered {} new pending payables",
                msg.new_sent_txs.len()
            ),
            Err(e) => error!(
                self.logger,
                "Failed to save {} new pending payable records due to '{:?}' which is integral \
...
mod tests {
    ...
    fn accountant_handles_config_change_msg() {
        assert_handling_of_config_change_msg(
            ...
            |subject: &Accountant| {
                ...
                let _ = TestLogHandler::new().assert_logs_contain_in_order(vec![
                    "INFO: ConfigChange: Earning Wallet has been updated",
                    "INFO: ConfigChange: Consuming Wallet has been updated",
                ]);
            },
        );
        assert_handling_of_config_change_msg(
    ...
    fn configure_accountant_for_startup_with_preexisting_pending_payables(
        ...
        compute_time_to_next_scan_params_arc: &Arc<Mutex<Vec<()>>>,
    ...
    #[test]
    fn duplicate_initial_pending_payable_scan_is_ignored() {
        let mut subject = AccountantBuilder::default()
            .consuming_wallet(make_wallet("abc"))
            .build();
        ...
        assert_eq!(hint, ScanReschedulingAfterEarlyStop::DoNotSchedule);
        assert!(!subject.scanners.initial_pending_payable_scan());
    }

    #[test]
    fn duplicate_automatic_receivable_scan_is_ignored() {
        let mut subject = AccountantBuilder::default().build();
        let receivable_scanner = ScannerMock::default().scan_started_at_result(Some(UNIX_EPOCH));
        subject
            .scanners
            .replace_scanner(ScannerReplacement::Receivable(ReplacementType::Mock(
                receivable_scanner,
        ...
        let hint = subject.handle_request_of_scan_for_receivable(None);

        assert_eq!(hint, ScanReschedulingAfterEarlyStop::DoNotSchedule);
    }

    #[test]
    fn duplicate_automatic_new_payable_scan_is_ignored() {
        let mut subject = AccountantBuilder::default()
            .consuming_wallet(make_wallet("abc"))
            .build();
        let payable_scanner = ScannerMock::default().scan_started_at_result(Some(UNIX_EPOCH));
        subject
            .scanners
            .replace_scanner(ScannerReplacement::Payable(ReplacementType::Mock(
                payable_scanner,
        ...
        let hint = subject.handle_request_of_scan_for_new_payable(None);
    ...
    fn periodical_scanning_for_receivables_and_delinquencies_works() {
        ...
        let second_msg_towards_bb =
            blockchain_bridge_recording.get_record::<RetrieveTransactions>(1);
    ...
    fn report_routing_service_provided_message_is_received() {
        ...
        TestLogHandler::new().exists_log_containing(
            "TRACE: Accountant: Charging 29658 wei for routing of 1234 bytes",
        );
    }
    ...
    fn report_routing_service_provided_message_is_received_from_our_consuming_wallet() {
        ...
        TestLogHandler::new().exists_log_containing(
            "WARN: Accountant: Declining to record a receivable against our own wallet for services we provided",
        );
    }
    ...
    fn report_exit_service_provided_message_is_received() {
        ...
        TestLogHandler::new().exists_log_containing(
            "TRACE: Accountant: Charging 29658 wei for exit service for 1234 bytes",
        );
    }
    ...
    fn report_services_consumed_message_is_received() {
        ...
        test_log_handler.exists_log_containing(
            "TRACE: Accountant: MsgId 123: Accruing 36120 wei of debt for consuming 1200 exited bytes",
        );
        test_log_handler.exists_log_containing(
            "TRACE: Accountant: MsgId 123: Accruing 82986 wei of debt for consuming 3456 routed bytes",
        );
        test_log_handler.exists_log_containing(
            "TRACE: Accountant: MsgId 123: Accruing 114100 wei of debt for consuming 3456 routed bytes",
        );
    }
    ...
    fn routing_service_consumed_is_reported_for_our_consuming_wallet() {
        ...
        TestLogHandler::new().exists_log_containing(
            "WARN: Accountant: Declining to record a payable against our own wallet for service we provided",
        );
    }
    ...
    #[test]
    #[should_panic(
        expected = "Was recording services provided but hit a fatal database error: \
RusqliteError(\"we cannot help ourselves; this is baaad\")"
    )]
    ...
    fn record_service_provided_handles_overflow() {
        ...
        TestLogHandler::new().exists_log_containing(&format!(
            "ERROR: Accountant: Overflow error recording service provided: service rate {}, byte rate 1, payload size 2. Skipping",
            i64::MAX as u64
        ));
    }
    ...
    fn record_service_consumed_handles_overflow() {
        ...
        TestLogHandler::new().exists_log_containing(&format!(
            "ERROR: Accountant: Overflow error recording consumed services: total charge {}, service rate {}, byte rate 1, payload size 2. Skipping",
            i64::MAX as u64 + 1 * 2,
            i64::MAX as u64
        ));
    }
    ...
    #[test]
    #[should_panic(
        expected = "Recording services consumed has hit fatal database error: \
RusqliteError(\"we cannot help ourselves; this is baaad\")"
    )]
    ...
    fn accountant_handles_registering_new_pending_payables() {
        ...
        TestLogHandler::new().exists_log_containing(&format!(
            "DEBUG: {test_name}: Registered 2 new pending payables"
        ));
    }
    ...
    fn sent_payable_insertion_clearly_failed_and_we_log_at_least() {
        ...
        TestLogHandler::new().exists_log_containing(&format!(
            "ERROR: {test_name}: Failed to save 2 new pending payable records \
    ...
    #[should_panic(
     ...
     db query constraints; balance: 7686005"
    )]
        ...
        match message.response_skeleton_opt {
            Some(response_skeleton) => {
                ...
                TestLogHandler::new().assert_logs_contain_in_order(vec![
                    &format!(
                        "ERROR: {}: Received an error from the {:?} scan; details redacted",
                        test_name, message.scan_type
                    ),
                    &format!(
