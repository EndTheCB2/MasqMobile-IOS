...
impl StartableScanner<ScanForReceivables, RetrieveTransactions> for ReceivableScanner {
    fn start_scan(
        ...
        info!(logger, "Scanning for receivables");
    ...
    fn find_and_ban_delinquents(&self, timestamp: SystemTime, logger: &Logger) {
        self.receivable_dao
            ...
            .for_each(|account| {
                ...
                info!(
                    logger,
                    "A wallet (balance: {} gwei, age: {} sec) was banned for delinquency",
                    balance_str_wei,
                    age.as_secs()
                )
            });
    ...
    fn find_and_unban_reformed_nodes(&self, timestamp: SystemTime, logger: &Logger) {
        self.receivable_dao
            ...
            .for_each(|account| {
                ...
                info!(
                    logger,
                    "A wallet (balance: {} gwei, age: {} sec) is no longer delinquent and was unbanned",
                    balance_str_wei,
                    age.as_secs()
                )
            });
