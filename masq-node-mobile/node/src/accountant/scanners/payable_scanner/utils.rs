        ...
        qualified_accounts
            ...
            .map(|(payable, threshold_point)| {
                let p_age = now
                    ...
                    .expect("Payable time is corrupt");
                format!(
                    "{} wei owed for {} sec exceeds the threshold {} wei for a creditor",
...
pub fn mark_pending_payable_fatal_error(
    ...
    panic!(
        "Unable to create marks in the payable table for {} payments due to {:?}",
        sent_payments.len(),
        error
    )
}
    ...
    fn payables_debug_summary_prints_pretty_summary() {
        ...
        TestLogHandler::new().exists_log_containing(
            "Paying qualified debts:\n\
                   ...
                   10,000,000,001,152,000 wei for a creditor\n\
                   999,999,999,000,000,000 wei owed for 86455 sec exceeds the threshold \
                   999,978,993,055,555,580 wei for a creditor",
        );
    }
