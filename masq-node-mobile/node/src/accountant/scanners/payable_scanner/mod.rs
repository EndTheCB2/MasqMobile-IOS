...
use crate::accountant::{
    gwei_to_wei, PayableScanType, PendingPayable, ResponseSkeleton, ScanForNewPayables,
...
impl PayableScanner {
    ...
    fn check_on_missing_sent_tx_records(&self, sent_payments: &[&PendingPayable]) {
        fn missing_record_msg(nonexistent: &[PendingPayableMissingInDb]) -> String {
            format!(
                "Expected {} sent-payable records were not found. The system has become unreliable",
                nonexistent.len()
            )
        }
    ...
    fn handle_batch_results_for_new_scan(&self, batch_results: &BatchResults, logger: &Logger) {
        ...
        if failed > 0 {
            debug!(logger, "Recording {} failed transactions", failed);
    ...
    fn log_failed_txs_during_retry(failed_txs: &[FailedTx], logger: &Logger) {
        warning!(
            logger,
            "While retrying, {} transactions failed.",
            failed_txs.len()
        )
    }
...
mod tests {
    ...
    fn payable_with_debt_above_the_slope_is_qualified() {
        ...
        TestLogHandler::new().exists_log_matching(&format!(
            ...
            500,000,000,000,000,000 wei for a creditor",
            test_name
        ));
    }
