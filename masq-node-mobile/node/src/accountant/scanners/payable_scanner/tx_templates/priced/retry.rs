...
impl PricedRetryTxTemplate {
    ...
    fn create_and_update_log_data(
        ...
        let safe_gas_price_wei = if computed_gas_price_wei > ceil {
            log_builder.push(computed_gas_price_wei);
            ceil
        } else {
            computed_gas_price_wei
        };
...
impl RetryLogBuilder {
    ...
    fn push(&mut self, gas_price: u128) {
        self.log_data.push(gas_price);
    }
        ...
        if self.log_data.is_empty() {
            ...
            Some(format!(
                "The computed gas price(s) in wei is \
                 ...
                 {} transaction(s) are affected; attempted gas prices (wei):\n\
                 {}",
                self.ceil.separate_with_commas(),
                self.log_data.len(),
                join_with_separator(
                    &self.log_data,
                    |gas_price| gas_price.separate_with_commas(),
                    "\n"
                )
            ))
        }
