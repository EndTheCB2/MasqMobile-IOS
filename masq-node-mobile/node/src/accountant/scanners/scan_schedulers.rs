...
impl RescheduleScanOnErrorResolverReal {
    fn resolve_new_payables(
        ...
        match err {
            ...
            // The running scan owns the continuation of the automatic scan chain. A duplicate
            // timer or start message must therefore be dropped rather than panic or create a
...
mod tests {
    ...
    #[test]
    fn duplicate_automatic_pending_payable_scan_is_dropped_without_rescheduling() {
        let subject = ScanSchedulers::new(*TEST_SCAN_INTERVALS, true);
        let error = StartScanError::ScanAlreadyRunning {
            cross_scan_cause_opt: None,
            started_at: SystemTime::now(),
        };

        for initial_pending_payable_scan in [false, true] {
            let result = subject
                .reschedule_on_error_resolver
                .resolve_rescheduling_on_error(
                    UnableToStartScanner::PendingPayables {
            ...
            assert_eq!(result, ScanReschedulingAfterEarlyStop::DoNotSchedule);
        }
    }
    ...
    #[test]
    fn duplicate_automatic_new_payable_scan_is_dropped_without_rescheduling() {
        let subject = ScanSchedulers::new(*TEST_SCAN_INTERVALS, true);
    ...
    #[test]
    fn duplicate_automatic_receivable_scan_is_dropped_without_rescheduling() {
        let subject = ScanSchedulers::new(*TEST_SCAN_INTERVALS, true);
