...
impl TxReceiptInterpreter {
    ...
    fn handle_still_pending_tx(
        ...
        match tx {
            TxByTable::SentPayable(sent_tx) => {
                info!(
                    logger,
                    "A transaction was not confirmed within {} ms and will be resubmitted with a higher gas price",
            ...
            TxByTable::FailedPayable(failed_tx) => {
                ...
                if replacement_tx.iter().next().is_none() {
                    panic!(
                        "Expected a replacement transaction but couldn't find one in the database"
                    )
                }
                warning!(
                    logger,
                    "A previously failed transaction was still pending unexpectedly; its replacement should be pending instead"
                );
    ...
    fn handle_tx_confirmation(
        ...
        match tx {
            TxByTable::SentPayable(sent_tx) => {
                info!(logger, "A pending transaction was confirmed");
            ...
            TxByTable::FailedPayable(failed_tx) => {
                info!(
                    logger,
                    "A previously failed transaction was confirmed and will be reclaimed"
                );
    ...
    fn handle_reverted_tx(
        ...
        match tx {
            TxByTable::SentPayable(sent_tx) => {
                ...
                warning!(logger, "A pending transaction reverted");
            ...
            TxByTable::FailedPayable(failed_tx) => {
                debug!(
                    logger,
                    "A reverted transaction was rechecked after {}. Status will be changed to \"Concluded\"",
                    failed_tx.reason,
                );
    ...
    fn handle_rpc_failure(
        ...
        warning!(
            logger,
            "Failed to retrieve a transaction receipt: {:?}. Will retry receipt retrieval next cycle",
            rpc_error
        );
...
mod tests {
    ...
    fn interprets_receipt_for_pending_tx_if_it_is_a_success() {
        ...
        TestLogHandler::new().exists_log_containing(&format!(
            "INFO: {test_name}: A pending transaction was confirmed"
        ));
    }
    ...
    fn interprets_receipt_for_failed_tx_being_rechecked_when_it_is_a_success() {
        ...
        TestLogHandler::new().exists_log_containing(&format!(
            "INFO: {test_name}: A previously failed transaction was confirmed and will be reclaimed"
        ));
    }
    ...
    fn interprets_tx_receipt_for_pending_tx_when_tx_status_says_reverted() {
        ...
        TestLogHandler::new().exists_log_containing(&format!(
            "WARN: {test_name}: A pending transaction reverted"
        ));
    }
    ...
    fn interprets_tx_receipt_for_failed_tx_when_newly_fetched_tx_status_says_reverted() {
        ...
        TestLogHandler::new().exists_log_containing(&format!(
            "DEBUG: {test_name}: A reverted transaction was rechecked after \"PendingTooLong\". Status will be changed to \"Concluded\""
        ));
    }
    ...
    fn interprets_tx_receipt_for_pending_payable_if_the_tx_keeps_pending() {
        ...
        let log_idx = log_handler.exists_log_matching(&format!(
            "INFO: {test_name}: A transaction was not confirmed within \
            \\d{{1,3}}(,\\d{{3}})* ms and will be resubmitted with a higher gas price"
        ));
    ...
    fn interprets_tx_receipt_for_supposedly_failed_tx_if_the_tx_keeps_pending() {
        ...
        TestLogHandler::new().exists_log_containing(&format!(
            "WARN: {test_name}: A previously failed transaction was still pending unexpectedly; its replacement should be pending instead"
        ));
    }
    ...
    #[test]
    #[should_panic(
        expected = "Expected a replacement transaction but couldn't find one in the database"
    )]
    ...
    fn test_failed_retrieval_of_receipt_for_pending_payable(
        ...
        TestLogHandler::new().exists_log_containing(&format!(
            "WARN: {test_name}: Failed to retrieve a transaction receipt: Remote(InvalidResponse(\"blah\")). \
    ...
    fn test_failed_retrieval_of_receipt_for_failed_tx(
        ...
        TestLogHandler::new().exists_log_containing(&format!(
            "WARN: {test_name}: Failed to retrieve a transaction receipt: Local(Internal). \
            Will retry receipt retrieval next cycle"
        ));
    }
}
