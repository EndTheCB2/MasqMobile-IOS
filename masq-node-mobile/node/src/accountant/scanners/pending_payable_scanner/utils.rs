...
impl ReceiptScanReport {
    pub fn requires_payments_retry(&self) -> Option<Retry> {
        match (
            ...
            (None, true) => Some(Retry::RetryTxStatusCheckOnly),
            (None, _) => None,
...
impl PendingPayableCache<SentTx> for CurrentPendingPayables {
    ...
    fn ensure_empty_cache(&mut self, logger: &Logger) {
        if !self.sent_payables.is_empty() {
            debug!(
                logger,
                "Cache misuse - {} pending payables left unprocessed. Dumping.",
                self.sent_payables.len()
            );
        }
...
impl PendingPayableCache<FailedTx> for RecheckRequiringFailures {
    ...
    fn ensure_empty_cache(&mut self, logger: &Logger) {
        if !self.failures.is_empty() {
            debug!(
                logger,
                "Cache misuse - {} tx failures left unprocessed. Dumping.",
                self.failures.len()
            );
        }
        self.failures.clear()
...
mod tests {
    ...
    #[test]
    fn requires_payments_retry_repeats_status_check_with_no_results_in_whole_summary() {
        ...
        assert_eq!(
            report.requires_payments_retry(),
            Some(Retry::RetryTxStatusCheckOnly)
        );
    }
    ...
    fn pending_payable_cache_ensure_empty_sad_path() {
        ...
        TestLogHandler::default().exists_log_containing(&format!(
            "DEBUG: {test_name}: \
        Cache misuse - 1 pending payables left unprocessed. Dumping."
        ));
    }
    ...
    fn failure_cache_ensure_empty_sad_path() {
        ...
        TestLogHandler::default().exists_log_containing(&format!(
            "DEBUG: {test_name}: \
        Cache misuse - 1 tx failures left unprocessed. Dumping."
        ));
    }
