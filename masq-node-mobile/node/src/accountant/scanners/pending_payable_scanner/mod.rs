...
impl PendingPayableScanner {
    ...
    fn interpret_tx_receipts(
        ...
        if msg.results.is_empty() {
            warning!(
                logger,
                "Received an empty transaction-receipt batch; the status check will be repeated"
            );
        }
    ...
    fn handle_tx_failure_reclaims(&mut self, reclaimed: Vec<SentTx>, logger: &Logger) {
        ...
        debug!(logger, "Processing {} failure reclaims", reclaimed.len());
    ...
    fn collect_hashes_and_blocks(reclaimed: &[SentTx]) -> HashMap<TxHash, TxBlock> {
        reclaimed
            .iter()
            .map(|reclaim| {
                let tx_block = if let TxStatus::Confirmed {
                    block_hash,
                    block_number,
                    ..
                } = &reclaim.status
                {
                    TxBlock {
                        block_hash: H256::from_str(&block_hash[2..])
                            .expect("Failed to construct hash from str"),
                        block_number: (*block_number).into(),
                    }
                } else {
                    panic!("Processing a reclaim without the required confirmation details")
                };
    ...
    fn replace_sent_tx_records(
        ...
        match self.sent_payable_dao.replace_records(&btreeset) {
            ...
            Err(e) => {
                panic!(
                    "Unable to proceed in a reclaim because replacement of {} sent transaction records failed: {:?}",
                    hashes_and_blocks.len(),
                    e
                )
            }
        }
    ...
    fn delete_failed_tx_records(&self, hashes_and_blocks: &[(TxHash, TxBlock)], logger: &Logger) {
        ...
        match self.failed_payable_dao.delete_records(&hashes) {
            Ok(_) => {
                info!(
                    logger,
                    "Reclaimed {} transactions as confirmed on-chain",
                    hashes_and_blocks.len()
                )
            }
            Err(e) => {
                panic!(
                    "Unable to delete {} failed transaction records to finish the reclaims: {:?}",
                    hashes_and_blocks.len(),
                    e
                )
            }
    ...
    fn confirm_transactions(&mut self, confirmed_sent_txs: &[SentTx]) {
    ...
    fn log_tx_success(logger: &Logger, tx_hashes_and_tx_blocks: &HashMap<TxHash, TxBlock>) {
        logger.info(|| {
            format!(
                "Recorded {} confirmed transaction(s) in the local ledger",
                tx_hashes_and_tx_blocks.len()
            )
        });
    }
    ...
    fn transaction_confirmed_panic(confirmed_txs: &[SentTx], e: PayableDaoError) -> ! {
        panic!(
            "Unable to complete confirmation of {} transactions by adjusting payable accounts: {:?}",
            confirmed_txs.len(),
            e
        )
    }
    fn update_tx_blocks_panic(
        ...
        panic!(
            "Unable to update {} sent payable records with their transaction blocks: {:?}",
            tx_hashes_and_tx_blocks.len(),
            e
        )
    }
    ...
    fn add_new_failures(&self, new_failures: Vec<FailedTx>, logger: &Logger) {
        ...
        fn log_procedure_finished(logger: &Logger, new_failures: &[FailedTx]) {
            info!(
                logger,
                "{} failed transactions were processed in the database",
                new_failures.len()
            )
        }
        ...
        debug!(
            logger,
            "Processing {} reverted transactions",
            new_failures.len()
        );
        ...
        {
            panic!(
                "Unable to persist {} failed transactions: {:?}",
                new_failures.len(),
                e
            )
        }
    ...
    fn finalize_suspected_failures(&self, rechecks_completed: Vec<TxHash>, logger: &Logger) {
            ...
            .update_statuses(&prepare_hashmap(&rechecks_completed))
        {
            ...
            Err(e) => {
                panic!(
                    "Unable to conclude rechecks for {} failed transactions: {:?}",
                    rechecks_completed.len(),
                    e
                )
            }
            ...
            if !updatable.is_empty() {
                match self.sent_payable_dao.update_statuses(&updatable) {
                    Ok(_) => {
                        info!(
                            logger,
                            "{} pending-transaction statuses were processed for validation failures",
                            sent_payable_failures.len()
                        )
                    }
                    Err(e) => {
                        panic!(
                            "Unable to update {} pending-transaction statuses for validation failures: {:?}",
                            sent_payable_failures.len(),
                            e
                        )
                    }
            ...
            if !updatable.is_empty() {
                match self.failed_payable_dao.update_statuses(&updatable) {
                    Ok(_) => {
                        info!(
                            logger,
                            "{} failed-transaction statuses were processed for validation failures",
                            failed_txs_validation_failures.len()
                        )
                    }
                    Err(e) => {
                        panic!(
                            "Unable to update {} failed-transaction statuses for validation failures: {:?}",
                            failed_txs_validation_failures.len(),
                            e
                        )
                    }
...
mod tests {
    ...
    #[test]
    fn finish_scan_repeats_status_check_when_receipt_provider_returns_an_empty_batch() {
        let logger = Logger::new("test");
        let mut subject = PendingPayableScannerBuilder::new().build();
    ...
    fn handle_failed_transactions_can_process_standard_tx_failures() {
        ...
        TestLogHandler::new().exists_log_containing(&format!(
            "INFO: {test_name}: 2 failed transactions were processed in the database"
        ));
    }
    ...
    fn handle_failed_transactions_can_process_receipt_retrieval_rpc_failures() {
                ...
                FailedValidationByTable::FailedPayable(FailedValidation::new(
                    ...
                    FailureStatus::RecheckRequired(ValidationStatus::Reattempting(
                        PreviousAttempts::new(
                            ...
                            &SimpleClockMock::default().now_result(timestamp_d),
                        ),
                    )),
                )),
        ...
        let expected_params = hashmap!(
                hash_1 => FailureStatus::RecheckRequired(
                    ValidationStatus::Reattempting(PreviousAttempts::new(BlockchainErrorKind::AppRpc(AppRpcErrorKind::Remote(RemoteErrorKind::Unreachable)), &SimpleClockMock::default().now_result(timestamp_b)))
                ),
                hash_2 => FailureStatus::RecheckRequired(
                    ValidationStatus::Reattempting(PreviousAttempts::new(BlockchainErrorKind::AppRpc(AppRpcErrorKind::Local(LocalErrorKind::Internal)), &SimpleClockMock::default().now_result(timestamp_d)).add_attempt(BlockchainErrorKind::AppRpc(AppRpcErrorKind::Local(LocalErrorKind::Internal)), &SimpleClockReal::default())))
        ...
        test_log_handler.exists_log_containing(&format!(
            "INFO: {test_name}: 1 pending-transaction statuses were processed for validation failures"
        ));
        test_log_handler.exists_log_containing(&format!(
            "INFO: {test_name}: 2 failed-transaction statuses were processed for validation failures"
        ));
    ...
    #[test]
    #[should_panic(
        expected = "Unable to update 1 pending-transaction statuses for validation failures: InvalidInput(\"blah\")"
    )]
    fn update_validation_status_for_sent_txs_panics_on_update_statuses() {
        ...
        let subject = PendingPayableScannerBuilder::new()
            .sent_payable_dao(sent_payable_dao)
            .validation_failure_clock(Box::new(SimpleClockReal::default()))
            .build();
    ...
    #[test]
    #[should_panic(
        expected = "Unable to update 1 failed-transaction statuses for validation failures: InvalidInput(\"blah\")"
    )]
    ...
    fn handle_failed_transactions_can_process_mixed_failures() {
        ...
        let subject = PendingPayableScannerBuilder::new()
            .sent_payable_dao(sent_payable_dao)
            .failed_payable_dao(failed_payable_dao)
            .validation_failure_clock(Box::new(SimpleClockMock::default().now_result(timestamp)))
            .build();
        let detected_failures = DetectedFailures {
        ...
        assert_eq!(
            *update_statuses_params,
            vec![
                hashmap!(tx_hash_2 => TxStatus::Pending(ValidationStatus::Reattempting(PreviousAttempts::new(BlockchainErrorKind::AppRpc(AppRpcErrorKind::Local(LocalErrorKind::Internal)), &SimpleClockMock::default().now_result(timestamp)))))
            ]
        );
    }

    #[test]
    #[should_panic(expected = "Unable to persist 2 failed transactions: NoChange")]
    ...
    #[test]
    #[should_panic(expected = "Unable to conclude rechecks for 2 failed transactions: \
    InvalidInput(\"Booga\")")]
    ...
    fn handles_failure_reclaims_alone() {
        ...
        log_handler.exists_log_containing(&format!(
            "INFO: {test_name}: Reclaimed 2 transactions as confirmed on-chain"
        ));
    }

    #[test]
    #[should_panic(
        expected = "Unable to proceed in a reclaim because replacement of 2 sent transaction records failed: NoChange"
    )]
    ...
    #[test]
    #[should_panic(
        expected = "Unable to delete 2 failed transaction records to finish the reclaims: EmptyInput"
    )]
    ...
    #[test]
    #[should_panic(expected = "Processing a reclaim without the required confirmation details")]
    ...
    fn handles_standard_confirmations_alone() {
        ...
        let log_handler = TestLogHandler::new();
        log_handler.exists_log_containing(&format!(
            "INFO: {test_name}: Recorded 2 confirmed transaction(s) in the local ledger"
        ));
    }
    ...
    fn mixed_tx_confirmations_work() {
        ...
        let log_handler = TestLogHandler::new();
        log_handler.exists_log_containing(&format!(
            "INFO: {test_name}: Reclaimed 1 transactions as confirmed on-chain"
        ));
        log_handler.exists_log_containing(&format!(
            "INFO: {test_name}: Recorded 1 confirmed transaction(s) in the local ledger"
        ));
    }

    #[test]
    #[should_panic(
        expected = "Unable to update 2 sent payable records with their transaction blocks: \
        SqlExecutionFailed(\"The database manager is \
    ...
    #[test]
    #[should_panic(
        expected = "Unable to complete confirmation of 1 transactions by adjusting payable accounts: \
    ...
    fn log_tx_success_is_agnostic_to_singular_or_plural_form() {
        ...
        let log_handler = TestLogHandler::new();
        log_handler.exists_log_containing(&format!(
            "INFO: {plural_case_name}: Recorded 2 confirmed transaction(s) in the local ledger"
        ));
        log_handler.exists_log_containing(&format!(
            "INFO: {singular_case_name}: Recorded 1 confirmed transaction(s) in the local ledger"
        ));
    }
