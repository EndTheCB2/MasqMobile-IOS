...
mod tests {
    ...
    #[test]
    fn pending_payable_scanner_skips_automatic_scan_when_no_pending_payables_are_known() {
        let consuming_wallet = make_paying_wallet(b"consuming");
        let mut subject = make_dull_subject();
        let pending_payable_scanner = PendingPayableScannerBuilder::new().build();
        subject.pending_payable = Box::new(pending_payable_scanner);
        ...
        assert_eq!(result, Err(StartScanError::NothingToProcess));
        assert_eq!(subject.pending_payable.scan_started_at(), None);
    }
    ...
    fn pending_payable_scanner_handles_tx_receipts_message() {
        ...
        test_log_handler.exists_log_containing(&format!("WARN: {test_name}: Failed to retrieve a transaction receipt: Remote(Unreachable). Will retry receipt retrieval next cycle"));
        test_log_handler.exists_log_containing(&format!("WARN: {test_name}: Failed to retrieve a transaction receipt: Remote(InvalidResponse(\"game over\")). Will retry receipt retrieval next cycle"));
        test_log_handler.exists_log_containing(&format!(
    ...
    #[test]
    fn pending_payable_scanner_repeats_status_check_for_empty_receipt_batch() {
        ...
        let result = subject.finish_pending_payable_scan(msg, &Logger::new("test"));
    ...
    fn receivable_scanner_can_initiate_a_scan() {
        ...
        TestLogHandler::new()
            .exists_log_containing(&format!("INFO: {test_name}: Scanning for receivables"));
    }
    ...
    fn receivable_scanner_scans_for_delinquencies() {
        ...
        tlh.exists_log_matching(
            "INFO: DELINQUENCY_TEST: A wallet \\(balance: 1,234 gwei, age: \\d+ sec\\) \
            was banned for delinquency",
        );
        tlh.exists_log_matching(
            "INFO: DELINQUENCY_TEST: A wallet \\(balance: 2,345 gwei, age: \\d+ sec\\) \
            was banned for delinquency",
        );
        tlh.exists_log_matching(
            "INFO: DELINQUENCY_TEST: A wallet \\(balance: 3,456 gwei, age: \\d+ sec\\) \
            is no longer delinquent and was unbanned",
        );
        tlh.exists_log_matching(
            "INFO: DELINQUENCY_TEST: A wallet \\(balance: 4,567 gwei, age: \\d+ sec\\) \
            is no longer delinquent and was unbanned",
        );
    }
