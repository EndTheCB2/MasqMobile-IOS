...
impl ReceivableDaoReal {
    ...
    fn verify_possibly_unknown_wallet(
        ...
        match Self::is_row_present(txn, address) {
            false => {
                info!(
                    logger,
                    "Received {amount} wei from an unknown debtor in block {block_number}."
                );
                Ok(())
            }
            true => Err(RusqliteError(format!(
                "Update for received payment with {amount} wei ran without producing a change \
                    in the database, despite a matching wallet record."
            ))),
        }
    }
    ...
    fn log_more_money_received_error_with_roll_back(
        ...
        let received_in_total = payments.iter().fold(0_u128, |total, payment| {
            total.saturating_add(payment.wei_amount)
        });
        error!(
            logger,
            "Payment reception failed; rolling back {} transactions totaling {} wei",
            payments.len(),
            received_in_total
        );
    }
...
mod tests {
    ...
    fn more_money_received_ignores_unknown_address_without_affecting_the_good_ones() {
        ...
        log_handler.exists_log_containing(&format!(
            "INFO: {test_name}: Received 2222 wei from an unknown debtor in block 4446."
        ));
    ...
    fn more_money_received_leaves_transactions_uncommitted_if_panic_occurs() {
        ...
        log_handler.exists_log_containing(&format!(
            "ERROR: {test_name}: Payment reception failed; rolling back 2 transactions totaling 156900 wei"
        ));
    ...
    fn verify_possibly_unknown_wallet_returns_error_when_the_row_for_this_wallet_exists() {
        ...
        without producing a change in the database, despite a matching wallet record.";
