...
impl PayableDao for PayableDaoReal {
    ...
    fn transactions_confirmed(
        ...
        if confirmed_payables.is_empty() {
            return Ok(());
        }

        let big_int_db_processor = &self.big_int_db_processor;
        let transaction = self.conn.transaction().map_err(|_| {
            PayableDaoError::RusqliteError(
                "Unable to start confirmed-payable database transaction".to_string(),
            )
        })?;
        ...
        confirmed_payables.iter().try_for_each(|confirmed_payable| {
            ...
            big_int_db_processor.execute(Either::Right(&transaction), BigIntSqlConfig::new(
                main_sql,
        ...
        transaction.commit().map_err(|_| {
            PayableDaoError::RusqliteError(
                "Unable to commit confirmed-payable database transaction".to_string(),
            )
        })
    }
...
mod tests {
    ...
    fn test_transaction_confirmed_works(
        ...
        let mut subject = PayableDaoReal::new(boxed_conn);
    ...
    fn transaction_confirmed_works_for_generic_sql_error() {
        ...
        let mut subject = PayableDaoReal::new(conn_wrapped);
    ...
    fn transaction_confirmed_works_for_overflow_from_sent_tx_record() {
        ...
        let mut subject = PayableDaoReal::new(
            DbInitializerReal::default()
                .initialize(&home_dir, DbInitializationConfig::test_default())
                .unwrap(),
        );
    ...
    #[test]
    fn transactions_confirmed_roll_back_earlier_update_when_later_update_fails() {
        let home_dir = ensure_node_home_directory_exists(
            "payable_dao",
            "transactions_confirmed_roll_back_earlier_update_when_later_update_fails",
        );
        let conn = DbInitializerReal::default()
            .initialize(&home_dir, DbInitializationConfig::test_default())
            .unwrap();
        ...
        conn.prepare("delete from payable where wallet_address = ?")
            ...
            .unwrap();
        let mut subject = PayableDaoReal::new(conn);
        let account_1_before = subject.account_status(&wallet_1).unwrap();
        ...
        let account_1_after = subject.account_status(&wallet_1).unwrap();
        assert_eq!(account_1_after, account_1_before);
        let account_2_opt = subject.account_status(&wallet_2);
        assert_eq!(account_2_opt, None);
    }

    #[test]
    fn transactions_confirmed_skips_database_transaction_for_empty_batch() {
        let mut subject = PayableDaoReal::new(Box::new(
            crate::database::test_utils::ConnectionWrapperMock::default(),
        ));
    ...
    #[test]
    fn transactions_confirmed_returns_bounded_error_when_transaction_cannot_start() {
        let conn = crate::database::test_utils::ConnectionWrapperMock::default()
            .transaction_result(Err(rusqlite::Error::SqliteSingleThreadedMode));
        let mut subject = PayableDaoReal::new(Box::new(conn));

        let result = subject.transactions_confirmed(&[make_sent_tx(123)]);
