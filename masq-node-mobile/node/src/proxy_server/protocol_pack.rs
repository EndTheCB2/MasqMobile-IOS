...
pub fn from_ibcd(ibcd: &InboundClientData) -> Result<Box<dyn ProtocolPack>, String> {
    let origin_port = match ibcd.reception_port_opt {
        None => {
            return Err(format!(
                "No origin port specified with {}-byte non-clandestine packet; contents redacted",
                ibcd.data.len()
            ))
        }
    ...
    match from_standard_port(origin_port) {
        Some(pp) => Ok(pp),
        None => Err(format!(
            "No protocol associated with origin port {} for {}-byte non-clandestine packet; contents redacted",
            origin_port,
            ibcd.data.len()
        )),
    }
