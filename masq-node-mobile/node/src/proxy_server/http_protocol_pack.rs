...
impl ProtocolPack for HttpProtocolPack {
    ...
    fn find_host(&self, data: &PlainData) -> Option<Host> {
        // RFC 7231 defines CONNECT's request target in authority-form (host:port). Some Apple
        // clients omit the port from the Host header even though it is present in that target.
        // For CONNECT, the request target is therefore authoritative; using the Host header
        // first would incorrectly turn a TLS tunnel to port 443 into an HTTP request to port 80.
        if HttpProtocolPack::is_connect(data.as_slice()) {
            return HttpProtocolPack::find_url_host(data.as_slice())
                .or_else(|| HttpProtocolPack::find_header_host(data.as_slice()));
        }
    ...
    fn describe_packet(&self, data: &PlainData) -> String {
        if data.as_slice().starts_with(b"HTTP/") {
            format!("HTTP response ({} bytes); contents redacted", data.len())
        } else {
            format!(
                "HTTP request ({} bytes); target and contents redacted",
                data.len()
            )
        }
    }
...
mod tests {
    ...
    #[test]
    fn connect_uses_authority_port_when_host_header_omits_it() {
        let data = PlainData::new(
            b"CONNECT masqbrowser.com:443 HTTP/1.1\r\nHost: masqbrowser.com\r\nProxy-Connection: keep-alive\r\n\r\n",
        );

        let host = HttpProtocolPack {}.find_host(&data).unwrap();

        assert_eq!(host, Host::new("masqbrowser.com", 443));
    }
