    ...
    fn make(
        ...
        let host_from_request_result_closure = Box::new(|| {
            ...
            match protocol_pack.find_host(&data) {
                ...
                None => Err(format!(
                    "No hostname information found in client packet or ProxyServer for protocol {:?}; packet and stream identifiers redacted",
        ...
        let target_host = match (host_from_request_result_closure(), host_from_history_opt) {
            ...
            (Err(e), None) => {
                if ibcd.last_data && ibcd.data.is_empty() {
                    warning!(
                        logger,
                        "Client opened {:?} connection and immediately closed it without sending any data; stream identifier redacted",
                        protocol_pack.proxy_protocol()
                    );
                } else {
                    error!(logger, "{}", e);
...
mod tests {
    ...
    fn logs_error_and_returns_none_if_no_ibcd_host_and_no_supplied_host() {
        ...
        TestLogHandler::new().exists_log_containing(&format!("ERROR: {test_name}: No hostname information found in client packet or ProxyServer for protocol HTTP; packet and stream identifiers redacted"));
    }
    ...
    fn handles_tls_without_hostname() {
        ...
        assert_eq!(result, None);
        TestLogHandler::new().exists_log_containing(&format!("ERROR: {test_name}: No hostname information found in client packet or ProxyServer for protocol TLS; packet and stream identifiers redacted"));
    }
    ...
    fn makes_no_payload_if_origin_port_is_not_specified() {
        ...
        TestLogHandler::new().exists_log_containing(
            &format!("ERROR: {test_name}: No origin port specified with 3-byte non-clandestine packet; contents redacted"),
        );
    }
    ...
    fn makes_no_payload_if_origin_port_is_unknown() {
        ...
        assert_eq!(result, None);
        TestLogHandler::new().exists_log_containing(&format!("ERROR: {test_name}: No protocol associated with origin port 1234 for 3-byte non-clandestine packet; contents redacted"));
    }
