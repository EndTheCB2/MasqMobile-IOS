...
pub const RECEIPT_SETTLEMENT_CONTRACT_HELP: &str =
    "The independently deployed MASQ receipt-settlement verifier/escrow address. This is not the \
     MASQ ERC-20 token address. Receipt sessions and receipt capability advertisement remain disabled \
     unless this explicit nonzero address is configured.";
...
pub const MOBILE_PROXY_PORT_HELP: &str =
    "Bind the consume-only browser proxy to this unprivileged \
     loopback port while treating accepted traffic as HTTPS/CONNECT traffic for port 443. This is \
     intended for embedded mobile clients and requires --neighborhood-mode consume-only.";
pub const NEIGHBORS_HELP: &str = "Up to ten Node descriptors for active Nodes in the MASQ Network to which you'd like your Node to connect \
     on startup. Every descriptor must use the configured chain and a unique public key. A Node descriptor looks similar to one of these:\n\n\
...
pub fn shared_app(head: App<'static, 'static>) -> App<'static, 'static> {
    ...
    .arg(chain_arg())
    .arg(
        Arg::with_name("receipt-settlement-contract")
            .long("receipt-settlement-contract")
            .value_name("ADDRESS")
            .min_values(0)
            .max_values(1)
            .validator(common_validators::validate_nonzero_ethereum_address)
            .help(RECEIPT_SETTLEMENT_CONTRACT_HELP),
    )
    .arg(
    ...
    .arg(
        Arg::with_name("mobile-proxy-port")
            .long("mobile-proxy-port")
            .value_name("PORT")
            .min_values(0)
            .max_values(1)
            .validator(common_validators::validate_clandestine_port)
            .help(MOBILE_PROXY_PORT_HELP),
    )
    .arg(min_hops_arg())
    .arg(
...
pub mod common_validators {
    ...
    pub fn validate_nonzero_ethereum_address(address: String) -> Result<(), String> {
        validate_ethereum_address(address.clone())?;
        if address[2..].chars().all(|character| character == '0') {
            Err(address)
        } else {
            Ok(())
        }
...
mod tests {
    ...
    #[test]
    fn receipt_settlement_contract_validator_rejects_zero_and_malformed_addresses() {
        assert_eq!(
            common_validators::validate_nonzero_ethereum_address(
                "0x1111111111111111111111111111111111111111".to_string()
            ),
            Ok(())
        );
        assert!(common_validators::validate_nonzero_ethereum_address(
            "0x0000000000000000000000000000000000000000".to_string()
        )
        .is_err());
        assert!(
            common_validators::validate_nonzero_ethereum_address("not-an-address".to_string())
                .is_err()
        );
    }
    ...
    fn constants_have_correct_values() {
        ...
        assert_eq!(
            NEIGHBORS_HELP,
            "Up to ten Node descriptors for active Nodes in the MASQ Network to which you'd like your Node to connect \
             on startup. Every descriptor must use the configured chain and a unique public key. A Node descriptor looks similar to one of these:\n\n\
                  masq://polygon-mainnet:d2U3Dv1BqtS5t_Zz3mt9_sCl7AgxUlnkB4jOMElylrU@172.50.48.6:9342\n\
                  masq://eth-mainnet:gBviQbjOS3e5ReFQCvIhUM3i02d1zPleo1iXg_EN6zQ@86.75.30.9:5542\n\
                  masq://base-mainnet:ZjPLnb9RrgsRM1D9edqH8jx9DkbPZSWqqFqLnmdKhsk@112.55.78.0:7878\n\
