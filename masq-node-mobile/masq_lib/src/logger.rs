...
/// Clears the process-local UI log recipient after an embedded Node actor system exits.
///
/// Desktop Node still treats replacement without teardown as an invariant violation in
/// `prepare_log_recipient`. Mobile embedding is the only lifecycle that intentionally starts a
/// later actor system in the same process.
pub fn clear_log_recipient() {
    LOG_RECIPIENT_OPT
        .lock()
...
impl Logger {
    ...
    fn transmit(msg: String, log_level: SerializableLogLevel) {
        ...
        if let Some(recipient) = recipient {
            ...
            // UI log broadcast is best-effort. During embedded mobile teardown a sender may have
            // cloned this Recipient just before the runtime clears it; a closed UiGateway must
            // never turn that benign shutdown race into a process panic.
...
mod tests {
    ...
    #[test]
    fn clearing_the_log_recipient_is_idempotent_and_allows_a_later_actor_system() {
        let _test_guard = TEST_LOG_RECIPIENT_GUARD
            .lock()
            .expect("Unable to lock TEST_LOG_RECIPIENT_GUARD");
        clear_log_recipient();
        let recording = Arc::new(Mutex::new(vec![]));
        let _system = System::new("clear_log_recipient");
        let first = TestUiGateway::new(0, &recording).start().recipient();
        prepare_log_recipient(first);

        clear_log_recipient();
        clear_log_recipient();

        let second = TestUiGateway::new(0, &recording).start().recipient();
        prepare_log_recipient(second);
        clear_log_recipient();
    }
    ...
    #[test]
    fn transmit_ignores_a_recipient_closed_during_teardown() {
        let _guard = prepare_test_environment();
        let system = System::new("Closed log recipient");
        let recording = Arc::new(Mutex::new(vec![]));
        let recipient = TestUiGateway::new(0, &recording).start().recipient();
        LOG_RECIPIENT_OPT.lock().unwrap().replace(recipient);
        System::current().stop();
        system.run();

        Logger::transmit("Late shutdown log".to_owned(), Level::Warn.into());
    }
