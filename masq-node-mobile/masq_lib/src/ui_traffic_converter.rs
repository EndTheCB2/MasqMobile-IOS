...
// Error details deliberately retain only JSON types, never caller-controlled values: malformed
// local UI traffic can contain passwords, wallet material or signatures.
...
impl Display for TrafficConversionError {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> Result<(), std::fmt::Error> {
        match self {
            ...
            NotJsonObjectError(json_type) => write!(
                f,
                "Root JSON value should have been an object but had type {}",
                json_type
            ),
            MissingFieldError(field) => write!(f, "Required field was missing: {}", field),
            FieldTypeError(field, wanted, got) => write!(
                f,
                "Field {} should have been of type {}, but had JSON type {}",
                field, wanted, got
            ),
        }
    }
...
impl UiTrafficConverter {
    ...
    pub fn new_unmarshal(json: &str) -> Result<MessageBody, UnmarshalError> {
        match serde_json::from_str(json) {
                ...
                match map.get("payload") {
                    ...
                    Some(other_value) => Err(NonCritical(
                        ...
                        FieldTypeError(
                            "payload".to_string(),
                            "object".to_string(),
                            Self::json_type(other_value).to_string(),
                        ),
                    )),
            ...
            Ok(e) => Err(Critical(NotJsonObjectError(
                Self::json_type(&e).to_string(),
            ))),
        ...
        match map.get(name) {
            Some(Value::Number(n)) => match n.as_u64() {
                ...
                None => Err(FieldTypeError(
                    name.to_string(),
                    "u64".to_string(),
                    Self::json_type(&Value::Number(n.clone())).to_string(),
                )),
            },
            Some(x) => Err(FieldTypeError(
                name.to_string(),
...
mod tests {
    ...
    fn new_unmarshaling_handles_badly_typed_json() {
        ...
        assert_eq!(
            result,
            Err(Critical(NotJsonObjectError("array".to_string())))
        );
    }
    ...
    fn display_works_for_traffic_conversion_error() {
        ...
        assert_eq!(
            ...
            "Root JSON value should have been an object but had type a".to_string()
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            "Field a should have been of type b, but had JSON type c".to_string()
        );
    }

    #[test]
    fn conversion_errors_never_include_mistyped_sensitive_values() {
        let sensitive = "SENSITIVE_VALUE_SHOULD_NOT_LEAK";
        let json = format!(r#"{{"opcode":"whomp","payload":"{}"}}"#, sensitive);

        let error = UiTrafficConverter::new_unmarshal_from_ui(&json, 1234).unwrap_err();
