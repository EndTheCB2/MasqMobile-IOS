...
impl Debug for NameValueVclArg {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("NameValueVclArg")
...
#[cfg(test)]
mod name_value_vcl_arg_debug_tests {
    use super::*;

    #[test]
    fn debug_redacts_arbitrary_configuration_values() {
        let subject = NameValueVclArg::new("db-password", "SENSITIVE_CONFIG_VALUE");

        let debug = format!("{:?}", subject);

        assert!(debug.contains("db-password"));
        assert!(debug.contains("[REDACTED]"));
        assert!(!debug.contains("SENSITIVE_CONFIG_VALUE"));
    }
}
