...
/// Optional embedding policy applied before a flow consumes session capacity.
///
/// Defaults preserve upstream tun2proxy behavior. Embedders with a more
/// restrictive downstream proxy can fail closed without creating a proxy
/// handler that is guaranteed to reject the flow.
#[derive(Debug, Clone, Default)]
pub struct SessionPolicy {
    /// Reject IPv6 flows before they consume session capacity. This is
    /// separate from `Args::ipv6_enabled`, which controls downstream proxy and
    /// DNS behavior rather than whether the TUN must capture IPv6 fail closed.
    pub reject_ipv6: bool,

    /// Reject every UDP flow except DNS handled by the virtual DNS responder.
    pub reject_udp_except_virtual_dns: bool,

    /// When non-empty, reject TCP destinations whose port is not in this list.
    pub allowed_tcp_ports: Vec<u16>,

    /// Bound proxy socket creation plus protocol negotiation (for example an
    /// HTTP CONNECT exchange). `None` preserves the upstream unbounded
    /// behavior. The timeout future owns the proxy socket, so expiry closes a
    /// half-open setup fail closed and releases the session permit.
    pub proxy_handshake_timeout: Option<Duration>,

    /// Close an established TCP relay after this much time without payload in
    /// either direction. Payload activity resets the shared deadline. `None`
    /// preserves upstream behavior; TCP/application keep-alive payload and
    /// active transfers remain unaffected.
...
pub struct Args {
    ...
    /// Embedding-only transport restrictions, evaluated before session
    /// accounting. This is intentionally not exposed as a CLI flag.
    #[arg(skip)]
    #[serde(skip)]
