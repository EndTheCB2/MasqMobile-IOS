...
impl IpStackTcpStream {
    ...
    /// Abort this accepted userspace TCP connection immediately and notify
    /// the downstream peer with RST/ACK.
    ///
    /// This is intended for policy rejection and capacity refusal after
    /// `IpStack::accept()` has already emitted a SYN/ACK. It never waits for a
    /// graceful FIN exchange, so one unacknowledged or half-open connection
    /// cannot head-of-line block a TUN accept loop. Consuming `self` also makes
    /// it impossible for a caller to reuse the rejected stream.
    pub fn abort_with_reset(mut self) -> std::io::Result<()> {
        self.reset_and_cancel()
    }

    fn reset_and_cancel(&mut self) -> std::io::Result<()> {
        ...
        let reset_result = {
            ...
            if state == TcpState::Closed {
                ...
                // The background task normally advances Listen ->
                // SynReceived and increments ACK for the incoming SYN. If a
                // policy rejects even earlier, do that increment here so the
                // reset still acknowledges the SYN correctly.
                if state == TcpState::Listen {
                    tcb.increase_ack();
                }
                let result = write_packet_to_device(&self.up_packet_sender, network_tuple, &tcb, None, ACK | RST, None, None).map(|_| ());
        ...
        self.write_notify.lock().unwrap().take().map(|waker| waker.wake_by_ref());
        self.read_notify.lock().unwrap().take().map(|waker| waker.wake_by_ref());
...
impl Drop for IpStackTcpStream {
    fn drop(&mut self) {
        ...
        if self.task_handle.as_ref().is_some_and(|task| !task.is_finished()) {
            // Stream values are frequently dropped while a Tokio runtime is
            // cancelling all relay tasks. Blocking this worker until the
            // private TCP task runs can deadlock when enough sessions are
            // cancelled together. Reset and abort instead; dropping the task
            // future releases its destroy messenger asynchronously.
            if let Err(error) = self.reset_and_cancel() {
                log::trace!("{nt} {state:?}: [drop] reset error: {error}");
            }
        } else {
...
mod masq_patch_tests {
    ...
    async fn abort_with_reset_rejects_a_syn_without_waiting_for_fin() {
        let source: SocketAddr = "10.0.0.2:41000".parse().expect("source address");
        let destination: SocketAddr = "198.18.0.1:80".parse().expect("destination address");
        let mut syn = TcpHeader::new(source.port(), destination.port(), 7, 65_535);
        syn.syn = true;
        ...
        let syn_ack_seen = tokio::time::timeout(Duration::from_millis(250), async {
            while let Some(packet) = packet_receiver.recv().await {
                if let TransportHeader::Tcp(header) = packet.transport_header()
                    && header.syn
                    && header.ack
                {
                    return true;
        ...
        .await
        .expect("SYN/ACK observation must be bounded");
        assert!(syn_ack_seen, "the userspace stack must accept the SYN first");
        assert_eq!(
            stream.tcb.lock().unwrap().get_state(),
            TcpState::SynReceived,
            "the regression requires the exact state that graceful shutdown cannot leave"
        );

        stream.abort_with_reset().expect("RST rejection");

        let reset_seen = tokio::time::timeout(Duration::from_millis(250), async {
            while let Some(packet) = packet_receiver.recv().await {
                if let TransportHeader::Tcp(header) = packet.transport_header()
                    && header.rst
                    && header.ack
        ...
        .await
        .expect("reset observation must be bounded");
        assert!(reset_seen, "the rejected peer must receive RST/ACK");
        let _ = tokio::time::timeout(Duration::from_millis(250), destroy_receiver)
            .await
            .expect("the aborted task must release its session identity");
    }

    #[tokio::test(flavor = "current_thread")]
    async fn dropping_a_live_stream_never_blocks_the_runtime_worker() {
        let source: SocketAddr = "10.0.0.2:41001".parse().expect("source address");
        let destination: SocketAddr = "198.18.0.2:443".parse().expect("destination address");
        let mut syn = TcpHeader::new(source.port(), destination.port(), 8, 65_535);
        syn.syn = true;
        let (packet_sender, mut packet_receiver) = mpsc::unbounded_channel();
        let (destroy_sender, destroy_receiver) = oneshot::channel();
        let stream = IpStackTcpStream::new(
        ...
        let _ = tokio::time::timeout(Duration::from_millis(250), packet_receiver.recv())
            .await
            .expect("SYN/ACK observation must be bounded");
        drop(stream);
        ...
        .await
        .expect("drop reset observation must be bounded");
        assert!(reset_seen, "dropping a live stream must reset its peer");
        let _ = tokio::time::timeout(Duration::from_millis(250), destroy_receiver)
            .await
            .expect("the dropped task must release its session identity");
    }
}
